...
; base.immovable = true
; base.alpha = 0.6
; #if FLX_DEBUG base.ignoreDrawDebug = true
; #end add(base)
...
; thumb.alpha = 0.6
; #if FLX_DEBUG thumb.ignoreDrawDebug = true
; #end add(thumb)
; } /** * Creates the touch zone. It's based on the size of the background. * The thumb will react when the touch is in the zone.
*/ public function createZone():Void
{ if (base != null && _radius == 0) _radius = base.width * 0.5
...
; // There is no reason to get into the loop if their is already a pointer on the analog if (_currentTouch != null) _tempTouches.push(_currentTouch)
; else
{ for (touch in FlxG.touches.list)
{ var touchInserted:Bool = false
; for (analog in _analogs)
{ // Check whether the pointer is already taken by another analog.
// TODO: check this place. This line was 'if (analog != this && analog._currentTouch != touch && touchInserted == false)' if (analog == this && analog._currentTouch != touch && !touchInserted)
...
{ offAll = false
; if (Pressed)
{ if (Touch != null) _currentTouch = Touch
; status = PRESSED
; if (JustPressed && onDown != null) onDown()
; if (status == PRESSED)
...
*/ public function getAngle():Float return _direction * FlxAngle.TO_DEG
; /** * Whether the thumb is pressed or not.
*/ public var pressed(get, never):Bool
; inline function get_pressed():Bool return status == PRESSED
; /** * Whether the thumb is just pressed or not.
*/ public var justPressed(get, never):Bool
; function get_justPressed():Bool
{ if (_currentTouch != null) return _currentTouch.justPressed && status == PRESSED
; return false
; } /** * Whether the thumb is just released or not.
*/ public var justReleased(get, never):Bool
; function get_justReleased():Bool
{ if (_currentTouch != null) return _currentTouch.justReleased && status == HIGHLIGHT
; return false
