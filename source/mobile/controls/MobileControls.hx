...
{ case 'Pad-Right': virtualPad = new FlxVirtualPad(RIGHT_FULL, NONE)
; add(virtualPad)
; case 'Pad-Left': virtualPad = new FlxVirtualPad(LEFT_FULL, NONE)
; add(virtualPad)
; case 'Pad-Custom': virtualPad = MobileControls.customVirtualPad
; add(virtualPad)
; case 'Pad-Duo': virtualPad = new FlxVirtualPad(BOTH_FULL, NONE)
; add(virtualPad)
; case 'Hitbox': hitbox = new FlxHitbox(4, Std.int(FlxG.width / 4), FlxG.height, [0xFF00FF, 0x00FFFF, 0x00FF00, 0xFF0000])
; add(hitbox)
; case 'Keyboard':
// do nothing
}
...
; if (FlxG.save.data.buttons == null) return virtualPad
; var tempCount:Int = 0
