...
; // Old vars for backwards compatbility public var groupX:Float = 90
...
; switch (char)
{ case ' ': switch (alignment)
{ case LEFT: charX += font.spaceDifference
; case CENTER: if (column < 1)
{ charX += font.spaceDifference
; } case RIGHT: charX += (column < 1) ? font.spaceDifference : -font.spaceDifference
; default:
} columns[column].text += ' '
; updateColumns()
...
; case char: var charSprite:AlphaCharacter = new AlphaCharacter(char, font)
; charSprite.x = charX
...
{ switch (character)
{ case ' ': width += font.spaceDifference
; default: var char:AlphaCharacter = columnData.characters[charIterator]
; width += char.width
; charIterator++
; }
} return width
...
; } return width
; } /** * Updates the column's text to match with the current alignment. * @param columnId The column to update.
*/ function updateColumn(columnId:Int)
{ var columnData:ColumnData = columns[columnId]
; switch (alignment)
{ case CENTER: var centerChar:Int = Math.floor((columnData.characters.length - 1) / 2)
; if (column < 1 || columnData.characters.length <= 0) return
; // Position the middle character of the column based on how many letters there are switch (columnData.characters.length)
{ case 1: columnData.characters[centerChar].x = (getHighestWidthColumn() - columnData.characters[centerChar].width) / 2 + this.x
; default: columnData.characters[centerChar].x = switch (columnData.characters.length)
{ case (_ % 2 == 0) => true: this.x + (getHighestWidthColumn() / 2) - columnData.characters[centerChar].width
; case (_ % 2 == 1) => true: this.x + (getHighestWidthColumn() - columnData.characters[centerChar].width) / 2
; default: 0
; } // Iterate backwards to the characters left from the middle character var iterator:Int = centerChar - 1
; while (iterator >= 0)
{ columnData.characters[iterator].x = (columnData.characters[iterator + 1].x) - columnData.characters[iterator].width
; iterator--
; } // Iterate forwards to reposition the characters from the right side of the middle for (i in centerChar + 1...columnData.characters.length)
{ columnData.characters[i].x = (columnData.characters[i - 1].x) + columnData.characters[i - 1].width
; }
} // Apply spaces for (i in 0...columnData.text.length)
...
{ if (spaceType == "left") spaceType = "both"
; else spaceType = "right"
; break
; }
} switch (spaceType)
{ case "left": for (char in columnData.characters)
{ char.x -= font.spaceDifference
; } case "both": var spaceOccurance:Int = 1
; var spaceIterator:Int = i - 1
; while (spaceIterator >= 0)
{ if (columnData.text.charAt(spaceIterator) != ' ')
...
; char.x += font.spaceDifference / 2
; }
} case "right": for (char in columnData.characters)
{ char.x += font.spaceDifference
; }
}
}
} case RIGHT: if (column < 1 || columnData.characters.length <= 0) return
...
{ columnData.characters[iterator].x = columnData.characters[iterator + 1].x - columnData.characters[iterator].width
; iterator--
; } // Apply Spaces var spaceType:String = "none"
; for (i in 0...columnData.text.length)
{ if (columnData.text.charAt(i) == ' ')
{ var spaceIterator:Int = i - 1
; while (spaceIterator >= 0)
{ // Character exists on the left side. if (columnData.text.charAt(spaceIterator) != ' ')
{ spaceType = "left"
; break
; } spaceIterator--
; } for (j in i + 1...columnData.text.length)
{ // Character exists on the right side. if (columnData.text.charAt(j) != ' ')
{ // Characters only exist on the right side. if (spaceType == "none")
{ spaceType = "right"
; break
; }
}
} switch (spaceType)
{ case "left": var spaceIterator:Int = i - 1
; while (spaceIterator >= 0)
{ if (columnData.text.charAt(spaceIterator) != ' ')
{ var char:AlphaCharacter = columnData.getCharAtIndex(spaceIterator)
; char.x -= font.spaceDifference
...
{ updateColumn(column)
; for (i in 0...columns.length)
{ if (i != column) updateColumn(i)
; }
} /** * Sets up the tween that moves the text group based on the current menu item selection. * @param index The index of the text group.
*/ public function setupMenuTween(index:Float)
{ if (!isMenuItem || unlockY) return
; var memberIndex:Int = menuItemGroup.indexOf(this)
; // This alphabet isn't in this array. if (memberIndex == -1) return
; // Get the index of the current selected. var targetYIndex:Int = menuItemGroup.indexOf(menuItemGroup.filter((alphabet:Alphabet) ->
{ return alphabet.targetY == 0
; } )[0])
; if (targetYIndex == -1) return
...
{ if (onMenuTweenComplete != null) onMenuTweenComplete(t)
; menuItemTween?.cancel()
; menuItemTween = null
; } menuItemTween = FlxTween.tween(this,
{ x: newX, y: newY
} , defaultTime, menuTweenOptions)
; } public function forEachCharacter(func:AlphaCharacter->Void)
{ for (char in members)
{ if (char != null) func(char)
; }
} override function get_width():Float
{ return getHighestWidthColumn()
; } // NOTE: Not an accurate represenation of the "height", a better way would be to get the sum of the tallest character of each column. override function get_height():Float
...
; }
} /** * Gets the position offsets for a character. * @param char The character to get the position offsets for. * @return The position offsets for this char.
...
; default: [0.0, 0.0]
; }
} /** * Gets the animation prefix for the associated character. * @param char The character to get the animation prefix for. * @return The name of the animation prefix for this character.
