...
{ super()
; this.char = Character.create(0, 0, char.id, CharacterType.OTHER)
; for (i in char.sheetsInUse)
{ this.char.addCharAtlas(i.path, i.anims, i.offsetFile)
...
; add(gridBG)
; animationGhost = Character.create(0, 0, char.id)
; for (i in char.sheetsInUse)
{ animationGhost.addCharAtlas(i.path, i.anims, i.offsetFile)
; } animationGhost.alpha = 0.3
; animationGhost.screenCenter()
; animationGhost.dance()
; add(animationGhost)
; char.screenCenter()
; char.dance()
; add(char)
...
{ char.animOffsets[anim] = [0.0, 0.0]
; } var text:FlxText = new FlxText(10, 20 + (18 * daLoop), 0, anim + ": " + char.animOffsets[anim], 15)
...
; _file.save(offsetString, char.id + ".txt")
