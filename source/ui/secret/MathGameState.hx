...
*/ var problem:Int = 0
; /** * The answer the player needs to get this right. * Generated when the problem is generated.
...
; // This state is meant to be purely aliased. FlxSprite.defaultAntialiasing = false
; FlxG.signals.preStateSwitch.addOnce(() ->
{ // Make sure the game runs on antialiasing again after moving from this state. FlxSprite.defaultAntialiasing = true
; // Switch border information back. FlxG.stage.window.title = Main.applicationName
; PlatformUtil.setDarkMode(FlxG.stage.window.title, Preferences.darkMode)
; // Reset cursor. Cursor.reset()
; Cursor.hide()
; } )
; FlxG.stage.window.title = "Baldi's Basics Classic Remastered"
; // Set to light mode. PlatformUtil.setDarkMode(FlxG.stage.window.title, false)
; buildCursor()
; buildYCTP()
; buildMusic()
; playIntro()
...
{ Cursor.load(
{ graphic: Paths.image('backgrounds/math/CursorSprite'), scale: 0.5
} )
; Cursor.show()
...
; var buttons:Array<String> = [ '7', '8', '9', '4', '5', '6', '1', '2', '3', 'clear', '0', 'minus' ]
...
; case 'minus': inputText.text += '-'
; default: inputText.text += buttonId
; }
...
{ queueAudio(
{ sound: Paths.sound('math/intro/BAL_Math_Intro1')
} )
; queueAudio(
{ sound: Paths.sound('math/intro/BAL_Math_Intro2')
} )
; queueAudio(
{ sound: Paths.sound('math/intro/BAL_Math_Intro3')
} )
; queueAudio(
{ sound: Paths.sound('math/yctp/BAL_YCTP_Intro1')
} )
; queueAudio(
{ sound: Paths.sound('math/yctp/BAL_YCTP_Intro2')
} )
...
{ if (queuedSoundList.length > 0) playNextQueueSound()
...
; } if (!failedGame)
{ queueAudio(
{ sound: Paths.sound('math/problem/BAL_YCTP_Problem$problem')
} )
; queueAudio(
{ sound: Paths.sound('math/number/BAL_Math_$
{ num1
} ')
} )
; queueAudio(
{ sound: Paths.sound('math/operations/BAL_Math_$
{ operation
} ')
} )
; queueAudio(
...
; queueAudio(
{ sound: Paths.sound('math/operations/BAL_Math_Equals')
} )
; }
} function clearInputText():Void
{ inputText.text = ''
; } function checkAnswer():Void
{ // Make sure the player can't check their answer if the game has ended. if (problem > 3 || isEndingGame) return
...
{ queueAudio(
{ sound: Paths.soundRandom('math/praise/BAL_Praise', 1, 6)
} )
; } function updateLearnMusic():Void
{ switch (problem)
{ case 1: learnMusic2.volume = 1.0
; case 2: learnMusic3.volume = 1.0
; default:
}
} function handleHangMusic():Void
{ var learnTimeBeat:Float = learnMusicGroup.time % Conductor.instance.crochet
; // Completely stop the learn music from playing after the player gets a problem wrong. learnMusicGroup.pause()
; learnMusicGroup.stop()
; learnMusicGroup.volume = 0.0
; // Play the given hang music based on the beat we're on (so it's synced with the instrument). var hangType:String = (Conductor.instance.curBeat % 2 == 0) ? 'mus_hang_1' : 'mus_hang_2'
; var hangAudio:GameSound = new GameSound(MUSIC).load(Paths.music('math/hang/$
{ hangType
} '))
...
; var graphic:FlxGraphic = Paths.image('backgrounds/math/$
{ correct ? 'Check' : 'X'
} ')
...
{ targetSong: PlayState.lastParams.targetSong, targetVariation: PlayState.lastParams.targetVariation
} ))
; } else
...
; FlxG.switchState(() -> new PlayState(
{ targetSong: roofsSong, targetVariation: PlayState.lastParams.targetVariation
} ))
; }
}
...
{ // TODO: See if it's possible to make the frames change based on the audio. animation.play('talk')
; } else
...
; pressedSprite = Paths.image('backgrounds/math/buttons/btn_$
{ id
} _pressed')
...
{ super(x, y, graphic)
; // Every YCTP sprite in accordance to the game is scaled by 2. scale.set(2, 2)
; updateHitbox()
; scrollFactor.set()
; }
}
