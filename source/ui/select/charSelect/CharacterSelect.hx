...
; /** * The last parameters that were given by the user. * Used as a fallback in-case 0 parameters were given into this state.
*/ static var lastParams:CharacterSelectParams
; /** * The current parameters that were given by the user when exiting this selection. * These parameters will be later used to initalize PlayState.
...
; /** * The data for the none portrait. * * This is used for if a character is lockedIt's not able to be selected.
*/ final LOCKED_CHARACTER:PlayableCharacter = PlayerRegistry.instance.fetchEntry('locked')
; /** * The data for the none portrait. * * This is used for if a slot is unavailable. It's not able to be selected.
*/ final NONE_CHARACTER:PlayableCharacter = PlayerRegistry.instance.fetchEntry('none')
; /** * A list of all of the characters that are selectable, and displayed on this menu. * `Int` => The page number. * `Array<PlayableCharacter>` => All of the characters available to choose on this page.
...
; /** * A map of the all of the selectable characters. * All characters are added into this map so they're easily preloaded, making switching the characters simple. * * `String` => The id of the character. * `Boyfriend` => The character.
*/ var charMap:Map<String, Character> = []
; // CAMERA
// /** * The main in-game camera used in the menu. * The character, and background is rendered onto this sprite.
*/ var cameraWorld:GameCamera = new GameCamera()
; /* * The camera used for any UI elements. * The strumlines, and portrait selections, and other UI elements are rendered onto here.
*/ var cameraHUD:GameCamera = new GameCamera()
; var cameraFollow:FlxObject = new FlxObject(0, 0, 1, 1)
; // MUSIC
// /** * A sound group that handles all of the music for the characters. * Each character song is added to this group to be synced up with each other.
*/ var musicGroup:SoundGroup = new SoundGroup()
; /** * A map of the music for the character. * * `String` => The id of the character. * `GameSound` => The song for the character. Fallbacks to the default theme is none exist for it.
*/ var charThemeMap:Map<String, GameSound> = new Map<String, GameSound>()
; // UI
// /** * The strumline that displays over the character. * This is used to display the note style this character uses, as well as previewing the animations for the strums.
...
; /** * The last page that was selected by the user. * If they first entered the menu this will be null, else this'll be disappeared and stored for use.
...
; function get_selectedPortrait():CharacterPortrait return currentPageSelected.members[currentSelectedIndex]
; /** * Stores the last selected portrait to make sure it de-selects.
*/ var lastSelectedChar:PlayableCharacter
; /** * The current character the user has selected.
*/ var selectedChar(get, never):PlayableCharacter
; function get_selectedChar():PlayableCharacter return currentPageCharacters[currentSelectedIndex]
...
; function get_currentPageCharacters():Array<PlayableCharacter> return characters.get(currentPage)
; // CONTROLS
// /** * A list of `FlxKey` press to make the player sing. * This is normally the first set of the player's keybinds.
*/ var singControls:Array<FlxKey> = []
; /** * A map of `FlxKey' to press for the user to interact with the menu. * This is normally the second set of the player's keybinds.
...
{ super()
; if (params == null && lastParams == null) throw 'Tried initalizing CharacterSelect with 0 parameters.'
; else if (params == null && lastParams != null) params = lastParams
; this.params = params
; lastParams = params
...
{ if (firstPressed == key)
{ switch (control)
{ case LEFT: changeRowSelection(-1)
; case DOWN: changeColumnSelection(1)
; case UP: changeColumnSelection(-1)
; case RIGHT: changeRowSelection(1)
; default:
}
}
...
; }
} #if mobileC if (virtualPad.buttonLeft.justPressed) changeRowSelection(-1)
; if (virtualPad.buttonDown.justPressed) changeColumnSelection(1)
; if (virtualPad.buttonUp.justPressed) changeColumnSelection(-1)
; if (virtualPad.buttonRight.justPressed) changeRowSelection(1)
; #end if (controls.ACCEPT) selectCharacter()
...
; SoundController.play(Paths.sound('scrollMenu'))
; } curRow += amount
; if (curRow < 0) curRow = 2
; if (curRow > 2) curRow = 0
; updateSelection()
; } function changePageSelection(amount:Int = 0)
{ if (amount != 0)
{ lastPageSelected = currentPageSelected
; } currentPage += amount
; if (currentPage < 0) currentPage = portraitPages.size() - 1
; if (currentPage > portraitPages.size() - 1) currentPage = 0
; updatePageDisplay()
; } function updatePageDisplay()
{ if (lastPageSelected != null)
{ lastPageSelected.setVisiblity(false)
; } currentPageSelected.setVisiblity(true)
; pageText.text = LanguageManager.getTextString('charSelect_page') + ' ' + '$
{ currentPage + 1
} /$
{ portraitPages.size()
} '
...
{ // Selected an invalid portrait. if (selectedChar.characterId == 'none' || selectedChar.characterId == 'locked')
...
{ targetSong: params.targetSong, targetVariation: targetVariationId
} , true)
; } )
; } function updateBF()
...
{ case 'locked': hiddenCharacter.visible = true
; case 'none': customCharacterSign.visible = true
; } if (char != null) char.visible = false
; strumLine.visible = false
; } else
{ hiddenCharacter.visible = false
; customCharacterSign.visible = false
; // If the last portrait was an unavailable one, reset the state. if (lastSelectedChar == null || ['locked', 'none'].contains(lastSelectedChar.characterId))
{ if (char != null) char.visible = true
; strumLine.visible = true
; } var oldNoteSkin = char?.skins.get('noteSkin') ?? 'normal'
; if (char != null)
{ // Update the character. remove(char)
...
{ // Create 2 pages, one for the default playable characters, and another to display that you can add more. createPage(0)
; createPage(1)
; // Sort the playable characters by their slot position. var playableChars = [for (entry in PlayerRegistry.instance.listEntryIds()) PlayerRegistry.instance.fetchEntry(entry)]
...
{ // Skip initalize if it's none. it's used to initalize a page. if (player.characterId == 'none') continue
...
; // Make sure there's pages initalized up until this player's page for (i in 0...(page + 1))
...
; characters.set(page, pageCharacters)
; // Preload the character for this player. if (playerToUse.characterId != 'none' && playerToUse.characterId != 'locked')
...
{ isPlayer: false, noteStyle: char?.skins?.get('noteSkin') ?? 'normal'
} )
...
{ if (Paths.music('characterSelect/charSelect-$
{ character.characterId
} ') != null)
{ var theme:GameSound = new GameSound(MUSIC).load(Paths.music('characterSelect/charSelect-$
{ character.characterId
} '), true, true)
...
{ if (normalTheme != sound) sound.volume = 0
; } )
; musicGroup.play()
...
; if (charTheme == sound) sound.fadeIn()
; else sound.fadeOut()
; } )
