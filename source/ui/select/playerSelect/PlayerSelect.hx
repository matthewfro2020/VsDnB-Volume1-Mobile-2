...
{ var OPPONENT = 0
; var PLAYER = 1
; } /** * A selection menu used to be able to select between playing as the opponent, or player of a song. * * This
class represents the base functionaility of the menu. * Each variation should derived, and have the specified functions overwritten.
*/ abstract
...
; /** * A list of all of the character instances in this selection. * `String` => The character id. * `Character` => The character instance.
...
; /** * Initalizes a new player select menu. * @param songId The id of the song for this player.
*/ public function new(songId:String)
{ super()
; this.song = SongRegistry.instance.fetchEntry(songId)
; } public override function create():Void
{ super.create()
; // Stop any existing music. SoundController?.music?.stop()
; // Cache the portrait data into a variable for easy use. players = getAllPortraits()
; // Cache all of the characters to be used in the menu. buildCharacters()
; buildBackground()
; buildSelectLogo()
; buildPortraits()
; buildCharacterText()
...
; if (controls.ACCEPT) selectPlayer()
; }
...
; add(playerText)
; } /** * Changes the portrait selection from a given index amount. * @param amount How much to change the selection by.
...
; if (currentSelectedPortrait > players.length - 1) currentSelectedPortrait = 0
; if (currentSelectedPortrait < 0) currentSelectedPortrait = players.length - 1
; updateSelection()
...
{ targetSong: song, targetVariation: '', playerType: players.indexOf(selectedPlayerData)
} ))
; } )
; } // TO OVERWRITE/ /
/** * Builds the graphic logo that's shown at the right side of the screen that prompts the user. * This can be used in-case you want the logo to be animated, or have other effects.
*/ abstract function buildSelectLogo():Void
; /** * Builds the background used for the selection. * Is able to be overwritten to allow for custom made backgrounds.
*/ abstract function buildBackground():Void
; /** * Gets a list of all of the portaits used for this select. * @return An `Array<CharacterPortraitData`
*/ abstract function getAllPortraits():Array<PlayerData>
; /** * Builds the music used for the player select. * Helpful in-case the select has custom music to be used.
*/ abstract function buildMusic():Void
; /** * Returns the base position that the selected character should be positioned * @return An `FlxPoint` to use.
*/ abstract function getCharacterPosition():FlxPoint
; /** * Returns the point that's used to offset the selected character's position. * @return An `FlxPoint`
