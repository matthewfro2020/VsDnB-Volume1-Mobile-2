...
; portrait.frames = Paths.getSparrowAtlas('selectMenu/ui/portraits/$
{ this.data.portraitFile
} ', 'preload')
...
{ switch (anim)
{ case(_ == this.data.unselectedTransition.name) => true: playAnim(this.data.unselected, true)
; case(_ == this.data.selectedTransition.name) => true: playAnim(this.data.selected, true)
; default:
}
} )
; add(portrait)
