...
; /** * The parameters for the game's default cursor. * Used for in-case the cursor needs to be reset due to it being changed.
*/ public static final DEFAULT_CURSOR_PARAMS:CursorParams =
{ graphic: DEFAULT_CURSOR
} /** * Initalizes the cursor graphic. * Called upon the game opening.
*/ public static function initalize():Void
{ // Reset the cursor graphic to make sure it uses the default. reset()
; // Hide the cursor. hide()
...
// So we check to make sure the mouse is visible based on our property, and set it to that. if (visible != FlxG.mouse.visible) setVisible(visible)
; } /** * Loads a new cursor graphic given parameters. * @param params The parameters to use when loading the cursor.
*/ public static function load(params:CursorParams)
{ if (params.scale == null) params.scale = 1
; if (params.offset == null) params.offset = FlxPoint.get()
...
; // Make SURE the cursor is in the same toggle as before just to make sure. if (!visible)
{ hide()
; } else
{ show()
; }
...
{ show()
; }
} /** * Actually applies the given cursor parameters into the cursor. * @param params The parameters to set the cursor to.
...
{ // Load the given string graphic. bitmap = Paths.image(cast params.graphic).bitmap
