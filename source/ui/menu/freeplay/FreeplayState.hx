...
*/ public static final secretSongs:Map<String, String> = [ 'supernovae' => 'supernovae_hint', 'glitch' => 'glitch_hint', 'master' => 'master_hint', 'kabunga' => 'kabunga_hint', 'roofs' => 'roofs_hint', 'vs-dave-rap-two' => 'rapTwo_hint' ]
...
; /** * A list of all of the songs the user has unlocked IN the category. * Gets populated when the category is open. If any exist, all are unlocked in quick sequence.
...
; // CATEGORIES
// /** * The current pack that is selected by the user. * Static variable so the current selection is saved when the user goes back to the menu.
...
*/ var lerpScore:Int = 0
; /** * The actual score of the song. * The score text animates to this via `lerpScore`
*/ var intendedScore:Int = 0
; /** * Whether the character text should be shown. * Only true AFTER the user has entered the character select at least once.
...
; /** * The hint text that tells the user about skipping the character select. * Only displays AFTER the user has seen the character select at least once.
*/ var characterSelectText:FlxText
; // HINT TEXTS
// /** * Is the user able to see the hint texts shown?
*/ var canShowHints:Bool = false
; /** * Whether the game's currently showing a hint text. * Used for transitions.
...
; FlxG.save.flush()
; }
} /** * Queues a song to being unlocked. * @param song The song to unlock.
...
; } #if desktop DiscordClient.changePresence("In the Freeplay Menu", null)
; #end showCharText = FlxG.save.data.wasInCharSelect
...
; add(grpSongs)
; add(grpIcons)
; #if mobileC addVirtualPad(LEFT_FULL, A_B)
; #end super.create()
; } override function update(elapsed:Float)
{ super.update(elapsed)
; Conductor.instance.update(Conductor.instance.songPosition + FlxG.elapsed * 1000)
; Conductor.instance.quickWatch()
; // Selector Menu Functions if (!InMainFreeplayState)
{ if (canInteract && (controls.LEFT_P || FlxG.mouse.wheel < 0)) UpdatePackSelection(-1)
; if (canInteract && (controls.RIGHT_P || FlxG.mouse.wheel > 0)) UpdatePackSelection(1)
...
; } return
; } else
{ // Freeplay Functions var upP = controls.UP_P || FlxG.mouse.wheel > 0
...
; FlxTween.tween(lockHintGroup,
{ y: FlxG.height
} , 0.5,
{ ease: FlxEase.expoOut, onComplete: function(t:FlxTween)
...
; } if (scoreBG != null)
{ FlxTween.tween(scoreBG,
{ y: scoreBG.y - 100
} , 0.3,
{ ease: FlxEase.expoInOut, onComplete: function(spr:FlxTween)
{ scoreBG = null
; }
} )
; } if (scoreText != null)
{ FlxTween.tween(scoreText,
{ y: scoreText.y - 100
} , 0.3,
{ ease: FlxEase.expoInOut, onComplete: function(spr:FlxTween)
{ scoreText = null
...
{ FlxTween.tween(characterSelectText,
{ alpha: 0
} , 0.3,
{ ease: FlxEase.expoInOut, onComplete: function(spr:FlxTween)
...
{ remove(i)
; } if (Preferences.flashingLights) FlxTween.color(bg, 0.25, bg.color, defColor)
; // MAKE SURE TO RESET EVERYTHIN! songs = []
; curSelected = 0
; canInteract = true
; grpSongs.members = []
; grpSongs.clear()
...
{ case 'backseat': FlxG.switchState(() -> new ui.select.playerSelect.BackseatSelect())
; default: SoundController.music.fadeOut(1, 0)
...
{ targetSong: song, targetVariation: ''
} ))
; } else
...
; if (scoreText != null) scoreText.text = LanguageManager.getTextString('freeplay_score') + ': $lerpScore'
; positionHighscore()
; } function onUpdate(elapsed:Float)
{ for (song in grpSongs)
{ if (song.onUpdate != null) song.onUpdate(elapsed)
; }
} override function stepHit(step:Int)
{ if (!super.stepHit(step)) return false
; for (song in grpSongs.members)
{ if (song.onStepHit != null) song.onStepHit()
; } return true
; } override function beatHit(beat:Int)
{ if (!super.beatHit(beat)) return false
; for (song in grpSongs.members)
{ if (song.onBeatHit != null) song.onBeatHit()
; } return true
; } override function measureHit(measure:Int)
{ if (!super.measureHit(measure)) return false
; for (song in grpSongs.members)
{ if (song.onMeasureHit != null) song.onMeasureHit()
; } return true
...
; index++
; } )
; } case 'escape-from-california': songText.text = "Escape From\nCalifornia"
...
; FlxTween.tween(characterSelectText,
{ x: characterSelectText.x - 150, alpha: 1
} , 0.5,
{ ease: FlxEase.expoInOut
} )
; } add(scoreText)
; // Add Hint Text lockHintGroup.scrollFactor.set()
; add(lockHintGroup)
...
{ y: 0, alpha: i == curSelected ? 1 : 0.6
} , 0.5,
{ ease: FlxEase.expoInOut, onComplete: function(twn:FlxTween)
{ grpSongs.members[i].unlockY = false
; }
} )
; } new FlxTimer().start(0.5, function(t:FlxTimer)
{ updateSongPositions()
; // There's songs to be unlocked. var canUnlockSongs:Bool = unlockableSongs.length > 0
...
{ #if !switch intendedScore = Highscore.getScore(songs[curSelected].song.id)
; #end
} var bullShit:Int = 0
...
; if (Preferences.flashingLights) FlxTween.color(bg, 0.25, bg.color, songs[curSelected].color)
; if (canShowHints)
...
; FlxTween.cancelTweensOf(lockHintGroup)
; FlxTween.tween(lockHintGroup,
{ y: FlxG.height - lockHintGroup.height
} , 0.5,
...
{ CurrentPack += change
; if (CurrentPack == -1) CurrentPack = categories.length - 1
; if (CurrentPack == categories.length) CurrentPack = 0
; currentCategory = categories[CurrentPack]
...
{ if (scoreText != null) scoreText.x = FlxG.width - scoreText.width - 6
; if (scoreBG != null)
...
{ unlockableSongs = []
; for (song in songs)
{ // The song in THIS category exists in the unlocked songs list, push into the array. if (songsUnlocked.contains(song.song.id.toLowerCase()))
{ unlockableSongs.push(song.song.id.toLowerCase())
; songsUnlocked.remove(song.song.id.toLowerCase())
; }
}
} /** * Gets the index for a song from of its category. * @param song The lower-case version of the song to check. * @return The category index of the song.
*/ function getSongIndex(song:String):Int
{ for (i in 0...songs.length)
{ if (songs[i].song.id.toLowerCase() == song) return i
; } return -1
...
; } else
{ // No more songs need to be unlocked. waitingToInteract = true
; }
} /** * Plays sequence that happens when a song is about to be unlocked. * @param song The song to play the sequence for, and unlock.
*/ function playUnlockSequence(song:String)
{ var index:Int = getSongIndex(song)
; // Song either doesn't exist, or isn't in this category. if (index == -1) return
; var selectChange:Int = index - curSelected
; // Change the selection to the current unlocking song. changeSelection(selectChange)
...
; FlxTween.tween(songIcon.scale,
{ x: 1.1, y: 1.1
} , 2,
{ onComplete: (t:FlxTween) ->
{ SoundController.play(Paths.sound('freeplay/unlock_lockBreak'), 0.8)
; // Change the icon to the unlock icon. songIcon.frames = Paths.getSparrowAtlas('freeplay/freeplay_unlock')
; songIcon.animation.addByPrefix('unlock', 'freeplay_unlock', 24, false)
; songIcon.animation.play('unlock', true)
; // Reset the scale. songIcon.scale.set(1, 1)
; // Quick un-lock the next song in the list.
// If there's no song that needs to be unlocked, it'll automatically resume back the controls. new FlxTimer().start(0.5, (t:FlxTimer) ->
{ unlockNextAvailableSong()
; } )
; new FlxTimer().start(1, (t:FlxTimer) ->
{ // Song is unlocked now. Change the unlock state. @:privateAccess songMetadata.locked = false
; FlxG.save.data.locked.set(songMetadata.song.id.toLowerCase(), 'unlocked')
; FlxG.save.flush()
; // Do a quick succession with the text showing. var listLength:Array<Int> = [for (i in 0...songMetadata.song.songName.length) i]
...
; t.reset(0.05)
; } else
{ // Change the visuals to show it's unlocked. songIcon.char = songMetadata.songCharacter
...
; add(confetti)
; // Unlock controls now that all unlock animations have played. if (waitingToInteract)
{ canInteract = true
; canShowHints = true
; }
}
...
; this.color = color
; locked = (FlxG.save.data.locked.exists(this.song.id.toLowerCase()) && FlxG.save.data.locked.get(this.song.id.toLowerCase()) != 'unlocked') && !FreeplayState.unlockAll
; }
