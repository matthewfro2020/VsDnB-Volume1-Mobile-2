...
; /** * Is the selection bar currently closed? * If so, skip the selection bar part of the manual.
...
; if (delayTimer > 0) delayTimer -= elapsed
...
; // Reposition the turntable tutorial since the turntable's centered now. turnTableManual.x = value ? 0 : -150
; } function advanceManual():Void
{ uiManual.visible = false
; selectionBarManual.visible = false
; turnTableManual.visible = false
; // Skip the selection bar tutorial because it's closed. if (selectionBarClosed && state == UI) state = SELECTION_BAR
; switch (state)
{ case START: canAdvance = true
; uiManual.visible = true
; state = ManualState.UI
; case UI: selectionBarManual.visible = true
; state = ManualState.SELECTION_BAR
; case SELECTION_BAR: turnTableManual.visible = true
; state = ManualState.TURNTABLE
; case TURNTABLE: canAdvance = false
...
{ // Reposition the turntable tutorial if the turntable's centered. turnTableManual.x = !selectionBarClosed ? 0 : -150
; add(turnTableManual)
