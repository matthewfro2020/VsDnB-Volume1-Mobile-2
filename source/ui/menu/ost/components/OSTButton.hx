...
; if (toggleIdle != null) Animation.addToSprite(this, toggleIdle)
; if (togglePressed != null) Animation.addToSprite(this, togglePressed)
...
{ isPressed = false
; switch (pressType)
{ case TOGGLE: this.selected = !selected
; playIdle()
; if (onTogglePress != null) onTogglePress(this.selected)
; case SINGLE: playIdle()
; if (onPress != null) onPress()
; }
}
} function playPress(force:Bool = false):Void
{ switch (pressType)
{ case TOGGLE: this.animation.play(this.selected ? togglePressed.name : pressedAnimation.name, true)
; case SINGLE: this.animation.play(pressedAnimation.name, true)
; } if (force) this.animation.finish()
; } function playIdle(force:Bool = false):Void
{ switch (pressType)
{ case TOGGLE: this.animation.play(this.selected ? toggleIdle.name : idleAnimation.name, true)
; case SINGLE: this.animation.play(idleAnimation.name, true)
; } if (force) this.animation.finish()
; }
}
