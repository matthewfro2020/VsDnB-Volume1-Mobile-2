...
; /** * The current playing chart that's used to hold the song's data. * This is dependent on the song's selected variation, and may change the instrumental and vocals, etc.
...
; vinyl.x = (table.x - this.x) + (table.width - vinyl.width) / 2
; // Center the vinyl with the table. vinyl.y = 22
; add(vinyl)
; constructButtons()
; buildTextMetadata()
; buildAdditionalData()
; grainLoop = new GameSound().load(Paths.sound('ost/grain'), true)
; // Add the instrumental and vocals track into the sound group so they're properly updated with every other sound. SoundController.add(instrumentalTrack)
...
; switch (timeDisplay)
{ case 'timeElapsed': var currentTime:String = FlxStringUtil.formatTime(instrumentalTrack.time / 1000)
; var currentLength:String = FlxStringUtil.formatTime(instrumentalTrack.length / 1000)
; setTimeText(currentTime)
; #if desktop DiscordClient.changePresence('In the OST Menu', 'Listening to $
{ currentPlayData.name
} ($currentTime / $currentLength)')
; #end case 'timeLeft': var timeLeft:String = FlxStringUtil.formatTime((instrumentalTrack.length - instrumentalTrack.time) / 1000)
...
{ id: 'play', idle:
{ name: 'idle', prefix: 'buttonplay_norm'
} , pressed:
{ name: 'pressed', prefix: 'buttonplay_click'
} , pressType: SINGLE
} )
; playButton.onPress = () ->
{ playSong()
...
{ id: 'pause', idle:
{ name: 'idle', prefix: 'buttonpause_norm'
} , pressed:
{ name: 'pressed', prefix: 'buttonpause_click'
} , pressType: SINGLE,
} )
...
{ id: 'speed', idle:
{ name: 'idle', prefix: 'buttonspeed_norm'
} , pressed:
{ name: 'pressed', prefix: 'buttonspeed_click'
} , pressType: SINGLE,
} )
; speedButton.onPress = () ->
{ switch (remixType)
{ case SPEED: increaseSpeed(0.05)
; case BPM: increaseBPM(1)
; }
} add(speedButton)
; slowButton = new OSTButton(90, 251,
{ id: 'slow', idle:
{ name: 'idle', prefix: 'buttonslow_norm'
} , pressed:
{ name: 'pressed', prefix: 'buttonslow_click'
} , pressType: SINGLE,
} )
; slowButton.onPress = () ->
{ switch (remixType)
{ case SPEED: decreaseSpeed(0.05)
; case BPM: decreaseBPM(1)
; }
} add(slowButton)
; vocalsButton = new OSTButton(689, 53,
{ id: 'vocals', idle:
{ name: 'idle_toggle', prefix: 'buttonvocalno_norm'
} , pressed:
{ name: 'pressed_toggle', prefix: 'buttonvocalno_click'
} , toggleIdle:
{ name: 'idle', prefix: 'buttonvocal_norm'
} , togglePressed:
{ name: 'pressed', prefix: 'buttonvocal_click'
} , pressType: TOGGLE, startingSelect: true, forceSelect: true,
} )
; vocalsButton.onTogglePress = (v:Bool) ->
{ vocalsTrack.volume = v ? 1.0 : 0.0
; } add(vocalsButton)
; instrumentalButton = new OSTButton(689, 121,
{ id: 'instrumental', idle:
{ name: 'idle_toggle', prefix: 'buttoninstno_norm'
} , pressed:
{ name: 'pressed_toggle', prefix: 'buttoninstno_click'
} , toggleIdle:
{ name: 'idle', prefix: 'buttoninst_norm'
} , togglePressed:
{ name: 'pressed', prefix: 'buttoninst_click'
} , pressType: TOGGLE, startingSelect: true, forceSelect: true,
} )
...
{ id: 'remix', idle:
{ name: 'idle', prefix: 'buttonremix_norm'
} , pressed:
{ name: 'pressed', prefix: 'buttonremix_click'
} , pressType: SINGLE,
} )
; remixButton.onPress = () ->
{ switch (remixType)
{ case SPEED: remixType = BPM
; setBPM(Conductor.instance.currentTimeChange.bpm + bpmAddend)
; setRemixText(LanguageManager.getTextString('ost_remixMode_bpm'))
; case BPM: remixType = SPEED
; setSpeed(speed)
...
{ id: 'manual', idle:
{ name: 'idle', prefix: 'buttonmanual_norm'
} , pressed:
{ name: 'pressed', prefix: 'buttonmanual_click'
} , pressType: SINGLE,
} )
...
{ barCount: 10, width: 125, height: 60, spacing: 3, minFrequency: 50, maxFrequency: 30000, peakLines: false, color: FlxColor.WHITE,
} )
...
{ for (button in [playButton, pauseButton, speedButton, slowButton, vocalsButton, manualButton, remixButton]) func(button)
; } /** * Load the turntable given an `OSTPlayData` * @param playData The play data to load.
*/ public function loadSong(playData:OSTPlayData)
{ // Unload the current audio to save memory. if (currentPlayData != null)
{ Preloader.removeCachedSound(currentPlayData.instrumental)
; if (currentPlayData.vocals != null) Preloader.removeCachedSound(currentPlayData.vocals)
; } this.currentPlayData = playData
; // Cache the song so it loads easier.
// This'll also store the song into the cache to make sure it gets clear from memory. Preloader.cacheSound(this.currentPlayData.instrumental)
; if (currentPlayData.vocals != null) Preloader.cacheSound(this.currentPlayData.vocals)
; var vinylPath:String = currentPlayData.vinylPath ?? 'vinyl_extra'
; vinyl.loadGraphic(Paths.image('ost/vinyls/$vinylPath'))
; // Load the chart's audio tracks. instrumentalTrack.load(this.currentPlayData.instrumental)
; if (currentPlayData.vocals != null) vocalsTrack.load(this.currentPlayData.vocals)
; else vocalsTrack = new GameSound()
...
; // Set the conductor to the beginning so it's at the right time change. Conductor.instance.update(0)
; // BPM was changed, make sure it's reset back. if (remixType == BPM)
{ setBPM(Conductor.instance.currentTimeChange.bpm + bpmAddend)
; } else
{ // Else, reset the speed. setSpeed(this.speed)
; } // Update the track information based on the current song. setSongNameText(currentPlayData.name)
...
{ // Cancel the transition if it's already happening, or no song is playing. if (isSongPlaying || speedTween != null || currentPlayData == null) return
; isSongPlaying = true
...
{ // Cancel the transition if it's already happening, or no song is playing. if (!isSongPlaying || speedTween != null) return
; FlxG.autoPause = true
...
; spectrogram.visible = false
; spectrogram.stop()
; #if desktop DiscordClient.changePresence('In the OST Menu', null)
; #end
} /** * Sets the speed/pitch that the turntable's playing the song at. * @param speed The new speed.
...
; updateBPMText()
; } /** * Add the speed of the turntable by the given amount. * @param amount The amount to increase by.
*/ function increaseSpeed(amount:Float):Void
{ setSpeed(Math.min(10, speed + amount))
; } /** * Subtract the speed of the turntable by the given amount. * @param amount The amount to decrease by.
*/ function decreaseSpeed(amount:Float):Void
{ setSpeed(Math.max(0, speed - amount))
; } /** * Manually set the pitch of the current song playing to the given value. * @param pitch The new pitch value.
*/ function setSongPitch(pitch:Float)
{ instrumentalTrack.pitch = Math.max(0, pitch)
; vocalsTrack.pitch = Math.max(0, pitch)
; } /** * Sets the BPM of the turntable * @param bpm The new bpm.
*/ function setBPM(bpm:Float):Void
{ // Clamp BPM so it doesn't go below 1. bpm = Math.max(1, bpm)
...
{ switch (timeDisplay)
{ case 'timeElapsed': setTimeText(FlxStringUtil.formatTime(instrumentalTrack.time / 1000))
; case 'timeLeft': setTimeText(FlxStringUtil.formatTime((instrumentalTrack.length - instrumentalTrack.time) / 1000))
; }
...
{ switch (remixType)
{ case SPEED: var speedText:String = validateFloatSpeedText(FlxMath.roundDecimal(speed, 2))
; setSpeedText('$
{ speedText
} x')
; case BPM: var bpmSpeedText:String = validateFloatSpeedText(FlxMath.roundDecimal(bpmSpeed, 2))
; setSpeedText('$
{ bpmSpeedText
} x')
; }
...
; if (decimals == 0) text += '.'
; for (i in 0...2 - decimals) text += '0'
; return text
; } function updateBPMText():Void
{ if (currentPlayData != null)
{ switch (remixType)
{ case SPEED: setBPMText('BPM: $
{ FlxMath.roundDecimal(Conductor.instance.currentTimeChange.bpm * speed, 2)
} ')
; case BPM: setBPMText('BPM: $
{ FlxMath.roundDecimal(Conductor.instance.currentTimeChange.bpm + bpmAddend, 2)
} ')
; }
}
} function trimTextToWidth(text:FlxText, width:Float):Void
{ var wasTrimmed:Bool = false
; // The - 40 is to account for adding `...` to the end of the text. while (text.textField.textWidth > width - 40)
{ wasTrimmed = true
