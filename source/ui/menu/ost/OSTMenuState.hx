...
*/ var bg:FlxSprite
; /** * The original menu BG graphic. * Used to apply gradients to the background.
...
; /** * A list of all the selectable variations for this song. * Populated on song selected. Dependent on the currently selected song.
...
; // Build background.
// This starts as a gray background, but when a song is selected it changes to a gradient. menuBgGraphic = FileUtil.randomizeBG().bitmap
...
; add(bg)
; // Generate the turntable, this is what'll be used to control the playing song. turnTable = new Turntable(this)
...
; #end #if mobileC addVirtualPad(LEFT_FULL, A_B)
; #end
...
{ case CATEGORY: if (FlxG.keys.justPressed.UP #if mobileC || virtualPad.buttonUp.justPressed #end || FlxG.mouse.wheel > 0)
...
; if (FlxG.keys.justPressed.LEFT #if mobileC || virtualPad.buttonLeft.justPressed #end) changeCategorySelection(-1)
; } else
...
; if (FlxG.keys.justPressed.RIGHT #if mobileC || virtualPad.buttonRight.justPressed #end) changeCategorySelection(1)
; } else
{ arrowRight.scale.set(1, 1)
; } case SONG: if (FlxG.keys.justPressed.UP #if mobileC || virtualPad.buttonUp.justPressed #end)
...
; if (FlxG.keys.justPressed.LEFT #if mobileC || virtualPad.buttonLeft.justPressed #end) changeVariationSelection(-1)
; } else
...
; if (FlxG.keys.justPressed.RIGHT #if mobileC || virtualPad.buttonRight.justPressed #end) changeVariationSelection(1)
; } else
...
{ switch (anim)
{ case 'transition_select': playPlayrobotAnimation('idle_select')
; playrobotButton.animation.play('idle_select', true)
; case 'transition_unselect': playPlayrobotAnimation('idle_unselect')
; }
} )
...
; FlxG.save.flush()
; }
} /** * Generates a list of songs that are able to be selected from based on the given category. * @param category The category to generate a list from.
...
{ if (FlxG.save.data.locked.exists(catSong.id) && FlxG.save.data.locked.get(catSong.id) == 'locked') continue
...
; bg.color = FlxColor.GRAY
; } /** * Toggles the selection bar to play an animation of it either appearing, or disappearing disabling it. * Useful for if you want * @param appear Whether the selection bar should be visible, or not.
...
; FlxTween.tween(selectionBarGroup,
{ x: 0.0
} , 0.5,
{ ease: FlxEase.quadIn
} )
; FlxTween.tween(turnTable,
{ x: turnTable.x + 150
} , 0.5,
{ ease: FlxEase.quadIn, onComplete: (t:FlxTween) ->
...
; } /** * Change the currently selected category based on the given amount. * @param change How much to change from the current select category.
...
; if (currentCategorySelected > categoryList.length - 1) currentCategorySelected = 0
; if (currentCategorySelected < 0) currentCategorySelected = categoryList.length - 1
...
; } /** * Changes the currently selected song index based on the given amount. * @param amount The amount to change from the current selected index.
*/ function changeSongSelection(amount:Int = 0)
{ if (amount != 0)
{ SoundController.play(Paths.sound('scrollMenu'), 0.7)
; } if (selectedSongOption != null) selectedSongOption.unselectOption()
; currentSongSelected += amount
; if (currentSongSelected > categorySongsCount) currentSongSelected = 0
; if (currentSongSelected < 0) currentSongSelected = categorySongsCount
; updateSelectedSong()
; fadeOutSongs(currentSongSelected)
; } /** * Changes the variation from the given amount. * @param amount
*/ function changeVariationSelection(amount:Int = 0)
{ if (amount != 0)
{ SoundController.play(Paths.sound('scrollMenu'), 0.7)
; } selectedVariationIndex += amount
; if (selectedVariationIndex > currentSongVariationsList.length - 1) selectedVariationIndex = 0
; if (selectedVariationIndex < 0) selectedVariationIndex = currentSongVariationsList.length - 1
...
; // Update the selected variation index to make sure it matches with this song's current variation. selectedVariationIndex = currentSongVariationsList.indexOf(selectedSongOption.currentVariation)
; changeVariationSelection()
; setupSelectTween(currentSongSelected)
; } /** * Resets every song's state to be unselected. * Called when the user switches to being on a category.
*/ function unselectOptions():Void
{ for (song in grpSongsList.members)
{ song.unselectOption()
; }
} /** * Slowly fades out any songs under the given index. * @param index The index to start fading out songs to.
...
; } targetIndex++
; }
} /** * Creates, and starts the tween used to move the song selection bar. * @param index The selection index.
*/ function setupSelectTween(index:Int):Void
{ selectSongTween?.cancel()
; selectSongTween?.destroy()
; selectSongTween = null
; selectSongTween = FlxTween.tween(grpSongSelection,
{ y: -75 * index
} , 0.5,
{ ease: FlxEase.expoOut, onComplete: (t:FlxTween) ->
{ selectSongTween.destroy()
; selectSongTween = null
; }
} )
; } /** * Toggles whether you're able to select a category, or not. * @param appear The state to change to.
...
{ manual.visible = true
; manual.startManual()
; } /** * Plays an animation from playrobot while also accounting for animation offsets. * @param anim The animation to play.
*/ function playPlayrobotAnimation(anim:String):Void
{ playrobotButton.updateHitbox()
; playrobotButton.animation.play(anim, true)
; switch (anim)
{ case 'transition_unselect': playrobotButton.offset.y -= 4
; case 'idle_unselect': playrobotButton.offset.y -= 10
; case 'transition_select': playrobotButton.offset.y += 2.5
; }
}
...
{ // Change the icon. if (icon != null) icon.char = playData.icon
; else icon = new HealthIcon(playData.icon)
; icon.autoOffset = false
; icon.setGraphicSize(60)
; icon.updateHitbox()
; add(icon)
; } else
{ // Remove the icon since the play data doesn't have a icon to display. if (icon != null)
{ this.remove(icon)
; icon.destroy()
; icon = null
; }
} // Trim each text so it cuts off before the end of the width. scaleTextToWidth(songNameText, maxWidth - (icon != null ? (icon.width + 5) : 0) - songComposerText.x)
; scaleTextToWidth(songComposerText, maxWidth)
; // Reposition the icon to be at the end of the name text. if (icon != null)
...
; if (icon != null) icon.alpha = alpha
; } public function scaleTextToWidth(text:FlxText, width:Float):Void
{ // The - 25 is to account for if this option has additional variations. if (playData.size() > 1) width -= 25
; text.scale.set(1, 1)
