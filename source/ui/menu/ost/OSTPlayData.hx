...
; } /** * Builds a list of OSTPlayData information from `CategorySong` * @param categorySong The category song to build off of. * @return A `Map<String, OSTPlayData>` that maps each `OSTPlayData` by variation.
...
; // This category song is an external song (meaning it's not playable), load it from a music data file. if (categorySong.external && SongRegistry.instance.hasMusicDataFile(categorySong.id))
{ // Retrieve the data for the default variation. var musicData:SongMusicData = SongRegistry.instance.loadMusicDataFile(categorySong.id)
...
; // Load music data for any other variations. for (variation in musicData.variations)
...
; } /** * Builds an `OSTPlayData` from a given `SongMusicData`. * * @param musicData The music data to build from. * @param colors The colors to be used for the play data. * @param icon The icon that should be used. * @return A `OSTPlayData`
...
; return playData
; } /** * Builds an `OSTPlayData` from a given `SongPlayChart`. * * @param chart The play chart to build from. * @param colors The colors to be used for the play data. * @param icon The icon that should be used. * @return A `OSTPlayData`
...
; playData.vinylPath = vinyl
; if (Assets.exists(chart.getVoicesPath())) playData.vocals = chart.getVoicesPath()
; playData.colors = colors
; return playData
; }
}
