...
; /** * Whether the movement for the icons have finished. * Stored as a static so it doesn't have several times.
...
; #if desktop DiscordClient.changePresence("In the Menus", null)
; #end bg = new FlxSprite(-80).loadGraphic(FileUtil.randomizeBG())
; bg.scrollFactor.set()
...
; if (firstStart)
{ FlxTween.tween(item,
{ x: ((FlxG.width - item.width) / 2) + ((i - Math.floor(menuItems.length / 2)) * 160)
} , 1 + (i * 0.25),
...
; bigIcons.loadGraphic(Paths.image('mainMenu/icons/$
{ optionShit[curSelected]
} '))
; bigIcons.setGraphicSize(0, 350)
; bigIcons.updateHitbox()
; bigIcons.screenCenter(X)
; // Apply offsets so the characters are centered to each other. switch (optionShit[curSelected])
{ case 'credits': bigIcons.x += 50
; case 'options': bigIcons.x -= 50
; } var optionString:String = LanguageManager.getTextString(languagesOptions[curSelected])
; curOptText.text = optionString.format(' ')
; curOptDesc.text = LanguageManager.getTextString(languagesDescriptions[curSelected])
; if (amount != 0) SoundController.play(Paths.sound('scrollMenu'), 0.7)
...
; switch (i)
{ case(_ == index) => true: FlxTween.tween(spr,
{ 'scale.x': 0.7, 'scale.y': 0.7
} , 0.5,
{ ease: FlxEase.circOut
} )
; default: FlxTween.tween(spr,
{ 'scale.x': 0.6, 'scale.y': 0.6, alpha: 0.8
} , 0.3 + (i * 0.1),
{ ease: FlxEase.circOut
} )
; } new FlxTimer().start(1, function(t:FlxTimer)
...
; case 'freeplay': if (FlxG.random.bool(0.05))
{ PlatformUtil.openURL("https:
// www.youtube.com/watch?v=Z7wWa1G9_30%22")
; } FlxG.switchState(() -> new FreeplayState())
; case 'ost': FlxG.switchState(() -> new OSTMenuState())
; case 'options': var settings = new SettingsMenu()
