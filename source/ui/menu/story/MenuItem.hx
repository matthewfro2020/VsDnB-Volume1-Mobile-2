...
; if (fakeFramerate == 0) return
; if (flashingInt % fakeFramerate >= Math.floor(fakeFramerate / (!Preferences.flashingLights ? 0.5 : 2)))
{ week.color = 0xFF33ffff
; } else
{ week.color = FlxColor.WHITE
; }
} /** * Changes the x position of the menu item. * @param newTarget The new x position.
...
; currentTween = null
; } currentTween = FlxTween.tween(this,
{ x: (targetX * 450) + 420
} , easeTime,
{ type: ONESHOT, ease: easeType, onComplete: clearTween
} )
; } /** * Clears out the item's tween. Called when the tween's been completed. * @param t The tween.
*/ function clearTween(t:FlxTween)
{ currentTween = null
; } /** * Starts flashing the menu item. * This usually means it was selected.
