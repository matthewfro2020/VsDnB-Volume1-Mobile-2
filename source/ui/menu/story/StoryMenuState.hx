...
// WARMUP new Week(['warmup'], LanguageManager.getTextString('story_tutorial'), [0xFF8A42B7], 'warmup',
{ assetPath: 'daverender_warmup', animation:
{ name: 'render', prefix: 'warmup', loop: true, offsets: [0, 86]
}
} ),
// DAVE new Week(['house', 'insanity', 'polygonized'], LanguageManager.getTextString('story_daveWeek'), [0xFF4965FF], 'DaveHouse',
{ assetPath: 'daverender', animation:
{ name: 'render', prefix: 'davestuff', loop: true, offsets: [0, 95]
}
} ),
// MISTER BAMBI bro new Week(['blocked', 'corn-theft', 'maze'], LanguageManager.getTextString('story_bambiWeek'), [0xFF00B515], 'bamboi',
{ assetPath: 'bambirender', animation:
{ name: 'render', prefix: 'bambiweek', loop: true, offsets: [0, 84]
}
} ),
// SPLIT THE THONNNNN new Week(['splitathon'], LanguageManager.getTextString('story_finale'), [0xFF4965FF, 0xFF00B515], 'splitathon',
{ assetPath: 'daverender-splitathon', animation:
{ name: 'render', prefix: 'davesplitathon', loop: true, offsets: [0, 90]
}
} ),
// FESTEVAL new Week(['warmup', 'house', 'insanity'], LanguageManager.getTextString('story_festivalWeek'), [0xFF800080], 'festival',
{ assetPath: 'festivalrender', animation:
{ name: 'render', prefix: 'festival', loop: true, offsets: [0, 50]
}
} ), ]
...
; // MASTERA BAMBI var jokeWeek = new Week(['supernovae', 'glitch', 'master'], weekName, [0xFF116E1C], FlxG.save.data.hasPlayedMasterWeek ? 'masterweek' : 'masterweekquestion',
{ assetPath: 'jokerender', animation:
{ name: 'render', prefix: 'joke', loop: true, offsets: [0, 73.8]
}
} )
; weeks.push(jokeWeek)
; } #if desktop DiscordClient.changePresence("In the Story Menu", null)
; #end transIn = FlxTransitionableState.defaultTransIn
; transOut = FlxTransitionableState.defaultTransOut
; if (SoundController.music != null)
{ if (!SoundController.music.playing) SoundController.playMusic(Paths.music('freakyMenu'))
...
; var weekBanner:FlxSprite = new FlxSprite(bannerX, 56).loadGraphic(Paths.image('storyMenu/weekBanners/$
{ weeks[i].bannerName
} '))
...
; switch (i)
{ case 3: var hasBeatSplitathon:Bool = (FlxG.save.data.splitathonBeat != null && FlxG.save.data.splitathonBeat)
; if (hasBeatSplitathon)
{ weeks[3].render.assetPath = 'splitathonrender'
; weeks[3].render.animation =
{ name: 'render', prefix: 'splitathon', loop: true, offsets: [0, 91]
}
}
} var weekRender:FlxSprite = new FlxSprite(bannerX, 56)
; weekRender.frames = Paths.getSparrowAtlas('storyMenu/renders/$
{ weeks[i].render.assetPath
} ')
...
; // Make it so the master week only shows after playing the week. if (i == 5)
...
; } else
{ changeWeek(0)
; } #if mobileC addVirtualPad(LEFT_RIGHT, A_B)
; #end super.create()
...
; var params =
{ targetSong: storySong, targetVariation: ''
} new FlxTimer().start(1, function(tmr:FlxTimer)
{ switch (PlayStatePlaylist.storyWeek)
{ case 1: SoundController.music.stop()
; LoadingState.loadPlayState(params, true)
; case 5: if (!FlxG.save.data.hasPlayedMasterWeek)
{ FlxG.save.data.hasPlayedMasterWeek = true
; FlxG.save.flush()
; } LoadingState.loadPlayState(params, true)
; default: LoadingState.loadPlayState(params, true)
; }
} )
; } function changeWeek(change:Int = 0):Void
{ curWeek += change
; if (curWeek > weeks.length - 1) curWeek = 0
...
; titleTween?.destroy()
; titleTween = null
; } titleTween = FlxTween.tween(weekBanners,
{ x: -FlxG.width * curWeek
} , MenuItem.easeTime,
{ type: ONESHOT, ease: MenuItem.easeType, onComplete: (t:FlxTween) ->
...
; FlxTween.tween(yellowBG,
{ alpha: 1
} , 0.25)
; FlxTween.tween(gradientBg,
{ alpha: 0
} , 0.25)
; } else
...
; switch (curWeek)
{ case 4: stringThing = ['???', '???', '???', '???']
; case 5: if (!FlxG.save.data.hasPlayedMasterWeek) stringThing = ['???', '???', '???']
; } txtTracklist.text = stringThing.join(' - ')
; #if !switch intendedScore = Highscore.getWeekScore(curWeek)
; if (currentScoreTween != null)
{ currentScoreTween.destroy()
; } currentScoreTween = FlxTween.num(easeScore, intendedScore, 0.35,
{ type: FlxTweenType.ONESHOT, ease: FlxEase.quadOut
} , updateEaseScore)
; #end
