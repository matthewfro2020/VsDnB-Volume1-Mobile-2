...
// Originally was FlxSubstate, moved to MusicBeatSubstate for mobile controls.
{ static var curCategorySelection:Int = 0
; /** * Map of the classes that represent a category. * TODO: Make this more modular instead of several arrays?
*/ final categoryMap:Map<String, Class<SettingsCategory>> = [ 'general' => Options_General, 'accessibility' => Options_Accessibility, 'window' => Options_Window, 'audio' => Options_Audio, 'ui' => Options_UI, 'misc' => Options_Misc, ]
...
; #if mobileC addVirtualPad(LEFT_FULL, A_B_C)
; addVirtualPadCamera()
; #end
...
; // Flip the right arrow so it faces to the right. arrowRight.flipX = true
...
; #if mobileC var mobileControls = virtualPad.buttonC.justPressed
; #end switch (curState)
{ case SelectingCategory: if (arrowLeft != null && canInteract)
{ if (left)
...
; curState = SelectingOption
; } case SelectingOption: if ((upP && curCategory.curOptionSelected == curCategory.firstAvailableOption) || (downP && curCategory.curOptionSelected == curCategory.lastAvailableOption))
{ curState = SelectingCategory
; curCategory.deselectOption()
; } else
{ curCategory.handleInputs()
; }
} #if mobileC if (mobileControls)
...
; FlxTween.tween(clipboard,
{ y: 0
} , 1,
{ ease: expoOutStep, onComplete: function(t:FlxTween)
...
{ ease: sineOutStep, onComplete: function(t:FlxTween)
{ new FlxTimer().start(0.2, function(timer:FlxTimer)
{ FlxTween.tween(bg,
{ alpha: 0
} , 0.5,
...
; if (curCategorySelection < 0) curCategorySelection = categories.length - 1
; if (curCategorySelection > categories.length - 1) curCategorySelection = 0
; switchCategory(categories[curCategorySelection])
...
; arrowRight.setPosition(sectionText.x + sectionText.textField.textWidth + 10, sectionText.y + (sectionText.textField.textHeight - arrowRight.height) / 2)
...
{ switch (value)
{ case SelectingCategory: for (i in [sectionText, arrowLeft, arrowRight])
{ i.alpha = 1
; } sectionText.scale.set(1, 1)
; SoundController.play(Paths.sound('scrollMenu'), 0.7)
; case SelectingOption: for (i in [sectionText, arrowLeft, arrowRight])
{ i.alpha = 0.6
...
{ if (canInteract && menu.canInteract) selected = true
; } )
; onDeselected.add(function()
{ if (canInteract && menu.canInteract) selected = false
; } )
; }
} typedef CheckboxOptionParams =
{ > BaseOptionParams, var callback:Bool->Void
...
; var checkboxOffsets:Map<String, Array<Float>> = [ 'idle' => [0, 0], 'check' => [0, 17], 'check_idle' => [0.5, 4.5], 'unavailable' => [0, 0], ]
...
// ~300 is the start x position of the fill. descriptionText.fieldWidth = 700 - (descriptionText.x - 300)
; this.callback = params.callback
; onDeselected.add(function()
{ if (canInteract && menu.canInteract) checkboxScale.set(0.85, 0.85)
; } )
; onSelected.add(function()
{ if (canInteract && menu.canInteract) checkboxScale.set(1, 1)
; } )
; onAccept.add(function()
{ if (canInteract && menu.canInteract) setChecked(!checked)
; } )
...
{ > BaseOptionParams, var callback:Void->Void
...
// ~300 is the start x position of the fill. descriptionText.fieldWidth = 700 - (descriptionText.x - 300)
; onSelected.add(function()
{ if (canInteract) gear.scale.set(1, 1)
; } )
; onDeselected.add(function()
{ if (canInteract && menu.canInteract) gear.scale.set(0.9, 0.9)
; } )
; onAccept.add(function()
...
{ > BaseOptionParams, var min:Float
; var max:Float
; var callback:Float->Void
; } /** * An option that uses a slider for it's value.
*/ class SliderOption extends SettingsOption
{ public var value(get, set):Float
; function set_value(value:Float):Float return slider.value = value
; function get_value():Float return slider.value
...
; add(description)
; // 700 is the length of the fill of the clipboard
// ~300 is the start x position of the fill. description.fieldWidth = 700 - (description.x - 300)
; onSelected.add(function()
{ if (canInteract && menu.canInteract)
...
; function get_minHandleX():Float return (x - handle.width / 2) + offset.x
; public var maxHandleX(get, never):Float
; function get_maxHandleX():Float return x + _width - handle.width / 2 + offset.x
...
{ onRelease.dispatch()
; } updateValue()
; // Update the value variable if ((varString != null) && (Reflect.getProperty(_object, varString) != null))
{ value = Reflect.getProperty(_object, varString)
; } boundHandle()
; } override function updateValue()
{ if (_lastPos != relativePos)
{ if (callback != null) callback((relativePos * (maxValue - minValue)) + minValue)
...
; // Relative position can't be bigger than 1 if (pos > 1)
{ pos = 1
; } return pos
...
{ > BaseOptionParams, var options:Array<String>
...
; } public override function update(elapsed:Float)
{ super.update(elapsed)
; if (!canInteract || !menu.canInteract || !selected) return
; var left = PlayerSettings.controls.LEFT
; var leftP = PlayerSettings.controls.LEFT_P
; var right = PlayerSettings.controls.RIGHT
; var rightP = PlayerSettings.controls.RIGHT_P
...
; arrowRight.setPosition(selectedText.x + selectedText.textField.textWidth + 10, selectedText.y + (selectedText.textField.textHeight - arrowRight.height) / 2)
...
; if (callback != null && fireCallback) callback(this.value)
; }
}
