...
; } /** * A sub-menu in the settings menu used to help the user configure their in-game controls. * * Allows the user to be able to manually select a control, and set their keybinds. * Or they can just select one of the given presets to choose.
...
; /** * The current state of this menu. * This can either be that the user is: * * - Selecting a preset to choose from the list. * - Selecting a control to change the keybinds of. * - Selected a control, and is selecting a keybind to change. * - Changing a keybind.
*/ var curState(default, set):SelectingState
; function set_curState(value:SelectingState):SelectingState
{ switch (value)
{ case SelectingPreset: for (i in [presetTextTitle, presetArrowLeft, presetText, presetArrowRight])
{ i.alpha = 1
; } for (group in controlGroups)
{ group.available = false
; } case SelectingControl: for (i in [presetTextTitle, presetArrowLeft, presetText, presetArrowRight])
{ i.alpha = 0.6
; } for (group in controlGroups)
{ group.available = true
; } case SelectingKeybind: for (group in controlGroups)
...
; /** * Initalizes a new KeybindsMenu for the user. * @param parent The current settings menu.
...
; if (back)
{ switch (curState)
{ case SelectingKeybind: curControlGroup.deselectKeybinds()
; changeControlSelection()
; curState = SelectingControl
; case ChangeKeybind:
// Do nothing. default: parent.closeKeybindsMenu()
; }
} switch (curState)
{ case SelectingPreset: if (left || FlxG.mouse.overlaps(presetArrowLeft))
...
; } case SelectingControl: if (downP)
...
; } case SelectingKeybind: if (leftP)
{ changeKeybindSelection(-1)
; } if (rightP)
{ changeKeybindSelection(1)
; } if (accept)
{ curState = ChangeKeybind
; } case ChangeKeybind: var keybindsForControl:Array<FlxKey> = KeybindPrefs.keybinds.get(curControlGroup.uiControl.controlName)
...
; // Called when the user selects an invalid keybind. function invalidKeybind()
{ changeControlSelection()
...
{ // Don't check the keybind we're trying to bind already. if (i == curKeybindIndex) continue
; // Key is already binded. if (keybindsForControl[i] == key) keyAlreadyBinded = true
; } if (keyAlreadyBinded)
{ invalidKeybind()
; } else
...
; SoundController.play(Paths.sound('scrollMenu'), 0.7)
; if (curControlIndex < 0) curControlIndex = controlGroups.length - 1
; if (curControlIndex > controlGroups.length - 1) curControlIndex = 0
; selectControl()
...
; presetText = new FlxText(0, 200, 0, LanguageManager.getTextString('settings_keybinds_preset_$
{ keybindPresets[0]
} '))
...
; SoundController.play(Paths.sound('scrollMenu'), 0.7)
; if (curKeybindIndex < 0) curKeybindIndex = curControlGroup.keybindTextGroup.length - 1
; if (curKeybindIndex > curControlGroup.keybindTextGroup.length - 1) curKeybindIndex = 0
; selectKeybind()
...
; SoundController.play(Paths.sound('scrollMenu'), 0.7)
; if (curPresetIndex < 0) curPresetIndex = keybindPresets.length - 1
; if (curPresetIndex > keybindPresets.length - 1) curPresetIndex = 0
; updatePresetSelect()
; } function updatePresetSelect()
{ presetText.text = LanguageManager.getTextString('settings_keybinds_preset_$
{ keybindPresets[curPresetIndex]
} ')
...
; presetArrowRight.setPosition(presetText.x + presetText.textField.textWidth + 10, presetText.y + (presetText.textField.textHeight - presetArrowRight.height) / 2)
; } /** * Selects a given preset, and changes the controls based on it. * @param preset The preset to change to.
*/ function selectPreset(preset:String)
{ switch (preset)
{ case 'dfjk': KeybindPrefs.setKeybindPreset(KeyboardScheme.Solo)
; case 'askl': KeybindPrefs.setKeybindPreset(KeyboardScheme.Askl)
; case 'zx,.': KeybindPrefs.setKeybindPreset(KeyboardScheme.ZxCommaDot)
; default: KeybindPrefs.setKeybindPreset(KeyboardScheme.Duo)
; } SoundController.play(Paths.sound('confirmMenu'))
; canInteract = false
...
; return arrow
; }
} /** * Represents a control that the user's able to configure. * Contains all of the current keybinds.
...
; displayText = new FlxText(0, 0, 0, LanguageManager.getTextString('settings_keybinds_$
{ uiControl.displayName
} '))
