...
{ #if desktop checkbox_darkMode = new CheckboxOption(400, 600,
{ name: LanguageManager.getTextString('settings_window_darkMode'), description: LanguageManager.getTextString('settings_window_darkMode_description'), callback: function(value:Bool)
{ Preferences.darkMode = value
; }
} )
; checkbox_darkMode.setChecked(Preferences.darkMode, false, true)
; checkbox_darkMode.canInteract = !Preferences.borderless
; #end stepper_fps = new NumericStepperOption(375, 300,
{ name: LanguageManager.getTextString('settings_window_fps'), description: LanguageManager.getTextString('settings_window_fps_description'), min: 10, max: 240, stepper: 1, changeTimer: 0.02, callback: function(value:Float)
...
{ name: LanguageManager.getTextString('settings_window_borderless'), description: LanguageManager.getTextString('settings_window_borderless_description'), callback: function(value:Bool)
...
{ name: LanguageManager.getTextString('settings_window_VSync'), description: LanguageManager.getTextString('settings_window_VSync_description'), callback: function(value:Bool)
...
; add(checkbox_vsync)
; #if desktop
// Make sure this gets added last so it fits last in the group. list.push(checkbox_darkMode)
