...
{ // Cache the user's current language to use for later. if (selectedLanguage == null)
{ selectedLanguage = Preferences.language
; } var checkbox_hitsounds = new CheckboxOption(400, 300,
{ name: LanguageManager.getTextString('settings_misc_hitsounds'), description: LanguageManager.getTextString('settings_misc_hitsounds_description'), callback: function(value:Bool)
...
{ name: LanguageManager.getTextString('settings_misc_latencyOffsets'), description: LanguageManager.getTextString('settings_misc_latencyOffsets_description'), min: -2000, max: 2000, stepper: 1, changeTimer: 0.01, holdThreshold: 0.5, callback: function(value:Float)
...
{ name: LanguageManager.getTextString('settings_misc_language'), description: LanguageManager.getTextString('settings_misc_language_description'), options: [for (language in languages) LanguageManager.getTextString(language.id)], optionsID: [for (language in languages) language.id], selectCallback: function(value:String)
{ selectedLanguage = value
; var needsRestart:Bool = selectedLanguage != Preferences.language
; if (needsRestart)
{ // The language was changed, the game needs to be restarted. FlxG.stage.window.onClose.add(onWindowExit)
; } else
{ // User selected the same language they had before.
// The language doesn't need to be reset on exit. FlxG.stage.window.onClose.remove(onWindowExit)
...
{ name: LanguageManager.getTextString('settings_misc_resetData'), description: LanguageManager.getTextString('settings_misc_resetData_description'), callback: function()
