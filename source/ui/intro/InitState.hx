...
{ // Bind the save data to the correct path. FlxG.save.bind('funkin', 'dnbteam')
; // Sets sprites to be automatically antialiased when created. FlxSprite.defaultAntialiasing = true
; // Reduces physics accuracy in favor of higher FPS and animation framerate in states (DR those who know). FlxG.fixedTimestep = false
; // Sometimes audio automatically mutes because of the default Flixel save data.
// We need this to be controlled through the preferences instead of this. FlxG.sound.muted = false
; // Make sure the game auto pauses when you lose focus. FlxG.autoPause = true
; // Initalize the cursor. Cursor.initalize()
; // Load the user's preferences. Preferences.init()
; // Initalize controls. PlayerSettings.init()
; KeybindPrefs.loadControls()
; // Load any necessary save data. Highscore.load()
; CharacterSelect.initSave()
; FreeplayState.initSave()
; // Initalize Discord RPC. #if desktop DiscordClient.prepare()
; #end intializeRegistries()
; initalizePlugins()
; initalizeTransitions()
; Preloader.initalize()
; CrashHandler.initalize()
; #if debug if (FlxG.save.data.hasSeenOptionsReminder == null || !FlxG.save.data.hasSeenOptionsReminder)
{ FlxG.switchState(() -> new OptionsReminderState())
; } else
{ FlxG.switchState(() -> new TitleState())
; } #else FlxG.switchState(() -> new GameSplash())
; #end
...
{ // TODO: Move this to a registry maybe ? LanguageManager.init()
...
; FlxTransitionableState.defaultTransIn = new TransitionData(FADE, FlxColor.BLACK, 1, FlxPoint.get(-1, 0),
{ asset: diamond, width: 32, height: 32
} , new FlxRect(0, 0, FlxG.width, FlxG.height), NEW)
; FlxTransitionableState.defaultTransOut = new TransitionData(FADE, FlxColor.BLACK, 0.7, FlxPoint.get(1, 0),
{ asset: diamond, width: 32, height: 32
} , new FlxRect(0, 0, FlxG.width, FlxG.height), NEW)
