...
{ #if android FlxG.android.preventDefaultKeys = [BACK]
; // lol #end if (FlxG.random.int(0, 999) == 1) LoadingState.loadAndSwitchState(() -> new SusState())
; // Initalize intro text. introText = FlxG.random.getObject(getIntroText())
...
; var blackBg = FlxGradient.createGradientFlxSprite(FlxG.width, FlxG.height, [ FlxColor.interpolate(FlxColor.fromRGB(0, 0, 0), 0xFF4965FF, 0.6), FlxColor.interpolate(FlxColor.fromRGB(0, 0, 0), 0xFF00B515, 0.6) ], 1, 180)
; add(blackBg)
; bg = new FlxSprite()
...
; version.angle = -3
; add(version)
; #if desktop pressEnter = new FlxText(0, 600, 0, LanguageManager.getTextString('title_pressEnter'), 20)
; #else pressEnter = new FlxText(0, 600, 0, LanguageManager.getTextString('title_pressEnter_mobile'), 20)
; #end pressEnter.setFormat(Paths.font('comic.ttf'), 48, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK)
...
; #if switch if (gamepad.justPressed.B) pressedEnter = true
; #end
} #if mobile if (TouchUtil.justPressed) pressedEnter = true
; #end if (pressEnter != null && pressedEnter && !transitioning)
{ FlxTween.tween(pressEnter,
{ 'scale.x': 0, 'scale.y': 0, angle: 10
} , 0.5,
{ ease: FlxEase.backInOut
} )
; if (Preferences.flashingLights) FlxG.camera.flash(FlxColor.WHITE, 0.5)
...
; FlxTween.tween(introDisplayText.scale,
{ x: 1.0, y: 1.0
} , (Conductor.instance.crochet / 1100))
; return true
