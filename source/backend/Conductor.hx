...
; /** * The current instance of this Conductor. * This is the main instance of the Conductor that the game uses, and can be reset whenever.
...
; /** * The hit window the player has to hit a note before it's considered too early, or late. * Calculated based on beats.
*/ public final safeZoneOffset:Float = (safeFrames / 60) * 1000
; // is calculated in create(), is safeFrames in milliseconds
// CONDUCTOR INFORMATION
// /** * The current position this Conductor in at, in milliseconds. * Used to help calculate the Conductor's musical information.
*/ public var songPosition:Float
; /** * The amount of milliseconds this Conductor will be offset by. * Used normally to help with things like latency delay.
...
; /** * The starting BPM from when the Conductor was initalized. * Can be changed in-case you want to load another song, or use the Conductor for something else.
...
; } /** * The target step that the Conductor has yet to go to. * This is used so we can easily take into account if any steps, beats, or measures have been skipped, and call them.
...
; function get_stepCrochet():Float return stepCrochetOf(bpm, currentTimeChange.numerator, currentTimeChange.denominator)
; /** * The length of a beat for this Conductor, in milliseconds.
*/ public var crochet(get, never):Float
; function get_crochet():Float return crochetOf(bpm, currentTimeChange.numerator, currentTimeChange.denominator)
; /** * The length of a measure for this Conductor, in milliseconds.
*/ public var measureLength(get, never):Float
; function get_measureLength():Float return measureLengthOf(bpm, currentTimeChange.numerator, currentTimeChange.denominator)
; /** * The offset used for this Conductor. * Used in-case the player has any latency issues.
...
{ update(0, false)
; } /** * Initalizes the Conductor with time change basic parameters. * @param time The time of the Conductor * @param bpm * @param numerator * @param denominator
*/ public function initalize(bpm:Float, numerator:Int = 4, denominator:Int = 4)
{ // Create a basic time change object. var timeChange:SongTimeChange = new SongTimeChange(0.0, bpm, numerator, denominator)
; // Map the time change to further set it up, and reset the Conductor. mapTimeChanges([timeChange])
; reset()
; } /** * Updates this Conductor's position, and updates any musical information for it. * @param songPos The position to update it with. * @param canDispatch Whether signals should be dispatched if they're able to be. * @param applyOffsets Whether any global offsets are applied.
...
; if (songPos == null) songPos = Math.min(currentLength, currentTime)
; if (applyOffsets) songPos += offsets
; songPosition = songPos
; if (timeChangeMap.length == 0) return
; var newTimeChange = getTimeChangeAt(songPosition)
; if (currentTimeChange == null) currentTimeChange = newTimeChange
...
; } /** * Sets up and maps all of the given time changes. * @param songTimeChanges The time changes to map into the Conductor.
*/ public function mapTimeChanges(songTimeChanges:Array<SongTimeChange>)
{ if (songTimeChanges == null || songTimeChanges.length == 0) return
; timeChangeMap = []
; songTimeChanges.sort(SortUtil.sortTimeChanges)
; for (timeChange in songTimeChanges)
{ if (timeChangeMap.length == 0)
{ // This takes into account of in-case non zero timestamps. var numerator:Int = timeChange.numerator
...
; }
} /** * Gets a time change at a given position. * If none exists, it defaults to the fallback time change. * @param position The position to get the time change for. * @return A `SongTimeChange` object.
...
; } if (timeChange.time > position) break
; }
} return foundTimeChange
; } /** * Gets a step value at a certain time. * @param time The time in milliseconds to get the step value at. * @return A value in steps representing the time.
*/ public function getStepAtTime(time:Float):Float
{ if (timeChangeMap.length == 0) return time / stepCrochetOf(bpm)
...
; } /** * Given a step time, retrieve the time in milliseconds equal to the step time. * @param stepTime The step time to get the time in milliseconds for. * @return The stepTime in milliseconds.
*/ public function getTimeAtStepTime(stepTime:Float):Float
{ if (timeChangeMap.length == 0) return stepTime * stepCrochetOf(bpm)
; var lastChange:SongTimeChange = timeChangeMap[0]
; for (timeChange in timeChangeMap)
{ if (timeChange.stepTime < stepTime) lastChange = timeChange
; if (timeChange.stepTime > stepTime) break
; } return lastChange.time + ((stepTime - lastChange.stepTime) * stepCrochetOf(lastChange.bpm))
; } /** * Gets the start time for a given section of a song * @param section The section to get the time at. * @return The sections start time, in milliseconds.
...
; } if (timeChange.measureTime > section) break
...
; } /** * Updates this Conductor's musical information, and dispatches any events if necessary. * @param position The position to update based on. * @param canDispatch Whether any signals should be dispatched.
...
} /** * Loads a music data file from the given entry id and variation. * This'll also map any time changes the music data file will have. * @param id The entry id of the music data * @param variation (Optional) The variation of the music data.
*/ public function loadMusicData(id:String, ?variation:String):Void
{ if (!SongRegistry.instance.hasMusicDataFile(id, variation)) return
; var musicData:SongMusicData = SongRegistry.instance.loadMusicDataFile(id, variation)
; applyMusicData(musicData)
; } /** * Applies the given music data to this Conductor. * This'll also map the time changes the music data file may have. * @param musicData The music data to apply to this Conductor.
...
; } /** * Calculates the amount of steps needed for a beat given a denominator. * @param denominator The denominator to check. * @return The amount of steps needed for this demoniator.
*/ public static function beatSteps(denominator:Int = 4):Int return Std.int(STEP_VALUE / denominator)
; /** * Returns the length of a beat based on a given BPM. * @param bpm The BPM to check. * @return The length of a beat, in milliseconds.
*/ public static function beatLength(bpm:Float) return (60 / bpm) * 1000
; /** * Returns the length of a step based on a given BPM, and time signature. * @param bpm The BPM to check. * @return The length of a step, in milliseconds.
*/ public static function stepCrochetOf(bpm:Float, numerator:Int = 4, denominator:Int = 4) return beatLength(bpm) / 4
; /** * Returns the length of a beat based on a given BPM, and time signature. * @param bpm The BPM to check * @return The length of a beat, in milliseconds.
*/ public static function crochetOf(bpm:Float, numerator:Int = 4, denominator:Int = 4):Float return stepCrochetOf(bpm, numerator, denominator) * beatSteps(denominator)
; /** * Returns the length of a measure based on a given BPM, and time signature. * @param bpm The BPM to check * @param ts The time signature to check * @return The length of a measure, in milliseconds.
*/ public static function measureLengthOf(bpm:Float, numerator:Int = 4, denominator:Int = 4) return crochetOf(bpm, numerator, denominator) * numerator
; /** * Calculates the amount of beats in a measure based on a given time signature. * @param ts The time signature to check. * @return The number of steps in a measure.
*/ public static function measureBeats(numerator:Int = 4, denominator:Int = 4):Int return Std.int(measureSteps(numerator, denominator) / beatSteps(denominator))
; /** * Calculates the amount of steps in a measure based on a given time signature. * @param ts The time signature to check. * @return The number of steps in a measure.
*/ public static function measureSteps(numerator:Int = 4, denominator:Int = 4):Int return Std.int(numerator * beatSteps(denominator))
; }
