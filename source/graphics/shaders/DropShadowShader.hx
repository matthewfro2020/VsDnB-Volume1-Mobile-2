...
; return color
; } /* The angle of the drop shadow. for reference, depending on the angle, the affected side will be: 0 = RIGHT 90 = UP 180 = LEFT 270 = DOWN
...
; setFloat("thr2", val)
; return val
; } /* The FlxSprite that the shader should get the frame data from. Needed to keep the drop shadow shader in the correct bounds and rotation.
...
; baseContrast = c
; baseSaturation = s
; } /* Loads an image for the mask. While you *could* directly set the value of the mask, this function works for both HTML5 and desktop targets.
*/ public function loadAltMask(path:String)
{ #if html5 BitmapData.loadFromFile(path).onComplete(function(bmp:BitmapData)
{ altMaskImage = bmp
; } )
; #else altMaskImage = BitmapData.fromFile(path)
; #end
...
{ // NOTE: uv.width is actually the right pos and uv.height is the bottom pos setFloatArray("uFrameBounds", [frame.uv.left, frame.uv.right, frame.uv.left + frame.uv.top, frame.uv.right + frame.uv.bottom])
; // if a frame is rotated the shader will look completely wrong lol setFloat("angOffset", frame.angle * FlxAngle.TO_RAD)
; } public function new()
