...
{ /** * Loads all spritemap
{ i
} .json + its PNG into a combined FlxAtlasFrames.
...
; if (masterFrames == null) masterFrames = frames
; else masterFrames.frames = masterFrames.frames.concat(frames.frames)
; i++
...
{ for (f in frames.frames) if (f.name == name) return f
; return null
; }
}
