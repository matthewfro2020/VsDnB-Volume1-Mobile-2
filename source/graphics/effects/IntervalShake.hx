...
; /** * Shake an object based on a start and end interval. * @param Object The object to be shaking. * @param Duration The amount of time the shake should last for. * @param Interval The interval at which the object should shake. * @param startInterval The interval in which the shake should start with. * @param endInterval The interval in which the shake should end at. * @param axes The axes in which the object should shake at. * @param ease How the object's shake eases. * @param CompletionCallback Callback that's called after the shake is completed. * @param ProgressCallback Callback that's called every 'Interval' seconds. * @return A new IntervalShake object.
*/ public static function shake(Object:FlxObject, Duration:Float = 1, Interval:Float = 0.04, startInterval:Float = 0, endInterval:Float = 0, ?axes:FlxAxes, ?ease:EaseFunction, ?CompletionCallback:IntervalShake->Void, ?ProgressCallback:IntervalShake->Void):IntervalShake
{ if (isShaking(Object))
...
; } /** * Returns whether the object is shaking or not. * * @param Object The object to test.
*/ public static function isShaking(Object:FlxObject):Bool
{ return _boundObjects.exists(Object)
; } /** * Stops shaking of the object. Also it will make the object visible. * * @param Object The object to stop shaking.
*/ public static function stopShaking(Object:FlxObject):Void
{ var boundShake:IntervalShake = _boundObjects[Object]
; if (boundShake != null)
{ boundShake.stop()
; }
} function start(Object:FlxObject, Duration:Float, Interval:Float, startInterval:Float, endInterval:Float, ?axes:FlxAxes, ?ease:EaseFunction, ?CompletionCallback:IntervalShake->Void, ?ProgressCallback:IntervalShake->Void):Void
{ object = Object
; duration = Duration
; interval = Interval
...
; _pool.put(this)
; } /** * Helper function to update the shake's progression. * @param timer The timer for the shake.
...
; if (axes.x) object.x = initialPosition.x + (FlxG.random.float((-curInterval * object.width), (curInterval * object.width)))
; if (axes.y) object.y = initialPosition.y + (FlxG.random.float((-curInterval * object.height), (curInterval * object.height)))
