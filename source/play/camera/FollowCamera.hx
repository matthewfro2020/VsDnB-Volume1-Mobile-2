...
; switch (followType)
{ case EASE: followLerp = 1
; case LERP: followLerp = followLerpDuration
; case INSTANT: followLerp = 1
...
; // EASING OPTIONS
// /** * The tween object used to move the camera to it's given position. * The `setFollow()` function must be used to trigger this.
*/ public var followTween(default, null):FlxTween
; /** * Optional parameters that can be given to specified for the camera's ease. * For example, you can use this if you want to change the easing for the tween, or if you want something to happen when the tween complets.
*/ public var followEaseOptions:TweenOptions =
{
}
; /** * How long it should take for the camera to ease to it's given position. * Used for when the follow type is `FollowType.LERP`.
...
{ switch (followType)
{ case INSTANT:
// Disable note camera for instant camera movement as it's ugly. camFollow.setPosition(followPoint.x, followPoint.y)
; default: camFollow.setPosition(followPoint.x + cameraNoteOffset.x, followPoint.y + cameraNoteOffset.y)
; } super.update(elapsed)
; } /** * Immediately brings the camera to it's given target.
*/ public override function snapToTarget():Void
{ switch (followType)
{ case LERP: var lastLerp:Float = followLerp
; // Set the lerp to 1 so the camera instantly goes to the necessary position. followLerp = 1
; camFollow.setPosition(followPoint.x, followPoint.y)
; focusOn(camFollow.getPosition())
; // Reset the lerp back. followLerp = lastLerp
; super.snapToTarget()
; case EASE: TweenUtil.completeTweensOf(this)
; case INSTANT:
// Camera automatically snaps to the position so nothing needs to be done.
}
} /** * Sets the camera target destination to a given position. * @param x The x position the camera should go to. * @param y The y position the camera should go to.
...
; } switch (followType)
{ case EASE:
// Clear the tween. followTween?.cancel()
; followTween = null
; followTween = FlxTween.tween(followPoint,
{ x: x, y: y
} , followEaseDuration, followEaseOptions)
; default: followPoint.set(x, y)
; }
} /** * Snaps the camera to a given position. * @param x The x position the camera should snap to. * @param y The y position the camera should go to.
