...
; changeSelection()
; setupPauseCamera()
; #if mobileC addVirtualPad(UP_DOWN, A)
; addVirtualPadCamera()
; #end
...
; if (pauseMusic.volume < 0.75) pauseMusic.volume += 0.01 * elapsed
...
; super.close()
; } /** * Changes the currently selected option by the given amount. * @param change The amount to change by.
...
; }
} /** * Retrieves the pause options based on the current state of the game the user is playing. * @return A list of pause options.
...
; levelInfo.alpha = 0
; add(levelInfo)
; var composerInfo:FlxText = new FlxText(20, 15, 0, LanguageManager.getTextString('pause_composersText') + ': $
{ currentChart.songComposers.formatStringList()
} ')
...
; add(composerInfo)
; var artistInfo:FlxText = new FlxText(20, 15, 0, LanguageManager.getTextString('pause_artistsText') + ': $
{ currentChart.songArtists.formatStringList()
} ')
...
; artistInfo.alpha = 0
; add(artistInfo)
; var chartersInfo:FlxText = new FlxText(20, 15, 0, LanguageManager.getTextString('pause_chartersText') + ': $
{ currentChart.songCharters.formatStringList()
} ')
...
; add(chartersInfo)
; var codersInfo:FlxText = new FlxText(20, 15, 0, LanguageManager.getTextString('pause_codersText') + ': $
{ currentChart.songCoders.formatStringList()
} ')
...
; FlxTween.tween(composerInfo,
{ alpha: 1, x: composerInfo.x + 30
} , 0.4,
{ ease: FlxEase.quartInOut, startDelay: 1
} )
; FlxTween.tween(artistInfo,
{ alpha: 1, x: artistInfo.x + 40
} , 0.4,
{ ease: FlxEase.quartInOut, startDelay: 1.2
} )
; FlxTween.tween(chartersInfo,
{ alpha: 1, x: chartersInfo.x + 50
} , 0.4,
{ ease: FlxEase.quartInOut, startDelay: 1.4
} )
; FlxTween.tween(codersInfo,
{ alpha: 1, x: codersInfo.x + 60
} , 0.4,
{ ease: FlxEase.quartInOut, startDelay: 1.6
} )
...
{ var songText:Alphabet = new Alphabet(0, (70 * i) + 30, LanguageManager.getTextString('pause_$
{ menuItems[i].name
} '))
...
; }
} /** * Creates the camera used for the sub-menu. * This is done to make sure the zoom of the menu isn't changed, and that nothing interferes.
*/ function setupPauseCamera():Void
{ camera = new GameCamera()
; camera.bgColor.alpha = 0
; FlxG.cameras.add(camera, false)
; } // SELECTION OPTIONS
// /** * Closes this sub-menu. * Used for the `Resume` option
...
; Cursor.hide()
; FlxG.resetState()
; } /** * Toggles whether the player is able to miss notes. * Used for the `No Miss Mode` option.
*/ static function toggleNoMiss(state:PauseSubState):Void
{ PlayState.instance.noMiss = !PlayState.instance.noMiss
; } /** * Opens the settings menu while still staying paused. * Allows the user to change their options while still playing the song without having to restart.
...
{ if (PlayState.instance.currentDialogue == null) return
; PlayState.instance.currentDialogue.skipDialogue()
; state.close()
; } /** * Opens the character selection menu. * Will restart the song to make sure the changes are applied.
*/ static function changeCharacter(state:PauseSubState):Void
{ FlxG.switchState(() -> new CharacterSelect(
{ targetSong: PlayState.instance.currentSong
} ))
; } /** * Returns back to the user's last menu. * * If we're in story mode, we go back to story mode. * If we're in freeplay, we go back to the freeplay menu.
*/ static function returnBackToMenu(state:PauseSubState):Void
{ if (PlayStatePlaylist.isStoryMode) returnToMenu(() -> new StoryMenuState())
; else returnToMenu(() -> new FreeplayState())
...
{ // TODO: See if there's a way to softcoded this ? var selectToGo:NextState = switch (PlayState.instance.currentSong.id.toLowerCase())
{ case 'backseat': () -> new BackseatSelect()
; default: () -> new MainMenuState()
; } returnToMenu(selectToGo)
; } /** * Returns back to the given menu. * Reverts any needed changes before switching back * @param state The state to return back to.
*/ static function returnToMenu(state:NextState)
{ if (MathGameState.failedGame) MathGameState.failedGame = false
