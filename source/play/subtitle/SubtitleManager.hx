...
; /** * The scripted
class that subtitles are initalized from. * If the subtitle has it's own special scripted
class , that is used instead. * @return Null<String>
...
; } /** * The sounds that'll when subtitles are being typed out. * If the subtitle has it's own special sounds, that is used instead. * @return Null<Array<String>>
...
; /** * The conductor that this subtitle container is running on. * The container will check if the conductor is at the subtitle's given time based on this conductor.
...
{ if (_conductor == null) _conductor = Conductor.instance
; return _conductor
...
{ this.revive()
; // Initalize the subtitles when this container is created. songSubtitles = getDataSubtitles().copy()
; // Clear any existing subtitles. subtitlesGroup.clear()
...
{ // Make sure any subtitles currently rendering get received script functions this container does as well. subtitlesGroup.forEach((subtitle:Subtitle) ->
{ if (subtitle != null)
{ ScriptEventDispatcher.callEvent(subtitle, event)
; }
} )
; } /** * Adds a subtitle to the manager. * @param data The subtitle to add.
...
; } /** * Called when a subtitle has finished. * @param subtitle The subtitle that was completed.
...
; } /** * Retrieves the data for this entry. * @param id The id of the entry. * @return The data object for this entry.
...
; } subtitlesGroup.clear()
; super.destroy()
; } /** * Returns a string representation of this entry. * @return String
