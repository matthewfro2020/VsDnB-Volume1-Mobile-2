...
; /** * A list of all of the hard-coded type for a timer. * TODO: Softcode this ?
*/ final types:Map<String, TimerType> = [ 'normal' =>
{ graphic: Paths.image('ui/timer'), offsets: FlxPoint.get(0, 0), antialiasing: true
} , '3d' =>
{ graphic: Paths.image('ui/timer-3d'), scale: FlxPoint.get(0.9, 0.9), offsets: FlxPoint.get(-5, -4), antialiasing: false
} ]
...
; /** * The circular visual that actually displays the timer. * Overlayed over the fill graphic to show how much time of the song is left.
...
{ // If we're in minimal mode, we DON'T want to draw this. if (Preferences.minimalUI) return
; super.draw()
; } /** * Updates the timer's graphic to represent specified type. * @param type The type to change the timer to.
*/ public function switchType(type:String)
{ var timerType:TimerType = getType(type)
; if (timerType == null) return
...
; case 'elapsedAndLeft': '$
{ FlxStringUtil.formatTime(time / 1000)
} / $
{ FlxStringUtil.formatTime(length / 1000)
} '
; default: ''
; } timerText.x = timerGraphic.x + (timerGraphic.width - timerText.textField.textWidth) / 2
