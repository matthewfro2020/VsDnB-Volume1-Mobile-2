...
*/ public final noAaChars:Array<String> = [ 'dave-angey', 'bf-pixel', 'gf-pixel', 'bf-3d', 'gf-3d', 'baldi-3d', 'exbungo', ]
; /** * The character id this icon is based off.
*/ public var char(default, set):String
; public function set_char(value:String)
{ if (this.char == value) return value
; if (value == 'none' || !Assets.exists(Paths.imagePath('iconGrid/$
{ value
} ')))
{ value = 'face'
...
; return char = value
; } /** * The current state the icon is in. * This can either be `losing`, or `winning`
...
; if (autoOffset) offset.set(Std.int(FlxMath.bound(width - 150, 0)), Std.int(FlxMath.bound(height - 150, 0)))
; if (sprTracker != null) setPosition(sprTracker.x + sprTracker.width + 10 + offsets.x, sprTracker.y + (sprTracker.height - this.height) / 2 + 25 + offsets.y)
; } /** * Changes the state of the icon. * @param charState The new state.
