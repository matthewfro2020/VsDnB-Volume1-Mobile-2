...
; /** * Initalizes the Countdown to be ready for use. * @param char The character to use for the countdown.
*/ public static function initalize(char:Character):Void
{ Countdown.char = char
; FlxG.console.registerClass(Countdown)
; } /** * Starts the countdown. * Caches the sprites first, and initalizes the timer to start. * * @param skip Whether to skip the countdown entirely.
...
{ case THREE, TWO, ONE, GO: onIncrement.dispatch(countdownStep)
; case FINISH: stop()
; onFinish.dispatch()
; default:
} // Dispatch the post-tick event. dispatchEvent(new CountdownScriptEvent(COUNTDOWN_TICK_POST, countdownStep, true))
; }
} , 5)
...
; } /** * Pauses the countdown as long as it's currently running. * To resume the countdown, simply call `resume()`
...
; countdownTimer = null
; } countdownStep = START
; } /** * Shows the graphic for the given step. * @param step The step to show the graphic for.
...
; FlxTween.tween(spr,
{ alpha: 0
} , Conductor.instance.crochet / 1000,
{ ease: FlxEase.cubeInOut, onComplete: function(twn:FlxTween)
{ spr.destroy()
; }
} )
; } /** * Plays a sound based on the given countdown step. * @param step The step to play the sound for.
...
; switch (countdownStep)
{ case START: event = new CountdownScriptEvent(COUNTDOWN_START, countdownStep, true)
; case THREE, TWO, ONE, GO: event = new CountdownScriptEvent(COUNTDOWN_TICK, countdownStep, true)
; case FINISH: event = new CountdownScriptEvent(COUNTDOWN_END, countdownStep, true)
; } dispatchEvent(event)
; return event.eventCanceled
; } /** * Gets the animation name for the given countdown step. * @param step The step to get the animation name for. * @return A Countdown Step's animation name.
...
; default: null
; }
} /** * Gets the countdown graphic for the given step. * @param step The Countdown step. * @return The graphic for this step.
...
; if (stepGraphic == null) return null
...
; } var characterResult:Null<String> = checkForAsset(path, 'characters/$
{ char.id
} ', stepGraphic)
; var countdownResult = checkForAsset(path, '$
{ char.countdownGraphicType
} ', stepGraphic)
...
; }
} /** * Gets the name of the graphic for the given step. * @param step The step to get the graphic name for. * @return The name of the graphic for the step.
...
; default: null
; }
} /** * Gets the sound asset path for the given countdown step. * @param step The step to get the asset path for. * @return The asset path for the step.
*/ static function countdownSound(step:CountdownStep):Null<String>
{ var stepSound:Null<String> = countdownSoundStep(step)
; if (stepSound == null) return null
; if (Assets.exists(Paths.soundPath('countdown/$
{ char.countdownSoundType
} /$stepSound')))
{ return 'countdown/$
{ char.countdownSoundType
} /$stepSound'
...
; }
} /** * Gets the name of the sound for the given step. * @param step The step to get the sound asset name for. * @return The name of the sound for the step.
...
; default: null
; }
} /** * Caches all of the graphic that'll be played during the Countdown. * Helps with majorly decreasing lag, and loading times.
*/ public static function cacheSprites()
{ for (i in [TWO, ONE, GO])
{ Preloader.cacheImage(Paths.imagePath(getCountdownGraphic(i)))
; }
} /** * Caches all of the sounds that'll be played during the Countdown. * Helps with majorly decreasing lag, and loading times.
