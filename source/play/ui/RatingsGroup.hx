...
{ /** * List of all the types of ratings used, and the data for each. * TODO: Probably best to softcode this.
*/ var types:Map<OneOfTwo<String, Array<String>>, RatingsType> = [ 'normal' =>
{ directory: 'normal/', size: 0.7, antialiasing: true
} , ['3d', 'shape'] =>
{ directory: '3D/', size: 0.7, antialiasing: false
} , 'pixel' =>
{ directory: 'pixel/', size: 6, antialiasing: false
} ]
...
{ ratingData.directory
} combo'))
; comboSpr.setGraphicSize(Std.int(comboSpr.width * ratingData.size))
; comboSpr.updateHitbox()
; comboSpr.antialiasing = ratingData.antialiasing
; return style = value
; } /** * The rating sprite of this group. * Updates in accordance to the style. * Gets reused when popups happen for performance.
*/ var ratingSpr:FlxSprite
; /** * The combo sprite of this group. * Updates in accordance to this style. * Gets reused when popups happen for performance.
...
{ if (!Preferences.minimalUI) super.draw()
; } /** * Caches a the specified rating style. * Useful to make sure the game doesn't lag when a player hits a note. * @param style The style to be cached.
*/ public function cacheStyle(?style:String)
{ var data:RatingsType = getData(style)
; for (i in 0...10)
{ Preloader.cacheImage('ui/combo/$
{ data.directory
} num$
{ i
} ')
...
; }
} /** * Gets the data for the specified style. * @param style The style to get the data of.
*/ function getData(style:String)
{ var ratingData:RatingsType = types.get('normal')
; for (key => value in types)
{ if (key.contains(style)) ratingData = value
; } return ratingData
; } /** * Displays a visual popup showing the rating based on how a player is doing. * @param rating The rating to show. * @param combo The current combo the player has, used to display a 'combo' graphic if the specified combo is high enough. * @param style The style the rating should be.
*/ public function ratingPopup(rating:String, combo:Int, ?style:String)
{ var ratingData:RatingsType = getData(style ?? this.style)
; ratingSpr.loadGraphic(Paths.image('ui/combo/$
{ ratingData.directory
} $
{ rating
} '))
...
; // Reset the rating sprite to be re-used. ratingSpr.alpha = 1
...
; if (hasCombo)
{ // Reset the combo sprite to be re-used. comboSpr.alpha = 1
...
{ var numScore:FlxSprite = new FlxSprite().loadGraphic(Paths.image('ui/combo/$
{ ratingData.directory
} num$
{ comboSplit[i]
} '))
...
; if (numList.length == 0)
{ // Center the number to the combo sprite. numScore.x = (this.comboSpr.x - this.x) + comboSpr.width + 2
...
; } /** * Helper function to a quick tween relating to ratings. * @param spr The rating sprite to do a tween of. * @param delayTime Delay time before the rating disappears. Defaults to 1. * @param onComplete Function to call when the tween is complete.
*/ function ratingTween(spr:FlxSprite, delayTime:Float = 1, ?onComplete:FlxTween->Void)
{ FlxTween.tween(spr,
{ alpha: 0
} , 0.2,
{ onComplete: onComplete, startDelay: (Conductor.instance.crochet / 1000) * delayTime
} )
; }
}
