...
; } /** * The name of this display element. * Also known as the asset path.
...
; /** * Called whenever the text changes. * Used usually if you want to change what the text looks like.
...
; icon = new FlxSprite().loadGraphic(Paths.image('ui/$
{ params.name
} '))
...
{ // If we have minimal UI enabled, we DON'T want to draw this. if (Preferences.minimalUI) return
; super.draw()
