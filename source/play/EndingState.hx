...
*/ var ending:String
; /** * The song that should play for this ending. * Optional, if none is entered the good ending theme plays automatically.
*/ var ?song:String
...
; } /** * A state the player goes to after completing a story mode week. * Displays a visual ending depending on the player's score.
*/ class EndingState extends MusicBeatState
{ /** * The last parameters the player had. * Fallback in-case no parameters exist.
...
; this.endingTitleText = LanguageManager.getTextString('ending_title_$
{ ending
} ')
...
{ endingSpr.loadGraphic(Paths.image('endings/$week/$
{ this.ending
} _$week', 'shared'))
; } else
{ endingSpr.frames = Paths.getSparrowAtlas('endings/$week/$
{ this.ending
} _$week', 'shared')
...
; for (touch in FlxG.touches.list) if (touch.justPressed) justTouched = true
