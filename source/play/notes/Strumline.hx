...
{ // STATIC VARIABLES
// /** * The normal width of a strum receptor. * Used when generating the strums to position them.
...
; /** * The custom speed at which the notes go by. * If null, the song's chart speed is used instead.
*/ @:isVar public var scrollSpeed(get, set):Null<Float> = null
...
; } /** * The current scrolling type of the strumline. * Can either be 'downscroll' or 'upscroll'. * Updating this will update the strum and sustain notes to their appropriate positions.
...
// StrumCamera that flips instead of the strums itself (This would make it easier to do modcharts also). TweenUtil.completeTweensOf(strum, true)
; switch (value)
{ case 'downscroll': strum.y = DOWNSCROLL_Y
; case 'upscroll': strum.y = UPSCROLL_Y
; }
} )
...
; // VARIABLES
// /** * Whether the strumline is able to call the update() function. * Useful for when you want the strumline to only be used for graphical purposes, like for example only showing the strum receptors.
*/ public var canUpdate:Bool = true
; /** * The conductor this strumline in running on. * If none is specified, it uses the current static Conductor instance.
...
; /** * The amount of strums this strumline has. * This can be used to support multi-key.
*/ public var strumAmount:Int = 4
; /** * Mapping of the currently held note directions.
*/ private var heldKeys:Array<Bool> = []
; /** * The current starting index in the notes list. * Used to iterate through the chart to spawn notes.
...
; handleNoteSpawning()
; updateNotes()
; } /** * Generates a list of notes from a given chart data. * @param data The chart data to be used for generating the notes.
...
; if (note.direction > 3) gottaHitNote = !section.mustHitSection
; if (gottaHitNote != isPlayer) continue
; unspawnNotes.push(note)
; }
} unspawnNotes.sort(sortByDataStrumTime)
; } /** * Generates a group of strum receptors for this strumline to use. * @param fadeIn Whether there should be a fade-in effect when the strums are created.
...
; if (!params.showStrums) babyArrow.alpha = 0.0
; } if (fadeIn) fadeNotes()
; } /** * Re-generates the strums receptors of this strumline. * @param fadeIn Whether there should be a fade-in effect when the strums are created.
...
; // Note's blank. if (noteData == null) return
; // If the note's below the start of the song, or it's below the hit window. if (noteData.time < 0.0 || noteData.time < hitWindowStart)
{ nextNoteIndex = noteIndex + 1
; continue
; } // Note's too far ahead to render. if (noteData.time > startRenderTime) break
; var note:Note = buildNote(noteData)
; this.notes.add(note)
; if (noteData.length > 0)
{ note.sustainNote = buildHoldNote(noteData, note)
; this.sustains.add(note.sustainNote)
; } // Increment the note index. nextNoteIndex = noteIndex + 1
...
// This also updates the state of any rendering sustain notes. updateNoteState(note)
...
; }
} // Note is outside, destroy it. if (conductor.songPosition >= note.strumTime + (350 / (pixelsPerMs * noteSpeed)))
{ if (isPlayer && note.handledMissed) onNoteMiss.dispatch(note)
; killNote(note)
; }
} )
; forEachHoldNote(function(holdNote:SustainNote)
{ if (holdNote.sustainLength < holdNote.fullSustainLength)
{ // Hold note was dropped as it was being held, it's been missed. if (isPlayer && (!isKeyHeld(holdNote.direction) || (!isKeyHeld(holdNote.direction) && holdNote.noteStyle == 'shape' && !PlayerSettings.controls.KEY5)))
{ holdNote.cover?.hide()
; strums.members[holdNote.direction].playStatic()
; holdNote.hasMissed = true
; }
} // Calculate the these values of this hold note, as they gets used in each of the hold note's states. var holdNoteSpeed:Float = scrollSpeed * holdNote.localScrollSpeed
...
{ // Hold note is offscreen, kill it. killSustain(holdNote)
; } else if (holdNote.hasMissed && (holdNote.fullSustainLength > holdNote.sustainLength))
{ // Hold note was dropped as it was held, keep in it's clipped state. var yOffset:Float = SustainNote.sustainHeight(holdNote.fullSustainLength - holdNote.sustainLength, holdNoteSpeed)
...
{ holdNote.cover?.hide()
; }
} else if (conductor.songPosition >= holdNote.strumTime && holdNote.hasBeenHit && !holdNote.hasMissed)
// Hold note's currently being hit, clip it, and position it.
...
; // Reset the character hold timer to make sure they keep singing. if (character != null && character.holdTimer > 0)
{ character.holdTimer = 0
; } // Hold note's been complete, kill it. if (holdNote.sustainLength <= 0)
{ strums.members[holdNote.direction].playStatic()
...
{ // Hold note is new, position it normally. holdNote.y = if (scrollType == 'downscroll')
...
// Clear the hold cover so it doesn't persistent. if (holdCover.holdNote == null || holdCover.holdNote.sustainLength <= 0 && holdCover.animation.curAnim.name.startsWith('loop'))
{ holdCover.hide()
; }
}
} /** * Updates a note's logic checking if it's able to be hit, if it's too late, or early, etc. * @param note The note to update.
...
; note.tooEarly = true
; }
} /** * Cleans up the strumline by removing every rendering note, and resetting any needed properties. * Used for when time in a song has been skipped.
...
; } )
; nextNoteIndex = 0
; } /** * Creates, or recycles a note to be reused. * @return A newly created, or previously used note.
...
; } return note
; } /** * Builds a note sprite from the specified data. * @param data The data to build the note off of. * @return A note sprite that's ready to be used.
...
; return note
; } /** * Creates, or recycles a hold note to be reused. * @return A newly created, or previously used note.
*/ function constructHoldNote():SustainNote
{ var note:SustainNote = null
; note = recyclableHoldNotes.getFirstAvailable()
; if (note != null)
{ // Revive the previously used hold note. note.revive()
; this.recyclableHoldNotes.remove(note)
; } else
{ // Create a new one to be built later on. note = new SustainNote(0, 0, false, 0)
; } return note
; } /** * Builds a hold note sprite from the specified data. * @param data The data to build the note off of. * @return A hold note sprite that's ready to be used.
*/ function buildHoldNote(data:SongNoteData, parentNote:Note):SustainNote
{ // Setup graphic. var holdNote:SustainNote = constructHoldNote()
; // Copy properties from parent note. holdNote.parentStrumline = this
...
; return holdNote
; } /** * Creates, or recycles a new hold cover to be built onto a hold note. * @return A new hold cover.
*/ function constructHoldCover():HoldCover
{ var holdCover:HoldCover = recyclableHoldCovers.getFirstAvailable()
; if (holdCover != null)
{ // Revive a new hold cover to re-use. holdCover.revive()
; recyclableHoldCovers.remove(holdCover)
; } else
{ // Create a new hold cover. holdCover = new HoldCover(0, 'normal')
; } holdCover.alpha = this.alpha
; this.holdCovers.add(holdCover)
; return holdCover
; } /** * Sets up a new hold cover to use for a hold note. * @param holdNote The hold note to build the hold cover off of. * @return A new hold cover ready to be used.
*/ function startHoldCover(holdNote:SustainNote):HoldCover
{ var noteStyle:NoteStyle = holdNote.noteStyle
; if (!noteStyle.hasHoldCovers) return null
...
; // This helps prevent the hold cover showing for 1 frame before being applied. holdCover.alpha = holdCover.strum.alpha
; holdNote.cover.playStart()
; holdCover.visible = true
; holdCover.onKill.add(killHoldCover)
; } return holdCover
; } /** * Registers a note from this strumline as hit. * @param note The note to hit.
...
; killNote(note)
; } /** * Remove a note from it's associated rendering group. * @param note The note to destroy.
*/ public function killNote(note:Note)
{ if (note == null) return
; note.visible = false
; note.kill()
; recyclableNotes.add(note)
; } /** * Destroy a sustain note, and removes it from it's rendering group. * @param note The sustain note to destroy.
...
; } /** * Kills a hold cover, and removes it from it's rendering group. * @param cover The cover to kill.
...
; } /** * Iterates through each strum receptor. * @param func The function to call for each strum.
...
} /** * Iterates through each rendering note. * @param func The function to call for each note.
*/ public function forEachNote(func:Note->Void)
{ for (i in notes.members)
{ if (i == null || !i.exists || !i.alive) continue
; func(i)
; }
} /** * Iterates through each rendering hold note. * @param func The function to call for each note.
*/ public function forEachHoldNote(func:SustainNote->Void)
{ for (i in sustains.members)
{ if (i == null || !i.exists || !i.alive) continue
; func(i)
; }
} /** * Called when the specific note direction key is pressed. * @param direction The note direction to press.
*/ public function pressKey(direction:Int)
{ heldKeys[direction] = true
; } /** * Called when the specific note direction key is pressed. * @param direction The note direction to release.
*/ public function releaseKey(direction:Int)
{ heldKeys[direction] = false
; } /** * Checks whether the given note direction key is being pressed. * @param direction The direction to check. * @return Whether the direction key is pressed.
*/ function isKeyHeld(direction:Int):Bool
{ return heldKeys[direction]
; } /** * Gets a list of notes that are within the hit window. * @return A list of note that this strumline is able to hit.
...
; } )
; } /** * The default function for calculating the y position of a note. * @param strumTime The note's strum time to calculate the position for. * @param strumLine The target strumline object the note's going to. * @param speed The speed in which the note is going at. * @param downScroll Whether the note's rendering on downscroll. * @return A y position, in pixels, to be used for the note.
...
; return val
; } /** * Rotates a position by a certain angle and multiplier. * TODO: Put this in it's own utility
class ? * @param dist The base distance to calculate off of. * @param angle The angle to rotate the 'distance' * @param ymult A multipler for the y value after the distance has been rotated. * @return A rotated point.
...
; return point
; } /** * Compares a note by it's 'strumTime' value. * TODO: Put this in it's own sorting utility? * @param Obj1 The first note data to compare. * @param Obj2 The second note data to compare. * @return A comparing value to use when sorting.
...
; } /** * Compares a note by it's 'strumTime' value. * TODO: Put this in it's own sorting utility? * @param order The order to sort the notes in. * @param Obj1 The first note to compare. * @param Obj2 The second note to compare. * @return A comparing value to use when sorting.
*/ function compareNotes(order:Int, Obj1:Note, Obj2:Note):Int
{ return FlxSort.byValues(order, Obj1?.strumTime, Obj2?.strumTime)
; } /** * Compares a hold note by it's 'strumTime' value. * TODO: Put this in it's own sorting utility? * @param order The order to sort the notes in. * @param Obj1 The first note to compare. * @param Obj2 The second note to compare. * @return A comparing value to use when sorting.
*/ function compareHoldNotes(order:Int, Obj1:SustainNote, Obj2:SustainNote):Int
{ return FlxSort.byValues(order, Obj1?.strumTime, Obj2?.strumTime)
; } override function set_x(value:Float):Float
{ var diff:Float = value - this.x
; // Make sure the base X doesn't change for the strums if this strumline moves. forEachStrum((strum:StrumNote) ->
{ strum.baseX += diff
; } )
