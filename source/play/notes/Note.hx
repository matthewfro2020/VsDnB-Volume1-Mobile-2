...
; /** * Whether the note's position should be controlled through trigonometric rotation. * TODO: Re-factor this?
...
; /** * The scale of this note from when it was first generated. * Helpful for internally making calculations for changing the scale.
*/ public var baseScale(get, never):Float
; function get_baseScale():Float
{ return baseStyle.styleSize
; } // VARIABLES
// /** * Flag variable for if the sustain note isn't within the hit window and can't be hit. * Controlled by the note's strumline.
...
; /** * Flag variable for if the sustain note either wasn't held within the window. * Controlled by the note's strumline.
*/ public var tooLate:Bool = false
; /** * The sustain note used for when this note needs to be held.
*/ public var sustainNote:SustainNote
; /** * The strum receptor associated with this note. * Normally comes from the opponent's strumline, or the player strumline, but you can customize it if you want!
*/ public var strum:StrumNote
; /** * The character associated with this note. * Normally this is either the opponent, or the player, but you can customize it if you want!
*/ public var character:Character
; /** * The color directions that this note is being used to render this note. * TODO: Maybe re-factor this into it's own
class ?
...
; /** * The original direction of the note. * Exists in-case the direction changes due to modcharts, or anything else.
*/ public var originalType:Int = 0
; /** * Internal variable for keeping track of Bambi phone smashes. * TODO: Is this needed?
...
; public function new(direction:Int, ?musthit:Bool = true, noteStyle:NoteStyle = "normal", inCharter:Bool = false)
{ super(0, -9999)
...
; // Cancel gameplay logic if this note's being used in the chart editor. if (inCharter) return
; if (strum != null)
{ copyStrum()
; }
...
; handledMissed = false
; hasBeenHit = false
; } /** * Setups the animations and graphic for this note to used based on a note style. * @param noteStyle The note style to use for this note.
...
; if (!rotate) x = strum.x + (strum.width - this.width) / 2
; updateAlpha()
; if (copyAngle)
{ angle = strum.angle
; } if (copyScale)
...
; if (hasMissed) missModifier = 0.4
; if (copyAlpha) alpha = strum.alpha * alphaModifier * missModifier
...
; copyStrum()
; } /** * Sets the parent character for this note to use. * If no custom character is set. It's either the opponent or the player based on it's data properties. * @param char The parent character to use.
