...
{ if (noteStyle == value) return value
; setupHoldCoverSprite(value)
; return noteStyle = value
; } /** * The direction of the hold cover. * This is usually the same direction of the hold note.
...
; /** * Initalize a new hold cover sprite. * Usually only needed to do if all of the hold covers are full, and none are them are able to be used. They're normally recycled. * @param direction The direction of the hold cover. * @param noteStyle The note style of the hold cover.
...
; copyStrum()
; } /** * Sets up the hold cover based on a note style. * @param style The note style to apply to this hold cover.
*/ public function setupHoldCoverSprite(style:NoteStyle)
{ style.applyStyleToHoldCover(this)
; } /** * Called when an animation for this hold cover is finished. * @param anim The animation that was finished.
*/ function onAnimationFinish(anim:String)
{ switch (anim)
{ case 'start': playLoop()
; case 'end': isFinished = true
; hide()
; }
...
; } /** * Plays a hold cover animation. Adjusts the offsets according to the animation played. * @param anim The animation to play. * @param force Whether to wait for the current animation to finish, or not.
...
; this.y = strum.y + (strum.height - this.height) / 2
; }
} /** * Plays the 'start' animation for a hold cover. * If this animation doesn't exist, it plays the loop animation instead.
...
; } /** * Plays the ending animation for this hold cover. * If an animation doesn't exist, the cover is killed. * Else, after this is finished, then it's killed.
...
{ hide()
; }
} /** * Checks whether the hold cover for this note style has an start animation. * @return `Bool`
*/ public function hasStartAnimation():Bool
{ return animation.exists('start')
; } /** * Checks whether the hold cover for this note style has an end animation. * @return `Bool`
*/ public function hasEndAnimation():Bool
{ return animation.exists('end')
; } /** * Hides the cover. * This kills the cover allowing for it to be recycled for performance.
