...
; return style = value
; } return value
; } /** * The x position, in terms of the strumline position, in which the strum is centered. * Essentially, the 'middlescroll' position.
*/ public var centerX(get, null):Float
; function get_centerX():Float return baseX + 320 * (playerStrum ? -1 : 1) + 78 / 4
...
; /** * The trigonometric rotation of this strum object. * Used for when the notes use trigonometric rotation for calculation it's position.
...
{ switch (name)
{ case 'confirm': if (!this.playerStrum)
{ playAnim('static')
; }
}
} )
; } /** * Plays the strum's 'static' animation. * @param force Whether to play the animation immediately.
*/ public function playStatic(force:Bool = true)
{ playAnim('static', force)
; } /** * Plays the strum's 'press' animation. * @param force Whether to play the animation immediately.
*/ public function playPress(force:Bool = true)
{ playAnim('pressed', force)
; } /** * Plays the strum's 'confirm' animation. * @param force Whether to play the animation immediately.
*/ public function playConfirm(force:Bool = true)
{ playAnim('confirm', force)
; } /** * Plays the confirm animation for when a hold note is being held down. * Normally, this is the same as the regular confirm animation. * However, this is a separate animation to allow for customizability.
*/ public function holdConfirm(force = false)
{ if (animation.curAnim.name == 'confirm-hold')
{ return
; } playAnim('confirm-hold', force)
; } /** * Plays a strum animation. Takes into account any offsets that need to be done. * @param anim The animation to play. * @param force Whether the animation should immediately play, or wait until no other animation is playing.
...
; offset.y += offsets[1]
; }
} public inline function resetX() x = baseX
; public inline function resetY() y = baseY
; public inline function centerStrum() x = centerX
; }
