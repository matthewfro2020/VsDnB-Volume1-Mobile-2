...
; }
} /** * The type of graphic used for this note style. * This can either be a sparrow asset, or a tiled graphic asset given the width, and height of the asset.
...
; default: Sparrow
; }
} /** * Applies 'this' note style data onto a note object. * @param sprite The note to apply 'this' note style to.
...
; } /** * Applies 'this' note style data onto a sustain object. * @param sprite The sustain note to apply the note style to.
...
; } /** * Applies 'this' note style data onto a strum object. * @param sprite The strum to apply the note style to.
...
; } /** * Applies 'this' note style data onto a hold cover. * @param sprite The hold note to apply the note style to.
...
; } /** * Loads the graphic asset of 'this' note style onto a note sprite. * @param sprite The sprite to apply the graphic asset to. * @param path The path to the sprite.
*/ function loadStyleToSprite(sprite:FlxSprite, path:String)
{ switch (graphicType)
{ case TILES(width, height): switch (this)
{ case 'pixel':
// TODO: Is there a better way to handle paths for sustains than js this? if (sprite is SustainNote)
...
; default: sprite.loadGraphic(Paths.image(path), true, width, height)
; } case Sparrow: sprite.frames = Paths.getSparrowAtlas(path, 'shared')
; }
} /** * Loads the graphic asset of 'this' note style onto a note sprite. * @param sprite The hold cover to apply the graphic asset to.
*/ function loadStyleToHoldCover(sprite:HoldCover)
{ switch (this)
{ case 'pixel': sprite.frames = Paths.getSparrowAtlas('ui/notes/pixel/pixelNoteHoldCover', 'shared')
; case '3d': var directions:Array<String> = ['purple', 'blue', 'green', 'red']
; sprite.frames = Paths.getSparrowAtlas('ui/notes/3d/covers/covers_3d_$
{ directions[sprite.direction]
} ', 'shared')
; default: var directions:Array<String> = ['Purple', 'Blue', 'Green', 'Red']
; sprite.frames = Paths.getSparrowAtlas('ui/notes/normal/covers/holdCover$
{ directions[sprite.direction]
} ', 'shared')
; }
} /** * Applies the animations of 'this' note style onto a note object. * @param sprite The note to apply the animations to.
*/ function applyAnimsToNote(sprite:Note)
{ switch (this)
{ case 'pixel': sprite.animation.add('purpleScroll', [0])
; sprite.animation.add('blueScroll', [1])
; sprite.animation.add('greenScroll', [2])
; sprite.animation.add('redScroll', [3])
; case 'shape': sprite.animation.addByPrefix('greenScroll', 'green0')
...
; default: sprite.animation.addByPrefix('purpleScroll', 'purple0')
; sprite.animation.addByPrefix('blueScroll', 'blue0')
; sprite.animation.addByPrefix('greenScroll', 'green0')
; sprite.animation.addByPrefix('redScroll', 'red0')
; }
} /** * Applies the animations of 'this' note style onto a sustain object. * @param sprite The sustain to apply the animations to.
*/ function applyAnimsToSustain(sprite:SustainNote)
{ switch (this)
{ case 'pixel': sprite.animation.add('purplehold', [0])
...
; case 'shape': sprite.animation.addByPrefix('purplehold', 'purple hold piece')
...
; default: sprite.animation.addByPrefix('purpleholdend', 'pruple end hold')
...
; } /** * Applies the animations of 'this' note style onto a strum object. * @param sprite The strum to apply the animations to.
*/ function applyAnimsToStrum(sprite:StrumNote)
{ var ID:Int = Std.int(Math.abs(sprite.ID))
; switch (this)
{ case 'pixel': sprite.animation.add('static', [ID])
...
; default: var anims:Array<String> = ['left', 'down', 'up', 'right']
; sprite.animation.addByPrefix('static', 'arrow$
{ anims[ID].toUpperCase()
} ')
; sprite.animation.addByPrefix('pressed', '$
{ anims[ID]
} press', 24, false)
; sprite.animation.addByPrefix('confirm', '$
{ anims[ID]
} confirm', 24, false)
; sprite.animation.addByPrefix('confirm-hold', '$
{ anims[ID]
} confirm', 24, false)
; } sprite.playAnim('static')
; } /** * Applies 'this' note style's animations to the specified hold cover. * @param sprite The hold cover to apply the animations to.
*/ function applyAnimsToHoldCover(sprite:HoldCover)
{ switch (this)
{ case 'pixel': sprite.animation.addByPrefix('loop', 'loop', 24)
; case '3d': sprite.animation.addByPrefix('start', 'start', 24, false)
; sprite.animation.addByPrefix('loop', 'loop', 24)
; sprite.animation.addByPrefix('end', 'end', 24, false)
; default: sprite.animation.addByPrefix('start', 'sustain cover pre0', 24, false)
; sprite.animation.addByPrefix('loop', 'sustain cover0', 24)
; // Criticism vowed against having the splashes. sprite.animation.addByPrefix('end', 'sustain cover pre0', 24, false)
; }
} /** * Applies the properties of 'this' note style onto a note object. * @param sprite The note to apply the properties to.
*/ function applyPropertiesToNote(sprite:Note)
{ sprite.scale.set(styleSize, styleSize)
; sprite.updateHitbox()
; sprite.antialiasing = antialiasing
; switch (this)
{ case 'phone', 'phone-alt': sprite.LocalScrollSpeed = 1.08
; default: sprite.LocalScrollSpeed = 1
; }
} /** * Applies the properties of 'this' note style onto a sustain object. * @param sprite The sustain to apply the properties to.
*/ function applyPropertiesToSustain(sprite:SustainNote)
{ sprite.scale.x = styleSize
; sprite.antialiasing = antialiasing
; switch (this)
{ case 'shape': sprite.subdivisions = Std.int(Math.floor(sprite.fullSustainLength / Conductor.instance.stepCrochet))
; }
} /** * Applies the properties of 'this' note style onto a strum object. * @param sprite The strum to apply the properties to.
*/ function applyPropertiesToStrum(sprite:StrumNote)
{ sprite.scale.set(styleSize, styleSize)
; sprite.updateHitbox()
; sprite.antialiasing = antialiasing
; switch (this)
{ default: sprite.animOffsets.set('confirm', [0, 0])
; } sprite.x += noteStyleOffsets.x
; sprite.y += noteStyleOffsets.y
; } /** * Applies the properties of 'this' note style onto a strum object. * @param sprite The hold cover to apply the properties to.
