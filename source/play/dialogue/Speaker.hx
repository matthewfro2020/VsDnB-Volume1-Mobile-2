...
; } /** * A list of dialogue sounds this speaker has. * This is constructed on create for the dialogue to use when switching speakers, allowing for easy preloading.
...
{ // Speaker has dialogue sounds but they haven't been added yet. populateDialogueSounds()
; }
} override function kill():Void
{ clearDialogueSounds()
; super.kill()
; } public function onDestroy(event:ScriptEvent):Void
{ // Remove dialogue sounds. clearDialogueSounds()
...
; } dialogueSounds = []
; } /** * Constructs a dialogue sound from the given asset path. * Scripts can use this if they want to further manipulate the sound such as the volume, etc. * @return GameSound
*/ function constructDialogueSound(path:String):GameSound
{ var sound:GameSound = SoundController.load(Paths.sound(path))
; sound.volume = 0.8
; return sound
; } /** * Switches the speaker's expression to an new one given an id. * @param expressionId The name of the expression.
*/ public function switchToExpression(expressionId:String):Void
{ // Expression doesn't exist. if (!hasExpression(expressionId)) return
...
; // Apply position offsets. this.x += expressionData.offsets[0] ?? 0.0
; this.y += expressionData.offsets[1] ?? 0.0
; if (expressionData.animation != null)
{ // Apply animation offsets IN-CASE there's any provided. this.offset.x += expressionData.animation.offsets[0] ?? 0.0
; this.offset.y += expressionData.animation.offsets[1] ?? 0.0
; }
} /** * Does this speaker have the given expression? * @param name The name of the expression.
*/ public function hasExpression(name:String)
{ return getExpressionData(name) != null
; } /** * Fetches the data of an expression given the name of it. * @param name The name of the expression. * @return A `SpeakerExpressionData` if one is found, else null.
...
{ data.name == name
; } )
; // Return the first entry. if (expressions.length >= 1) return expressions[0]
; // No expression found. return null
