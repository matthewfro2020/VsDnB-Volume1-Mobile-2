...
; /** * Contains a mapping of all of the metadata for the song's variations. * * `String` (Variation) -> `SongMetadata` (The variation's metadata)
*/ var _metadata:Map<String, SongMetadata> = new Map<String, SongMetadata>()
; /** * A list of all of the playable charts for this song. * * `String` (Variation) -> `SongPlayChart` (The variation's SongPlayChart)
...
; _data = fetchData(id)
; if (_data == null)
{ throw 'No Song data was found with an id of $
{ id
} '
; } _metadata = [DEFAULT_VARIATION => _data]
; // Load all of the variations that this song has. for (variation in _data.variations)
...
; } else
{ FlxG.log.warn('There was an error prasing the metadata file for variation: $
{ variation
} ')
; }
} populateMetadataCharts()
; validScore = true
; } /** * Fetches the metadata of this object. * @param id The id of this entry object. * @return The metadata to use for this data structure.
*/ public function fetchData(id:String):SongMetadata
{ return SongRegistry.instance.parseEntryData(id)
; } /** * Destroys this object. * This should ONLY be used when this is being cleared from the registry. Destroying this outside of it can easily cause a crash. * Used to free up memory.
...
{ charts.set(key, null)
; } charts.clear()
; } /** * Returns a string representation of this object. * @return A string used for debugging.
*/ public function toString():String
{ return 'Song(id=$id)'
; } /** * Retrieves a playable chart from a given variation id. * @param variationId The variation to get the chart for. * @return The chart for this variation. Falls back to the default variation if none exist.
*/ public function getChart(?variationId:String):SongPlayChart
{ return charts?.get(variationId) ?? charts?.get(DEFAULT_VARIATION) ?? null
; } /** * Does a playable variation chart exist for the given id? * @param variationId The variation id to check. * @return Whether this song has a variation chart for the given id.
...
; }
} /** * Retrieves a list of all the variation ids available for this song. * @return A `Array<String>`
*/ public function listVariationIds():Array<String>
{ return [for (variation in _metadata.keys()) variation]
; } /** * Validates the given variation to that it's able to be used when retrieving it for files. * @param variation The variation to validate. * @return A validated variation.
...
; } /** * Validates the given variation so it isn't null. * @param variation The variation to validate. * @return A new variation `String`
...
} /** * An object used to store the information for a variation's metadata, and chart. * This is the main structure used for gameplay, and is used to keep track of a different variation's data information, if it exists.
*/ class SongPlayChart
...
; } /** * Gets the instrumental path for this chart. * @return The instrumental asset path.
*/ public function getInstrumentalPath():String
{ return Paths.instPath(song.id, variation)
; } /** * Retrieves the asset path for the vocals of this chart. * @return The vocals asset path.
