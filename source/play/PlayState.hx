...
; /** * Whether the game is currently paused, or not. * Stops all gameplay functionality.
...
; /** * The current chart that's being played. * Contains all the deta information for the song including the metadata, and chart.
...
; /** * A fade timer used to make the vocals fade in after the player misses. * Used to make sure the vocals don't stay quiet when the player misses causing issues like voicelines, opponent vocals, and etc being quiet.;
;
...
; /** * Whether we're currently in some sort of cutscene. * If this is the case, all input should be teased.
*/ public var isInCutscene:Bool
; /** * (Optional) Function called when the song's ready to be started. * Useful for if the song has an opening cutscene, and the countdown should be called later. * The countdown will have to be manually called if this is used.
*/ var startCallback:Void->Void
; /** * (Optional) Function called when the song's complete. * Useful for if the song has dialogue, or an ending cutscene, and it shouldn't end immediately. * State switching is to be done manually if this is used.
...
; /** * The current scrollType being used right now. * Changes the y position of all of the HUD elements based on this type.
...
{ // Mark as hit note.hasBeenHit = true
; // Internal scoring logic playingStrumline.hitNote(note)
; // Trigger player animation if (playingChar != null)
...
; // Show BOTPLAY text botplayTxt.visible = true
; // 1. ALL hittable "tap" notes var possible:Array<Note> = playerStrums.getPossibleNotes()
; // Sort closest → furthest haxe.ds.ArraySort.sort(possible, function(a, b)
{ return Reflect.compare( Math.abs(Conductor.instance.songPosition - a.strumTime), Math.abs(Conductor.instance.songPosition - b.strumTime) )
; } )
; // ================================
// TAP NOTES
...
; // SUSTAIN HEAD (first part) if (note.sustainNote != null)
{ // Hit the head playerStrums.pressKey(note.direction)
; playerStrums.hitNote(note)
; continue
; } // PHONE NOTE if (note.noteStyle == "phone")
...
; // Sustain active window var start = s.strumTime
; var end = s.strumTime + s.fullSustainLength
; if (now >= start && now <= end)
{ // Hold key while inside sustain window playerStrums.pressKey(s.direction)
; if (!s.hasBeenHit)
{ s.hasBeenHit = true
; s.hasMissed = false
; } // Play strum hold animation playerStrums.strums.members[s.direction].holdConfirm()
; } else if (now > end)
{ // Release key when sustain ends playerStrums.releaseKey(s.direction)
; }
} )
...
{ if (!botplay) return
; // Your player strumline: var pl:Strumline = strumlinePlayer
; // adjust if player index differs
// Get all notes that are hittable var possibleNotes:Array<Note> = pl.getPossibleNotes()
; // Sort by closest to hit possibleNotes.sort(function(a, b)
{ return Reflect.compare( Math.abs(Conductor.instance.songPosition - a.strumTime), Math.abs(Conductor.instance.songPosition - b.strumTime) )
; } )
; // ---- HIT TAPS ---- for (note in possibleNotes)
{ if (note == null) continue
; if (note.hasBeenHit) continue
; // Tap note (no sustain) if (note.sustainNote == null)
{ pl.pressKey(note.direction)
; pl.hitNote(note)
; pl.releaseKey(note.direction)
; } else
{ // HIT START OF HOLD if (!note.sustainNote.hasBeenHit)
{ pl.pressKey(note.direction)
; pl.hitNote(note)
; }
}
} // ---- HOLD SUSTAINS ---- pl.forEachHoldNote(function(hold:SustainNote)
{ if (hold == null) return
; var now = Conductor.instance.songPosition
; // Sustain active window if (now >= hold.strumTime && now <= hold.strumTime + hold.fullSustainLength)
{ // keep key held pl.pressKey(hold.direction)
; if (!hold.hasBeenHit)
{ hold.hasBeenHit = true
; hold.hasMissed = false
; }
} else
{ // release when finished pl.releaseKey(hold.direction)
; }
} )
; } function set_scrollType(value:String):String
{ if (dadStrums != null) dadStrums.scrollType = value
; if (playerStrums != null) playerStrums.scrollType = value
; for (i in [missesDisplay, scoreDisplay, accuracyDisplay, timer, healthBar])
{ if (i == null) continue
; var item = cast(i, IHudItem)
; item.scrollType = value
; } if (iconP1 != null) iconP1.y = healthBar.y - (iconP1.height / 2)
; if (iconP2 != null) iconP2.y = healthBar.y - (iconP2.height / 2)
...
; /** * The player's current health. * Once this value reaches zero, the user is at a GameOver and all gameplay logic should stop.
*/ public var health:Float = 1
; /** * The amount of health the player gains when hitting a note. * Separate variable to allow for scripts to control it.
*/ private var healthDrainer:Float = 0.04
; /** * The amount of health the player losses when missing a note. * Separate variable to allow for scripts to control it.
*/ private var healthGainer:Float = 0.023
; /** * The player's current health displayed from the HUD. * This interpolates to the player's current health to help with give nice smoothing effects.
...
; /** * The player's current combo. * Gets reset to 0 whenever the player misses.
...
; /** * The total amount of notes the user has hit. * This is increased based a rating the player gets.
*/ private var totalNotesHit:Float = 0
; /** * The ACTUAL total of amount of notes the player has hit. * Gets updated everytime the accuracy gets updated.
*/ private var totalPlayed:Int = 0
; // CAMERA
// /** * The current zoom the game camera is on. * Interpolates between this, and the current camera zoom value for a smooth lerp effect.
*/ private var defaultCamZoom:Float = 1.05
; /** * The current zoom the HUD camera is on. * Interpolates between this, and the current camera zoom value for a smooth lerp effect.
...
; /** * Whether the game is allowed to do any camera zooming. * Disables the camera zooming for all cameras including the game, and hud camera.
...
; /** * The current time of the icon bop. * Used to help make sure the icon bops are framerate independent.
...
; /** * A list of songs that contain shape notes. * TODO: Remove this?
...
; /** * The current character that the user plays as. * Dependent on whether the user's playing as the opponent, or the player for the song.
*/ private var playingChar(get, never):Character
; function get_playingChar():Character
{ return playerType == OPPONENT ? dad : boyfriend
; } /** * The current current that's against the player. * Dependent on whether the user's playing as the opponent, or the player for the song.
...
; /** * The strumline that's being used by the opponent. * If the current opponent is the `Opponent` this'll be `dadStrums` * Else, it'll be `playerStrums`
...
; } /** * The strumline that's being used by the current player. * If the current player is the `Opponent` this'll be `dadStrums` * Else, it'll be `playerStrums`
...
; // interdimensional var noteLimbo:Note
; var noteLimboFrames:Int
; var pressingKey5Global:Bool
; public var cpuControlled:Bool = false
; /** * Initalizes a new PlayState instance. * @param params The parameters to initalize PlayState with.
*/ public function new(?params:PlayStateParams)
{ super()
; if (params == null)
{ if (lastParams == null) throw 'Tried to initalize PlayState with 0 parameters.'
; else params = lastParams
...
; botplayTxt = new FlxText(healthBar.x + healthBar.width / 2 - 75, healthBar.y + (FlxG.save.data.downscroll ? 100 : -100), 0, "BOTPLAY", 20)
...
; generateSong()
; prepareSong()
; #if mobileC initalizeMobileControls()
; #end super.create()
...
// ============================== if ((isInCutscene && FlxG.keys.justPressed.ESCAPE) || (FlxG.keys.justPressed.ENTER && Countdown.countdownStarted && canPause)) runPause()
; // ==============================
...
; iconP2.updateHitbox()
; // icon positions var iconOffset:Int = 26
; switch (healthBar?.bar?.fillDirection)
{ case LEFT_TO_RIGHT: iconP1.x = (healthBar.x + healthBar.width) - (healthBar.width * (FlxMath.remapToRange(healthBar.percent,0,100,100,0) * 0.01) + iconOffset)
...
; default: iconP1.x = healthBar.x + (healthBar.width * (FlxMath.remapToRange(healthBar.percent,0,100,100,0) * 0.01) - iconOffset)
; iconP2.x = healthBar.x + (healthBar.width * (FlxMath.remapToRange(healthBar.percent,0,100,100,0) * 0.01)) - (iconP2.width - iconOffset)
; } // icon losing/normal switch (healthBar?.bar?.fillDirection)
{ case LEFT_TO_RIGHT: iconP1.changeState(healthBar.percent > 80 ? "losing" : "normal")
; iconP2.changeState(healthBar.percent < 20 ? "losing" : "normal")
; default: iconP1.changeState(healthBar.percent < 20 ? "losing" : "normal")
...
{ var list:Array<Dynamic> = [ [FlxKey.ONE, dad], [FlxKey.TWO, boyfriend], [FlxKey.THREE, gf] ]
; for (i in list)
{ if (FlxG.keys.firstJustPressed() == i[0])
{ if (FlxG.keys.pressed.CONTROL) FlxG.switchState(() -> new AnimationDebug(i[1]))
; if (FlxG.keys.pressed.SHIFT) FlxG.switchState(() -> new CharacterDebug(i[1].id))
; }
}
} // ==============================
// CONDUCTOR UPDATE
...
; if (Conductor.instance.songPosition >= 0 + Conductor.instance.offsets) startSong()
; }
} else
{ Conductor.instance.update( Conductor.instance.songPosition + FlxG.elapsed * 1000, true, false )
; }
...
// CAMERA ZOOMS
// ============================== if (camGameZoom.canWorldZoom) FlxG.camera.zoom = MathUtil.smoothLerp( FlxG.camera.zoom, defaultCamZoom, elapsed, 0.75, 1 / 1000)
; if (camHUDZoom.canWorldZoom) camHUD.zoom = MathUtil.smoothLerp( camHUD.zoom, defaultHUDZoom, elapsed, 0.75, 1 / 1000)
; // ==============================
// GAME OVER
// ============================== if (health <= 0 && !isPlayerDying) gameOver()
; // ==============================
// PHONE NOTES
...
// ============================== if (Preferences.botplay) botplayAutoHit()
; } override function destroy():Void
{ performCleanup()
; super.destroy()
; } /** * Called whenever the Conductor instance reaches a step. * @param step The step reached.
*/ override function stepHit(step:Int):Bool
{ if (!super.stepHit(step)) return false
...
; }
} var needsResync:Bool = (Math.abs(vocals.time - SoundController.music.time) >= 20) || (Math.abs(SoundController.music.time - (Conductor.instance.songPosition - Conductor.instance.offsets)) >= 20)
; if (!startingSong && needsResync && !paused) resyncVocals()
; return true
; } /** * Called whenever the Conductor instance reaches a beat. * @param beat The beat that was reached.
...
; switch (healthBar.bar.fillDirection)
{ case LEFT_TO_RIGHT: iconP1BopSize = new Vector2(Std.int(iconP2.width + (50 * ((2 - funny) + 0.1))), Std.int(iconP2.height - (25 * ((2 - funny) + 0.1))))
; iconP2BopSize = new Vector2(Std.int(iconP1.width + (50 * (funny + 0.1))), Std.int(iconP1.height - (25 * funny)))
; default: iconP1BopSize = new Vector2(Std.int(iconP1.width + (50 * (funny + 0.1))), Std.int(iconP1.height - (25 * funny)))
...
; return true
; } /** * Called whenever the Conductor instance reaches a measure. * @param measure The measure that was reached.
*/ override function measureHit(measure:Int):Bool
{ if (!super.measureHit(measure)) return false
; return true
; } /** * Called whenever the Conductor instance reaches a new time change. * @param event The time change event that was reached.
*/ override function timeChange(event:SongTimeChange)
{ if (!super.timeChange(event)) return false
; for (i in [camGameZoom, camHUDZoom])
{ if (i.timeSignatureAdjust)
{ i.timeSnap = event.numerator
; }
} return true
; } /** * Opens a given substate. Overriden to take into account if the player pauses. * @param SubState The substate to open.
...
; } #if desktop changePresence(PAUSED)
; #end if (Countdown.countdownStarted && !Countdown.finished) Countdown.paused = true
; } super.openSubState(SubState)
; } /** * Closes the currently active substate. Overriden to take into account if the player pauses. * @param SubState The substate to open.
...
; } if (Countdown.countdownStarted && !Countdown.finished) Countdown.paused = false
; TweenUtil.resumeTweens()
; FlxTimer.globalManager.forEach(function(t:FlxTimer)
{ t.active = true
; } )
; paused = false
; #if desktop changePresence(NORMAL(true, false))
; #end
} super.closeSubState()
...
{ if (canPause) runPause()
; } override function dispatchEvent(event:ScriptEvent):Void
{ // Dispatch modules first. super.dispatchEvent(event)
; // Dispatch to the stage. ScriptEventDispatcher.callEvent(this.currentStage, event)
; // Dispatch to all characters. this.currentStage.dispatchToCharacters(event)
; // Dispatch to the song. ScriptEventDispatcher.callEvent(this.currentSong, event)
; // Dispatch to any song modules. SongModuleHandler.callOnModules(event)
; // Dispatch any script events to the dialogue. if (currentDialogue != null)
{ ScriptEventDispatcher.callEvent(this.currentDialogue, event)
; } // Dispatch to the subtitle manager, in the case of any subtitles. if (subtitleManager != null)
...
; } /** * Called whenever the user changes a preferences. * @param preference The id of the preference that was changed. * @param value The new value of the preference.
*/ function onPreferenceChange(preference:String, value:Any):Void
{ switch (preference)
{ case 'downscroll': this.scrollType = value ? 'downscroll' : 'upscroll'
; case 'ghostTapping': this.ghostTapping = value
; case 'hitsounds': if (value)
{ SoundController.cache(Paths.sound('note_click'))
; } case 'minimalUI': Main.fps.visible = value ? false : Preferences.debugUI
...
{ module.initalize()
; } )
; // Dispatch create function to song to further initalize it before gameplay starts. var event = new ScriptEvent(CREATE, false)
...
; // Sets the offsets of the Conductor. Conductor.instance.offsets = Preferences.latencyOffsets
...
{ // Set the camera zoom based off the stage's zoom. defaultCamZoom = currentStage.stageZoom
; // Add the stage into the scene. this.add(currentStage)
; }
} function loadStage(id:String):Stage
{ var stage = StageRegistry.instance.fetchEntry(id)
; if (stage != null)
{ // Revive the stage if it was killed. if (!stage.alive)
{ stage.revive()
; } // Load the stage. stage.load()
; } return stage
...
{ // If this is true it means that the current variation we're on is a custom one. var customVariation:Bool = (this.currentVariation == Song.validateVariation(params.targetVariation) && this.currentVariation != Song.DEFAULT_VARIATION)
...
// This is where they'll be re-positioned, and properly initalized. this.currentStage.addCharacter(gf, GF)
...
; // Cache the opponent and player note skins so they don't cause stutters. Preloader.cacheNoteStyle(dad.skins.get('noteSkin'))
...
{ case 'gf': dad.setPosition(gf.x, gf.y)
; gf.visible = false
...
{ #if mobileC addMobileControls()
; mobileControls.visible = true
; #end
} /** * Initalizes the HUD, and any necessary user
interface .
*/ function initalizeUI():Void
{ createStrums()
; createHudDisplays()
; healthBar = new HealthBar(0,
{ graphic: healthBarOverride != null ? healthBarOverride : Paths.image('ui/bars/healthBar'), opponent: dad, player: boyfriend, parent: this, variable: 'healthLerp', min: 0, max: 2, playerType: this.playerType, scrollType: this.scrollType
} )
...
; add(subtitleManager)
; // Initalize the subtitle manager. ScriptEventDispatcher.callEvent(subtitleManager, new ScriptEvent(CREATE, false))
...
{ dadStrums = new Strumline(
{ isPlayer: false, noteStyle: dad.skins.get('noteSkin'), scrollType: scrollType, showStrums: false
} )
; dadStrums.x = 100
...
{ isPlayer: true, noteStyle: boyfriend.skins.get('noteSkin'), scrollType: scrollType, showStrums: false
} )
...
{ if (!noMiss) noteMiss(note.originalType, note, this.playingChar)
; muteVocals()
; } )
; // Flip the instances of the strumlines so they work properly. if (playerType == OPPONENT)
{ // Change whether you have to play the opponent or players side based on the user's selected type. playingStrumline.isPlayer = true
...
{ name: 'misses', parent: this, trackerVariable: 'misses', startString: '0', scrollType: this.scrollType
} )
; missesDisplay.screenCenter(X)
; accuracyDisplay = new HudDisplay(missesDisplay.x - 150,
{ name: 'accuracy', parent: this, trackerVariable: 'accuracy', startString: '100%', scrollType: scrollType
} )
; accuracyDisplay.textUpdateFunc = function(value:Float)
{ accuracyDisplay.text.text = '$
{ FlxMath.roundDecimal(value, 2)
} %'
; } scoreDisplay = new HudDisplay(missesDisplay.x + 150,
{ name: 'score', parent: this, trackerVariable: 'songScore', startString: '0', scrollType: this.scrollType
} )
; for (i in [scoreDisplay, missesDisplay, accuracyDisplay])
{ i.cameras = [camHUD]
; add(i)
; }
} /** * Starts a new dialogue conversation given an id. * @param id The id of the dialogue to open. * @param onComplete Called when the dialogue has been completed. * @param autoBegin Whether to immediately begin the dialogue. If false, `this.currentDialogue.start()` must be called in order for it to start.
...
; // Begin the dialogue, if false a script will have to call it themselves. if (autoBegin)
{ currentDialogue.start()
; }
}
} /** * Helper function for opening dialogue at the start of a song. * @param id The id of the dialogue box to open.
*/ function beginStartDialogue(id:String, ?onComplete:Void->Void)
{ // Start the countdown if no other callback was provided. onComplete ??= startCountdown
; if (!Preferences.cutscenes)
{ if (onComplete != null) onComplete()
; return
...
; } /** * Handles whenever there's a song with dialogue at the end. * @param id The id of the dialogue to open. * @param finishCallback Called when the dialogue is finished.
...
; // stop the game from trying to generate anymore music and to just cease attempting to play the music in general for (strumLine in [playerStrums, dadStrums])
...
{ case ALTERNATE: focusOnDadGlobal = !focusOnDadGlobal
; ZoomCam(focusOnDadGlobal)
; dispatchEvent(new CameraScriptEvent(CAMERA_MOVE_SECTION, focusOnDadGlobal, false))
; case LOCKED: focusOnDadGlobal = !boyfriend.startsCountdown
; ZoomCam(focusOnDadGlobal)
; }
} )
...
; // Checks if a voices file exists, and if it doesn't no vocals are played. if (Assets.exists(currentChart.getVoicesPath())) vocals = new GameSound(VOICES).load(currentChart.getVoicesPath())
; else vocals = new GameSound(VOICES)
; SoundController.add(vocals)
; generatedMusic = true
; } /** * Prepares to play the song. * Dependent on whether `startCallback` isn't `null`, and the song should start a different way.
...
; } startingSong = false
; camZooming = true
; if (timer != null) FlxTween.tween(timer,
{ alpha: 1
} , 0.5)
; if (!paused)
...
; vocals.play()
; } #if desktop changePresence(NORMAL(true, false))
; #end SoundController.music.onComplete = endSong
; } /** * Handles all necessary inputs. * Responsible for managing controls, and player inputs.
...
{ // Jump notes var lastHitNote:Int = -1
; var lastHitNoteTime:Float = -1
; for (note in possibleNotes)
{ if (controlArray[note.direction % 4])
// further tweaks to the conductor safe zone offset multiplier needed.
{ if (lastHitNoteTime > Conductor.instance.songPosition - Conductor.instance.safeZoneOffset && lastHitNoteTime < Conductor.instance.songPosition + (Conductor.instance.safeZoneOffset * 0.08))
// reduce the past allowed barrier just so notes close together that aren't jacks dont cause missed inputs
{ if ((note.direction % 4) == (lastHitNote % 4))
{ lastHitNoteTime = -999999
; // reset the last hit note time continue
; // the jacks are too close together
}
} if (note.noteStyle == 'shape' && !key5 || note.noteStyle != 'shape' && key5)
{ noteLimbo = note
; noteLimboFrames = 8
; // note limbo, the place where notes that could've been hit go. continue
...
; var lastHitNoteTime:Float = -1
; for (note in possibleNotes)
{ // Wrong mode? (shape mode mismatch → limbo) if ((note.noteStyle == "shape" && !pressingShape) || (note.noteStyle != "shape" && pressingShape))
{ noteLimbo = note
; noteLimboFrames = 8
; continue
; } // Anti-jack logic if (lastHitNoteTime > Conductor.instance.songPosition - Conductor.instance.safeZoneOffset && lastHitNoteTime < Conductor.instance.songPosition + (Conductor.instance.safeZoneOffset * 0.08))
{ if ((note.direction % 4) == (lastHitNote % 4))
{ lastHitNoteTime = -999999
; continue
; }
...
; lastHitNoteTime = note.strumTime
; // Perform a full hit check playingStrumline.hitNote(note)
; // Trigger player animations if (playingChar != null)
{ playingChar.sing(note.direction)
; playingChar.holdTimer = 0
; }
}
...
} /** * Changes gameplay depending on a note's current state (ones that may have been missed, pressed, etc). * Note states are handled and updated accordingly from `Strumline.hx` * @param elapsed The time since the last frame.
*/ function processNotes(elapsed:Float):Void
{ playingStrumline.forEachNote(function(note:Note)
{ if (note.tooLate && !note.handledMissed && !noMiss)
{ note.handledMissed = true
; // Loss health and score based the note's hold note. if (note.sustainNote != null)
...
// Increment the health in accordance to the cap. if (fullHealthGain > SustainNote.HEALTH_GAIN_MAX)
{ var maxHealthMultipler:Float = SustainNote.HEALTH_GAIN_MAX / fullHealthGain
; // Increment the health by the multiplier.
// This makes it so the amount of health gained is actually the max. health += elapsed * maxHealthMultipler * SustainNote.HEALTH_GAIN_PER_SECOND
; } else
{ health += elapsed * SustainNote.HEALTH_GAIN_PER_SECOND
; } songScore += Std.int(elapsed * SustainNote.SCORE_GAIN_PER_SECOND)
; return
; } // Hold note was dropped as player was holding it. if (holdNote.hasMissed && !holdNote.handledMiss && !noMiss)
{ holdNote.handledMiss = true
; // Penalize the player for dropping the hold note before it was completed. if (holdNote.sustainLength > SustainNote.PENALTY_MINIMUM)
...
; muteVocals()
; } else
{ // Hold note is too short to be penalized, so just drop it, and make invisible. holdNote.visible = false
; }
}
} )
; } /** * Zooms the camera to the given character. * @param focusondad Whether to focus on the opponent, or the player.
...
; }
} /** * Moves the camera based on the note direction that was hit. * @param note The direction of the note that was hit. * @param char The character that hit the note.
...
; switch (note)
{ case 0: amount[0] = -followAmount
; amount[1] = 0
; case 1: amount[0] = 0
; amount[1] = followAmount
; case 2: amount[0] = 0
; amount[1] = -followAmount
; case 3: amount[0] = followAmount
; amount[1] = 0
; } char.cameraNoteOffset.set(amount[0], amount[1])
; if (focusOnDadGlobal && char == dad || !focusOnDadGlobal && char == boyfriend) camGame.cameraNoteOffset.copyFrom(char.cameraNoteOffset)
...
; #if desktop changePresence(NORMAL(true, false))
; #end
} /** * Called when the song finishes. * If `endSongCallback` isn't `null`, you'll need to handle state switching through scripts.
...
{ t.active = false
; } )
; // 1 / 1000 chance for Gitaroo Man easter egg if (FlxG.random.bool(0.1))
{ FlxG.switchState(() -> new GitarooPause(params))
; } else
{ openSubState(new PauseSubState())
; }
} /** * Judges a note, and brings a popup score based on the result. * @param strumtime The time to judge. * @param note The note that was hit.
...
; } /** * Handles logic for when the player misses a note. * @param direction The direction of the miss. * @param note The note that was missed.
...
; FlxTween.tween(event.note.strum,
{ alpha: 1
} , 2,
{ ease: FlxEase.expoIn
} )
; } health -= 0.07
; } // Play the miss sound. event.missSound.play()
; misses++
; combo = 0
; songScore -= 100
; updateAccuracy()
; } /** * Called whenever the player presses the key of a note direction when there's no notes nearby. * @param direction The direction pressed. * @param char The character associated with the press.
...
; combo = 0
; songScore -= 100
; } /** * Constructs a default miss sound for the game to play. * @return A `GameSound` miss sound.
...
; return defaultMiss
; } /** * Resets the vocals fade timer, and completely unmutes the vocals. * Called whenever either player sings.
*/ function unmuteVocals():Void
{ FlxTween.cancelTweensOf(vocals)
; vocalsFadeTimer?.cancel()
; vocalsFadeTimer = null
; vocals.volume = 1
; } /** * Completely unmutes the vocals and runs a tween bringing a fade in. * Called when the player misses a note.
*/ function muteVocals():Void
{ FlxTween.cancelTweensOf(vocals)
; vocalsFadeTimer?.cancel()
; vocalsFadeTimer = null
; vocals.volume = 0
; vocalsFadeTimer = FlxTween.tween(vocals,
{ volume: 1
} , 0.5,
{ startDelay: 2.0, onComplete: (t:FlxTween) ->
{ vocalsFadeTimer = null
; }
} )
; } /** * Used to handle whenever the player presses inputs when there's no note in sight. * Also known as, ghost tapping.
...
{ if (controlArray[i])
{ if (!noMiss) ghostNoteMiss(i, playingChar)
; }
}
...
; SoundController.music.stop()
; removeSignals()
; #if desktop changePresence(GAMEOVER)
; #end var event = new ScriptEvent(GAME_OVER, true)
; dispatchEvent(event)
; if (!event.eventCanceled)
{ openSubState(new GameOverSubstate(playingChar.getScreenPosition().x, playingChar.getScreenPosition().y, playingChar))
; }
} /** * Switches the opponent character to the given character id. * @param newChar The character to switch to. * @param position The new position of the character. * @param reposition Whether to reposition the character based on it's offsets. * @param updateColor Whether to update any UI based on this Character.create's color.
...
; }
} /** * Switches the player character to the given character id. * @param newChar The character to switch to. * @param position The new position of the character. * @param reposition Whether to reposition the character based on it's offsets. * @param updateColor Whether to update any UI based on this Character.create's color.
...
; healthBar.updateColors(dad, boyfriend)
; } /** * Switches the player character to the given character id. * @param newChar The character to switch to. * @param position The new position of the character. * @param reposition Whether to reposition the character based on it's offsets.
...
; } /** * Toggles all strumlines visibility, and does a fade effect. * @param invisible Whether they should be invisible, or not.
...
; FlxTween.tween(strumNote,
{ alpha: (invisible ? 0 : 1)
} , 1)
; } )
; }
} /** * Changes the scroll speed of the song based on a given ease. * @param multiplier The speed to multiply based on the song's base speed. * @param time The duration to which the speed at. * @param easeType How the speed of the song should ease to the target. * @param onComplete Called when the tween associated with the speed change is complete.
...
{ ease: easeType, onComplete: function(tween:FlxTween)
{ if (onComplete != null) onComplete()
; }
} )
; }
} /** * Changes the Discord Rich Presence based on a specific type. * @param type The type to change it based off.
*/ #if desktop function changePresence(type:api.Discord.RPCType):Void
{ if (currentChart == null) return
; var icon:String = DiscordClient.getSongIcon(currentSong.id, currentChart.opponent)
; var detailsText:String = PlayStatePlaylist.isStoryMode ? 'Story Mode: Week $
{ PlayStatePlaylist.storyWeek
} ' : 'Freeplay'
...
; switch (type)
{ case NORMAL(stats, time): statsDetails = stats ? statsDetails : ''
...
; case PAUSED: var pausedText = '(PAUSED) $detailsText - $mainDetails'
; DiscordClient.changePresence('$pausedText', statsDetails, icon)
; case GAMEOVER: DiscordClient.changePresence('(GAME OVER) $
{ mainDetails
} ', statsDetails, icon)
; case CUSTOM(details, state, smallImageKey, hasStartTimestamp, endTimestamp, largeImageKey): DiscordClient.changePresence(details, state, smallImageKey, hasStartTimestamp, endTimestamp, largeImageKey)
; }
} #end
/** * Handles the singing for an opponent character. * @param char The opponent character to sing. * @param daNote The note that was hit by the opponent.
*/ function opponentSing(char:Character, daNote:Note):Void
{ if (char == null || daNote == null) return
; var event = new NoteScriptEvent(OPPONENT_NOTE_HIT, daNote, char, 0.0, combo, null)
; dispatchEvent(event)
; // Cancelling this event will make the opponent not sing. if (event.eventCanceled) return
; var altAnim:String = ""
; if (daNote.noteStyle == 'phone-alt')
{ altAnim = '-alt'
; } switch (event.note.noteStyle)
{ case 'phone': char.playAnim('singSmash', true)
; default: char.sing(daNote.direction, false, altAnim)
; } cameraMoveOnNote(daNote.originalType, char)
; unmuteVocals()
; } /** * Handles the singing for a player character. * @param char The character to sing. * @param note The note that was hit by the opponent.
...
; dispatchEvent(event)
; // Cancelling this event will make the player not sing. if (event.eventCanceled) return
; switch (note.noteStyle)
{ default: event.character.sing(event.note.direction)
; case 'phone': var hitAnimation:Bool = event.character.animation.getByName("dodge") != null
...
; combo++
; health += event.healthChange
; // This allows to be able to change how health the player gains from the note. popUpScore(event.note.strumTime, event.note)
...
; } updateAccuracy()
; } /** * Botplay-only version of phone note behavior. * Forces dodge animation + opponent throw.
*/ private function botplayPhoneHit(note:Note):Void
{ if (playingChar == null || note == null) return
; var char = playingChar
; var hasDodge = char.animation.getByName("dodge") != null
; var hasHey = char.animation.getByName("hey") != null
; // BOTPLAY dodge animation char.playAnim(hasDodge ? "dodge" : (hasHey ? "hey" : "singUPmiss"), true)
; // GF cheer if (gf != null) gf.playAnim("cheer", true)
; // Opponent throws phone if (!note.phoneHit && opposingChar != null)
{ var throwAnim = opposingChar.animation.getByName("singThrow") != null ? "singThrow" : "singSmash"
; opposingChar.playAnim(throwAnim, true)
; } // Mark phone hit note.phoneHit = true
; }
}
