...
; /** * The data used for this stage. * Retrieved from the registry.
...
; /** * Stores a list of all props that derive from `BGSprite` * This is used to easily retrieve a prop in case requested.
...
; /** * Create a new Stage object from the given id. * @param id The id to create the stage off of.
*/ public function new(id:String)
{ super()
; this.id = id
; _data = fetchData(id)
; } /** * Overrides the original toString to allow for easier debugging. * @return A string representation of this object.
*/ override function toString():String
{ return 'Stage(id = $id, name = $name)'
; } override function destroy():Void
{ // Make this stage active again so objects render their update() calls. this.active = true
; // Completely clear out the stage when destroyed so it can be used later when registered. kill()
...
; }
} /** * Loads all of the props, and necessary sprites for this stage. * This should be overwritten through a script.
...
{ // Position the character based relative stage position. character.x = characterStageData.position[0]
; character.y = characterStageData.position[1]
; // Offset character's camera position based on the stage camera offsets. character.cameraFocusPoint.x += characterStageData.cameraOffsets[0]
; character.cameraFocusPoint.y += characterStageData.cameraOffsets[1]
; // Set scroll factors. character.scrollFactor.x = characterStageData.scroll[0]
; character.scrollFactor.y = characterStageData.scroll[1]
; } // Override position if given. if (position != null)
{ character.x = position.x
; character.y = position.y
; } if (reposition)
{ // Position the character based on their offsets. character.reposition()
; } character.characterType = type
; this.add(character)
; } /** * Refreshes the stage's sorting layers by comparing each element by it's `zIndex`
*/ function refresh():Void
{ sort()
; } /** * Retrieves a prop from this stage given its name. * @param name The name of the prop. * @return A `BGSprite`
*/ public function getProp(name:String):Null<BGSprite>
{ return namedProps.get(name) ?? null
; } /** * Retrieves a character from this stage given its id. * @param id The id of the character to get. * @return A `Null<Character>`.
*/ public function getCharacter(id:String):Null<Character>
{ return characters.get(id) ?? null
; } /** * Iterates through each non-character sprite in this stage. * @param func The stage to iterate.
...
; func(prop)
; }
} /** * Dispatches the given script event to all characters in this stage. * @param event The script event to call.
...
} /** * Dispatches a script event to the given character through its id. * @param id The id of the character to. * @param event The event to call.
...
; }
} /** * Removes the given sprite out of this stage and brings it to the given stage. * * @param sprite The sprite to move. * @param stage The stage to put the new sprite in. * @param position The position to put the sprite at.
...
; sprite.y = position.y
; }
} /** * Removes the given prop out of this stage and brings it to the given stage. * * @param sprite The prop to move. * @param stage The stage to put the new prop in. * @param position The position to put the sprite at.
...
; default: null
; }
} /** * Retrieves the data for the given stage id. * @param id The stage id to get the data for. * @return A `StageData` for the stage.
*/ public function fetchData(id:String):StageData
{ return StageRegistry.instance.parseEntryDataWithMigration(id)
; } /** * Adjusts the position and other properties of the soon-to-be child of this sprite group. * Private helper to avoid duplicate code in `add()` and `insert()`. * * @param Sprite The sprite or sprite group that is about to be added or inserted into the group.
...
; // _cameras instead of cameras because get_cameras() will not return null if (clipRect != null) clipRectTransform(sprite, clipRect)
; if (Std.isOfType(sprite, Character))
{ // Store the character to their associated list in the stage. var character:Character = cast sprite
; this.characters.set(character.id, character)
; } else if (Std.isOfType(sprite, BGSprite))
{ // Store the character to their associated list in the stage. var prop:BGSprite = cast sprite
; this.namedProps.set(prop.spriteName, prop)
; } else
{ // This sprite is un-identified. Store it in the unnamed props list. this.unnamedProps.push(sprite)
; }
} /** * Removes the specified sprite from the group. * * @param sprite The `FlxSprite` you want to remove. * @param splice Whether the object should be cut from the array entirely or not. * @return The removed sprite.
*/ public override function remove(sprite:FlxSprite, splice = false):FlxSprite
{ if (Std.isOfType(sprite, Character))
{ // Character's being removed, remove it from their list. var character:Character = cast sprite
; var stageData = getCharacterStageData(character.characterType)
; if (character != null)
{ // Remove the camera offsets. character.cameraFocusPoint.x -= stageData?.cameraOffsets[0] ?? 0.0
; character.cameraFocusPoint.y -= stageData?.cameraOffsets[1] ?? 0.0
; } this.characters.remove(character.id)
; } else if (Std.isOfType(sprite, BGSprite))
{ // Prop's being removed, remove it from their list. var prop:BGSprite = cast sprite
; this.namedProps.remove(prop.spriteName)
; } else
{ // Remove the unnamed sprite from their group. this.unnamedProps.remove(sprite)
; } // Safe to remove them from the Stage now. var sprite:FlxSprite = super.remove(sprite, splice)
; // Make sure to refresh the group so everything's ordered properly. refresh()
; return sprite
; } /** * Adds a new `FlxSprite` subclass to the group. * * @param Sprite The sprite or sprite group you want to add to the group. * @return The same object that was passed in.
*/ override function add(sprite:FlxSprite):FlxSprite
{ var event:ScriptEvent = null
; if (Std.isOfType(sprite, Character))
{ // Character's being added, add it to the list. var character:Character = cast sprite
; this.characters.set(character.id, character)
; event = new AddCharacterScriptEvent(character, true)
; } else if (Std.isOfType(sprite, BGSprite))
{ // Sprite being added is a prop, add it to the list. var prop:BGSprite = cast sprite
; this.namedProps.set(prop.spriteName, prop)
; event = new AddPropScriptEvent(prop, true)
; } else
{ // Add the unnamed sprite to their list. this.unnamedProps.push(sprite)
