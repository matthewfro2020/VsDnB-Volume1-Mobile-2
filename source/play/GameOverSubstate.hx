...
// These get set in the death character's script file. reset()
...
; // If a variation exists for this game over variation, retrieve that, else just fallback to the default. var musicData:SongMusicData = SongRegistry.instance.loadMusicDataFile('game-over', hasMusicDataFile ? musicSuffix : '')
; musicSuffix = hasMusicDataFile ? musicSuffix : ''
; Conductor.instance.applyMusicData(musicData)
; // Cache the game over music. var gameOverMusic = Paths.music('gameOver/gameOver$
{ Song.validateVariationPath(musicSuffix)
} ')
; var deathSfx = Paths.sound('death/fnf_loss_sfx' + deathSuffix, 'shared')
; Preloader.cacheSound(Paths.soundPath('gameOver/gameOver$
{ Song.validateVariationPath(musicSuffix)
} -end', 'music/', AssetType.MUSIC))
...
{ super.create()
; #if mobileC addVirtualPad(NONE, A_B)
; #end
...
{ // Remove cached audio to save memory. Preloader.removeCachedSound(Paths.soundPath('gameOver/gameOver$
{ Song.validateVariationPath(musicSuffix)
} ', 'music/', MUSIC))
; Preloader.removeCachedSound(Paths.soundPath('gameOver/gameOver$
{ Song.validateVariationPath(musicSuffix)
} -end', 'music/', MUSIC))
; Preloader.removeCachedSound(Paths.soundPath('death/fnf_loss_sfx' + deathSuffix))
; // Reset GameOver substate properties after this state has been exited. reset()
; super.destroy()
...
; SoundController.play(Paths.music('gameOver/gameOver$
{ Song.validateVariationPath(musicSuffix)
} -end'))
