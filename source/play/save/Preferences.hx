...
*/ public static var defaults(default, null):Map<String, Any> = [ 'downscroll' => false, 'ghostTapping' => true, 'cutscenes' => true, 'flashingLights' => true, 'cameraShaking' => true, 'cameraNoteMovement' => true, 'masterVolume' => 1, 'musicVolume' => 1, 'voicesVolume' => 1, 'sfxVolume' => 1, 'hitsoundsVolume' => 0.7, 'minimalUI' => false, 'debugUI' => false, 'timerType' => 'timeLeft', 'gimmickWarnings' => true, 'hitsounds' => false, 'latencyOffsets' => 0, 'language' => 'en-US', 'vsync' => true, 'fps' => 144, 'borderless' => false, 'darkMode' => false, 'botplay' => false, ]
...
; // Make sure the data isn't broken. if (save.data == null) save.flush()
; load()
...
; } else
{ // So the accessor functions call on every preference updating them. Reflect.setProperty(Preferences, preference, Reflect.getProperty(Preferences, preference))
; }
} // Dispatch a script event for whenever the user's preference changes. onPreferenceChanged.add((preference:String, value:Any) ->
{ // If we're in an event dispatching state (Almost always). var eventHandler:IEventDispatcher = cast FlxG.state
...
; return value
; } static function get_downscroll():Bool return save?.data?.downscroll
...
; } /** * The amount of time, in milliseconds, in which the conductor is offset by. * Used to help account for headphone latency.
...
; save.flush()
; if (vsync)
{ #if !linux var refreshRate:Int = FlxG.stage.window.displayMode.refreshRate
; #else
// For some reason displayMode.refreshRate returns 0 on linux, so we leave this at 144.
// TODO: Find a way to get the linux refresh rate. var refreshRate:Int = 144
; #end
// Set the FPS to just the monitor's refresh rate if Vsync is on. Main.frameRate = refreshRate
...
; if (value)
{ #if !linux var refreshRate:Int = FlxG.stage.window.displayMode.refreshRate
; #else var refreshRate:Int = 144
; #end FlxG.updateFramerate = refreshRate
; FlxG.drawFramerate = refreshRate
...
; if (!borderless)
{ // Needs to be done, or else the border color won't be updated. FlxG.stage.window.borderless = true
