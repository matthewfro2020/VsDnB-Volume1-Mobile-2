...
; sheetsInUse.push(
{ path: path, anims: animations, offsetFile: offsetFile
} )
; } // --------------------------------------------------------
// DANCING
...
{ if (animation == null || animation.getByName(AnimName) == null || (isDanceAnimation(AnimName) && !canDance) || (isSingAnimation(AnimName) && !canSing))
{ return
; } // -------- FIXED INVALID SWITCH BLOCK ---------- if (isDanceAnimation(AnimName.toLowerCase()))
...
; case PLAYER: var note = event.note
...
; for (i in danceTypes)
{ if (i == 'ease' || (i.endsWith('-ease') && (animToDo + '-ease') == i)) return true
; var animToCheck = isDanceAnimation(animToDo) ? "idle" : isSingAnimation(animToDo) ? "pose" : ""
; switch (animToCheck)
