...
; if (Assets.exists(voicesPath)) checkLoadSong(voicesPath)
; checkLibrary("shared")
; var fadeTime = 0.5
...
{ @:privateAccess if (!LimeAssets.libraryPaths.exists(library)) throw "Missing library: " + library
...
; #if debug if (FlxG.keys.justPressed.SPACE) trace('fired: ' + callbacks.getFired() + " unfired:" + callbacks.getUnfired())
; #end
} function onLoad()
{ if (stopMusic && SoundController.music != null) SoundController.music.stop()
...
{ #if NO_PRELOAD_ALL var targetSongId:String = playStateParams.targetSong.id
; var targetVariation:String = playStateParams.targetVariation
; var voicesPath:String = getVocalPath(targetSongId, targetVariation)
; var hasVoices:Bool = Assets.exists(voicesPath)
; var loaded = isSoundLoaded(getSongPath(targetSongId, targetVariation)) && (!hasVoices || isSoundLoaded(voicesPath)) && isLibraryLoaded("shared")
; if (!loaded) return new LoadingState(target, stopMusic)
; #end if (stopMusic && SoundController.music != null) SoundController.music.stop()
; return target
; } #if NO_PRELOAD_ALL static function isSoundLoaded(path:String):Bool
{ return Assets.cache.hasSound(path)
; } static function isLibraryLoaded(library:String):Bool
{ return Assets.getLibrary(library) != null
; } #end override function destroy()
{ super.destroy()
; callbacks = null
