...
; public static var defaultKeybinds:Map<String, Array<FlxKey>> = [ 'left' => [A, LEFT], 'down' => [S, DOWN], 'up' => [W, UP], 'right' => [D, RIGHT], 'accept' => [SPACE, ENTER], 'key5' => [SPACE, SHIFT], 'back' => [BACKSPACE, ESCAPE], 'pause' => [ENTER, ESCAPE], 'reset' => [R, DELETE] ]
...
{ for (controlName => key in customControls) setKeybindsForControl(controlName, key)
; PlayerSettings.controls.setKeyboardScheme(Custom)
