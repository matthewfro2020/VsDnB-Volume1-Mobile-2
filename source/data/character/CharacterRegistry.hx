...
; /** * A list meant for caching the data for each data. * Populated as all entries are being loaded.
...
; // Cache all of the character entry data. for (entry in entryIds)
...
{ characterScriptClasses.size()
} scripted, $
{ entryIds.length - characterScriptClasses.size()
} unscripted)')
; } /** * Retrieves a new Character instance, that's ready to be added. * @param id The id of the character to retrieve. * @return A `Character` instance ready to be added.
...
; // This character is a scripted
class . if (charScriptClass != null && charData != null)
{ char = createScriptedEntry(charScriptClass)
; } else
{ char = createEntry(id)
; } return char
; } /** * Fetches the given character id's data. * @param id The id of the character to get the data for. * @return A `CharacterData`
...
; // Safe enable ignoreUnknownVariables on json2object (works on all versions) try
