...
*/ @:default('') public var variationId:String
...
*/ @:default(true) public var unlocked:Bool = true
; /** * Initalizes a new Player data object. * @param name The name of the playable character. * @param The id of the character. * @param The variation this represents.
*/ public function new(name:String, char:String, variation:String = '')
{ this.name = name
; this.charId = char
; this.variationId = variation
; } /** * Serializes this Player data object into a json string. * @return A PlayerData JSON string.
...
; /** * A zero-indexed integer that represents the page that this player is on. * The user is able to go down through the bottom characters to switch a page, and any character with this page number will show. * * A page is able to have 9 slots.
*/ @:default(0) public var page:Int
; /** * A zero-indexed integer that determines the character portrait's grid based position. * In the case where there' multiple of the same, this'll be incremented (as well as the page) until there's a free space.
