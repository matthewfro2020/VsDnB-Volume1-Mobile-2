...
; } /** * Reads the contents of a metadata file given it's entry id and variation. * @param id The id of the entry. * @param variation The variation of the metadata to load.
*/ public function readMetadataEntryFile(id:String, ?variation:String)
{ var path:String = 'songs/$id/$id'
; var filePath:String = Paths.json('$
{ path
} -metadata$
{ Song.validateVariationPath(variation)
} ')
; var contents:String = Assets.getText(filePath)
; return
{ fileName: filePath, contents: contents
}
; } /** * Retrieves the semantic version number of a metadata entry from it's id. * @param id The id of the entry. * @return The entry's version.
...
; } /** * Reads, and parses a metadata file from a given entry's id. * @param id The id of the entry. * @param variation The variation of the entry to get the metadata for. * @return The metadata file for this entry.
...
; } return parser.value
; } /** * Reads the contents of a chart data file given it's entry id and variation. * @param id The id of the entry. * @param variation The variation of the metadata to load.
...
; } /** * Reads, and parses the chart file from a given entry's id. * @param id The id of the entry. * @param variation The variation of the entry to get the chart for. * @param suffix (Optional) An additional suffix to use when retrieving the chart file. * @return The chart data for this entry.
...
{ printErrors(parser.errors)
; } return parser.value
; } /** * Retrieves the semantic version number of the entry chart data from it's id. * @param id The id of the entry. * @return The entry's version.
...
; return VersionUtil.getVersionFromJSON(entryContents)
; } /** * Checks if a music data file exists for the given path. * * @param id The entry id of the music data file. * @param variation The variation of the music data file. * @return If the game was able to find a music data file, or not.
*/ public function hasMusicDataFile(id:String, ?variation:String):Bool
{ var path:String = 'music/$id'
; var filePath:String = Paths.json('$path$
{ Song.validateVariationPath(variation)
} ')
; return Assets.exists(filePath)
; } /** * Reads the contents of a music data file given it's key, and additional parameters. * * @param id The entry id of the music data file. * @param variation The variation of the music data file.
...
; } /** * Reads, and parses the music data file from the given parameters. * * @param id The id of the entry. * @param variation The variation of the entry to get the chart for. * @return The chart data for this entry.
