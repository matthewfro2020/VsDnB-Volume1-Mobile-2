...
; /** * Replaces null Animation data values with default values. * @param data The animation data to validate. * @return A newly validated animation data.
...
; return data
; } /** * Adds an animation data into an FlxSprite. * @param target The sprite to apply the data into the animation. * @param animation The data used to apply to the sprite.
...
{ sprite.addByIndices( animation.name, animation.prefix, animation.indices, animation.frameRate, animation.loop )
; } else
{ sprite.addByPrefix( animation.name, animation.prefix, animation.frameRate, animation.loop )
; }
} else
{ if (animation.indices != null)
{ target.animation.addByIndices( animation.name, animation.prefix, animation.indices, '', animation.frameRate, animation.loop, animation.flip[0], animation.flip[1] )
; } else
...
} /** * Adds an animation data into an FlxSprite. * @param target The sprite to apply the animatiosn to. * @param animations The animations to add onto the sprite.
*/ public static function addAnimationsToSprite(target:FlxSprite, animations:Array<AnimationData>):Void
{ for (animation in animations)
{ if (animation != null) addToSprite(target, animation)
; }
}
}
