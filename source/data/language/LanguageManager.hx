...
{ /** * The parsed list of all of the language data. * Contains the id of the language key, and the text for it.
...
; /** * Initalizes the manager. * Loads, and caches all of the locale files to be used.
...
; } /** * Retrieves all languages objects. * @return A list of all of the languages
...
{ name: splitInfo[0], id: splitInfo[1], color: FlxColor.fromString(splitInfo[2])
} languages.push(languageData)
; } return languages
; } /** * Retrieves a language object from an id. * @param id The id of the language. * @return A Language object.
...
; } /** * Retrieves a string value for the given id of a locale list. * @param id The id of the language key. * @param list Optional, the language list to use for retrieving the string. * @return The value of the language key.
...
{ return id
; }
} /** * Parses a locale file, and gives out a new formatted language list. * @param path The asset path for the file. * @return A new `LanguageList`
...
; FlxG.stage.window.alert('There was an error while parsing the language. $
{ message
} ', 'Language Parsing Error')
; FlxG.stage.window.close()
; }
} return list
; } /** * Builds an error message to use for if there's a parsing error for a language. * @param file The file being parsed. * @param e The error message. * @return An error message.
*/ static function buildErrorMessage(file:String, e:Exception):String
{ var message:String = 'There was an error while parsing the language file for $
