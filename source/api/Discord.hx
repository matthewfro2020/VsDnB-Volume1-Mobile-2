...
; /** * Whether the game running right now is a dev build, and isn't supposed to be public. * Changes the rich presence to make sure nothing is leaked through it.
...
*/
// hides this field from scripts and reflection in general @:unreflective private static var __thread:Thread
...
; } )
; } /** * Shuts down the RPC client. * Dynamic function so it can easily be changed in-case you want custom functionality.
*/ public dynamic static function shutdown()
{ isInitialized = false
; Discord.Shutdown()
; } /** * Called when the RPC client is ready to be used. * @param request Information about the client.
...
; if (discriminator != '0')
// Old discriminators message += '($user#$discriminator)'
; else
// New Discord IDs/Discriminator system message += '($user)'
; trace(message)
; changePresence()
; } /** * Called when the RPC client reaches an error. * @param errorCode The code for the error. * @param message The error message.
*/ private static function onError(errorCode:Int, message:cpp.ConstCharStar):Void
{ trace('Discord: Error ($errorCode: $
{ cast (message, String)
} )')
; } /** * Called when the client disconnects. * @param errorCode The code for the error. * @param message The error message relating to the disconnect.
*/ private static function onDisconnected(errorCode:Int, message:cpp.ConstCharStar):Void
{ trace('Discord: Disconnected ($errorCode: $
{ cast (message, String)
} )')
...
{ while (true)
{ if (isInitialized)
{ #if DISCORD_DISABLE_IO_THREAD Discord.UpdateConnection()
; #end Discord.RunCallbacks()
; } // Wait 1 second until the next loop... Sys.sleep(1.0)
; }
} )
; } isInitialized = true
; } /** * Changes, and displays the rich presence shown on Discord. * @param details The details for the presence. * @param state The current state of the presence. * @param smallImageKey The image that displays on the bottom-right of the rich presence. * @param hasStartTimestamp Whether the RPC should be given a start timestamp of the current time. * @param endTimestamp The ending of the timestamp. Gets added onto the start timestamp. * @param largeImageKey The large image that's displayed in the rich presence.
*/ public static function changePresence(details:String = 'In the Menus', ?state:String, ?smallImageKey:String, ?hasStartTimestamp:Bool, ?endTimestamp:Float, largeImageKey:String = 'icon_logo')
...
; // Obtained times are in milliseconds so they are divided so Discord can use it presence.startTimestamp = Std.int(startTimestamp / 1000)
...
; updatePresence()
; } return newID
; } /** * Gets the icon to be used for the rich presence based on the given song, and character. * @param song The song to check. * @param char The character to check * @return The icon that should be used in the rich presence.
...
{ case 'splitathon': iconRPC = 'the-duo'
; case 'backseat': iconRPC = 'the-duo2'
; default: iconRPC
; } return iconRPC
; }
