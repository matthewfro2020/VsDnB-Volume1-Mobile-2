...
{ /** It creates an instance of Error from any value. If `err` is already an instance of `Error`, it is returned and nothing is created. *
*/ public static function fromDynamic(err:Dynamic, ?pos:PosInfos):Error
{ if (Std.isOfType(err, Error)) return cast err
...
; public function getPosition() return pos.className + "." + pos.methodName + "() at " + pos.lineNumber
; public function stackToString() return CallStack.toString(stackItems)
; }
