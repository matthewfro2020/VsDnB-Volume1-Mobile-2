...
; /** Generates a URL instance from a `String`. It throws an exception if the string cannot be parsed to a valid URL. The URL can be either relative or absolute.
*/ @:from public static function fromString(s:String):Url return parse(s, true)
...
{ protocol: pattern.matched(4), slashes: pattern.matched(5) == "
// ", auth: pattern.matched(7), hostName: pattern.matched(11), port: null == port ? null : Std.parseInt(port), pathName: pattern.matched(13), queryString: null, search: null, hash: pattern.matched(17)
}
...
*/ @:op(A == B) public static function equals(self:Url, that:Url):Bool return self.equalsTo(that)
; public function equalsTo(that:Url):Bool return this.protocol == that.protocol && this.slashes == that.slashes && this.auth == that.auth && this.hostName == that.hostName && this.port == that.port && this.pathName == that.pathName && this.queryString.equals(that.queryString) && this.search == that.search && this.hash == that.hash
; @:op(A / B) public function concatString(that:String):Url
{ var copy = clone()
; if (pathName.isEmpty())
{ if (!that.startsWith("/")) that = "/" + that
; copy.pathName = that
; } else
{ if (that.startsWith("/")) that = that.substring(1)
; if (pathName.endsWith("/")) copy.pathName = copy.pathName + that
; else copy.pathName = copy.pathName + "/" + that
; } return copy
; } @:to public function toString() return if (isAbsolute) '$
...
{ return
{ protocol: protocol, slashes: slashes, auth: auth, hostName: hostName, port: port, pathName: pathName, queryString: queryString.clone(), search: search, hash: hash
}
; } public function ensureQueryString()
{ if (this.queryString != null) return this.queryString
; else return queryString = new Map()
...
{ if (null != value) this.search = null
; return this.queryString = value
; } function get_search():String return null != this.search && "" != this.search ? this.search : this.queryString
...
{ // 'http:
// user:pass@host.com:8080/p/a/t/h?query=string#hash' protocol:String, slashes:Bool, auth:String, hostName:String, port:Null<Int>, pathName:String, queryString:QueryString, search:String,
// for unparsable query string hash:String
}
