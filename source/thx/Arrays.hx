...
; return array
; } /** Arrays.applyIndexes takes an `array` and returns a copy of it with its elements rearranged according to `indexes`. If the `indexes` array does not contain continuous values, you may want to set `incrementDuplicates` to `true`. var result = Arrays.applyIndexes(["B", "C", "A"], [1, 2, 0])
; trace(result)
; // output ["A", "B", "C"] *
*/ public static function applyIndexes<T>(array:ReadonlyArray<T>, indexes:Array<Int>, ?incrementDuplicates = false):Array<T>
{ if (indexes.length != array.length) throw new thx.Error('`Arrays.applyIndexes` can only be applied to two arrays with the same length')
; var result = []
; if (incrementDuplicates)
{ var usedIndexes = thx.Set.createInt()
; for (i in 0...array.length)
{ var index = indexes[i]
; while (usedIndexes.exists(index)) index++
...
*/ public static function monoid<A>():Monoid<Array<A>> return
{ zero: [], append: function(a:Array<A>, b:Array<A>) return a.concat(b)
}
; /** Finds the first occurrance of `element` and returns all the elements after it. *
*/ inline public static function after<T>(array:ReadonlyArray<T>, element:T) return array.slice(array.indexOf(element) + 1)
; /** Safe indexed access to array elements. Deprecated in favor of `getOption`. *
*/ @:deprecated("atIndex is deprecated, use getOption instead") public static function atIndex<T>(array:ReadonlyArray<T>, i:Int):Option<T> return if (i >= 0 && i < array.length) Some(array[i]) else None
; /** Safe indexed access to array elements. Null values within `array` will also return `None` instead of `Some(null)`. *
*/ public static function getOption<T>(array:ReadonlyArray<T>, i:Int):Option<T> return Options.ofValue(array[i])
...
{ for (i in 0...arr.length) if (!predicate(arr[i])) return false
; return true
...
{ for (i in 0...arr.length) if (predicate(arr[i])) return true
; return false
; } /** Creates an array of elements from the specified indexes. *
*/ public static function at<T>(arr:ReadonlyArray<T>, indexes:ReadonlyArray<Int>):Array<T> return indexes.map(function(i) return arr[i])
; /** Finds the first occurrance of `element` and returns all the elements before it. *
*/ inline public static function before<T>(array:ReadonlyArray<T>, element:T) return array.slice(0, array.indexOf(element))
...
; var count = 0
; for (pair in zip(self, other)) if (equality(pair._0, pair._1)) count++
; else break
; return self.slice(0, count)
; } /** Filters out all null elements in the array *
*/ @:deprecated("Arrays.compact is deprecated, use Arrays.filterNull instead.") public static function compact<T>(arr:ReadonlyArray<Null<T>>):Array<T>
{ #if cs var result:Array<T> = []
; for (element in arr)
{ if (null != element) result.push(element)
; } return result
; #else return arr.filter(function(v:Null<T>) return null != v)
; #end
...
*/ @:generic public static function count<T>(arr:ReadonlyArray<T>):Map<T, Int>
...
; } else
{ for (i in 0...array.length) if (eq(array[i], element)) return true
; return false
; }
...
{ for (i in 0...array.length) if (eq(array[i], element)) return true
; return false
; } /** Returns `true` if all items in `elements` are found in the array and they are a strict match or matched by the provided `eq` function. An optional equality function can be passed as the last argument. If not provided, strict equality is adopted. *
*/ public static function containsAllExact<T>(array:Array<T>, elements:Iterable<T>, ?eq:T->T->Bool):Bool
{ for (el in elements)
{ if (!containsExact(array, el, eq)) return false
; } return true
; } /** Returns `true` if all items in `elements` are found in the array. An optional equality function can be passed as the last argument. If not provided, strict equality is adopted. *
*/ public static function containsAllEq<T, PT>(array:Array<T>, elements:Iterable<PT>, eq:T->PT->Bool):Bool
{ for (el in elements)
{ if (!containsEq(array, el, eq)) return false
; } return true
; } /** Returns `true` if any element in `elements` is found in the array and it is a strict match or matched by the provided `eq` function. An optional equality function can be passed as the last argument. If not provided, strict equality is adopted. *
*/ public static function containsAnyExact<T>(array:ReadonlyArray<T>, elements:Iterable<T>, ?eq:T->T->Bool):Bool
{ for (el in elements)
{ if (containsExact(array, el, eq)) return true
; } return false
; } /** Returns `true` if any element in `elements` is found in the array. An optional equality function can be passed as the last argument. If not provided, strict equality is adopted. *
*/ public static function containsAnyEq<T, PT>(array:ReadonlyArray<T>, elements:Iterable<PT>, eq:T->PT->Bool):Bool
{ for (el in elements)
{ if (containsEq(array, el, eq)) return true
; } return false
...
; for (i in 0...length) arr[i] = fillWith
; return arr
; } /** Creates an `Array<T>` containing the given item *
*/ public static function fromItem<T>(t:T):Array<T> return [t]
; /** It returns the cross product between two arrays. ```haxe var r = [1,2,3].cross([4,5,6])
; trace(r)
; // [[1,4],[1,5],[1,6],[2,4],[2,5],[2,6],[3,4],[3,5],[3,6]] ``` *
*/ public static function cross<T>(a:ReadonlyArray<T>, b:ReadonlyArray<T>)
{ var r = []
; for (va in a) for (vb in b) r.push([va, vb])
; return r
; } /** It produces the cross product of each array element. ```haxe var r = [[1,2],[3,4],[5,6]].crossMulti()
; trace(r)
; // [[1,3,5],[2,3,5],[1,4,5],[2,4,5],[1,3,6],[2,3,6],[1,4,6],[2,4,6]] ``` *
*/ public static function crossMulti<T>(array:ReadonlyArray<ReadonlyArray<T>>)
{ var acopy = array.copy(), result = acopy.shift().map(function(v) return [v])
...
; if (null == predicate) predicate = Functions.equality
; for (v in array)
...
; } return result
; } /** It allows to iterate an array pairing each element with every other element in the array. The iteration ends as soon as the `callback` returns `false`. *
*/ public static function eachPair<TIn, TOut>(array:ReadonlyArray<TIn>, callback:TIn->TIn->Bool) for (i in 0...array.length) for (j in i...array.length) if (!callback(array[i], array[j])) return
; /** It compares the lengths and elements of two given arrays and returns `true` if all elements match. An optional equality function can be passed as the last argument. If not provided, strict equality is adopted. *
*/ public static function equals<T, PT>(a:ReadonlyArray<T>, b:ReadonlyArray<PT>, ?equality:T->PT->Bool)
{ if (a == null || b == null || a.length != b.length) return false
; if (null == equality) equality = cast F.equality
; for (i in 0...a.length) if (!equality(a[i], b[i])) return false
; return true
...
{ var arr:Array<T> = []
; for (v in a) if (null != v) arr.push(v)
; return arr
; } /** Filters out all `None` values from an array and extracts `Some(value)` to `value`. *
*/ public static function filterOption<T>(a:ReadonlyArray<Option<T>>):Array<T> return reduce(a, function(acc:Array<T>, maybeV)
{ switch maybeV
...
{ for (element in array) if (predicate(element)) return element
; return null
...
{ for (i in 0...array.length) if (predicate(array[i], i)) return array[i]
; return null
...
{ for (i in 0...array.length) if (predicate(array[i], i)) return Some(array[i])
; return None
; } /** It returns the first element of the array that matches the predicate function. If none is found it returns null. *
*/ public static function findOption<T>(array:ReadonlyArray<T>, predicate:T->Bool):Option<T>
{ for (element in array) if (predicate(element)) return Some(element)
; return None
...
{ for (i in 0...array.length) if (predicate(array[i])) return i
; return -1
...
*/ inline public static function first<T>(array:ReadonlyArray<T>):Null<T> return array[0]
...
; trace(chars)
; // ['H','e','l','l','o','W','o','r','l','d'] ``` *
*/ inline public static function flatMap<TIn, TOut>(array:ReadonlyArray<TIn>, callback:TIn->Array<TOut>):Array<TOut> return flatten(array.map(callback))
; /** It takes an array of arrays and 'flattens' it into an array. ```haxe var arr = [[1,2,3],[4,5,6],[7,8,9]]
; trace(arr)
; // [1,2,3,4,5,6,7,8,9] ``` *
*/ #if js inline #end public static function flatten<T>(array:ReadonlyArray<Array<T>>):Array<T>#if js return js.Syntax.code('Array.prototype.concat.apply')([], array)
; #else return reduce(array, function(acc:Array<T>, element) return acc.concat(element), [])
; #end
/** Finds the first occurrance of `element` and returns all the elements from that point on. *
*/ inline public static function from<T>(array:ReadonlyArray<T>, element:T) return array.slice(array.indexOf(element))
; /** Returns a Map of arrays. Each value in the array is passed to `resolver` that returns a key to use to group such element. This method is tagged with `@:generic` and needs a compatible type to be used (ex: no anonymous objects). In case you have to use a type that is not supported by `@:generic`, please use `groupByAppend`. *
*/ @:generic public static function groupBy<TKey, TValue>(arr:ReadonlyArray<TValue>, resolver:TValue->TKey):Map<TKey, Array<TValue>>
...
*/ @:generic public static function groupByIndex<A, K>(arr:ReadonlyArray<A>, groupKey:Int->K):Map<K, Array<A>>
...
*/ inline public static function hasElements<T>(array:ReadonlyArray<T>):Bool return null != array && array.length > 0
; /** It returns the first element of the array or null if the array is empty. Same as `first`. *
*/ inline public static function head<T>(array:ReadonlyArray<T>):Null<T> return array[0]
; /** `ifEmpty` returns `array` if it is neither `null` or empty, otherwise it returns `alt` *
*/ public static inline function ifEmpty<T>(array:Array<T>, alt:Array<T>):Array<T> return null != array && 0 != array.length ? array : alt
; /** Get all the elements from `array` except for the last one. *
*/ inline public static function initial<T>(array:ReadonlyArray<T>):Array<T> return array.slice(0, array.length - 1)
; /** Creates a new array that alternates the values in `array` with `value`. *
*/ public static function intersperse<T>(array:ReadonlyArray<T>, value:T):Array<T> return reducei(array, function(acc, v, i)
{ acc[i * 2] = v
; return acc
...
*/ inline public static function isEmpty<T>(array:ReadonlyArray<T>):Bool return null == array || array.length == 0
; /** It returns the last element of the array or null if the array is empty. *
*/ inline public static function last<T>(array:ReadonlyArray<T>):Null<T> return array[array.length - 1]
; /** It returns an option of the last element, `None` if the array is empty. *
*/ inline public static function lastOption<T>(array:ReadonlyArray<T>):Option<T> return Options.ofValue(last(array))
; /** Static wrapper for `Array` `map` function. *
*/ #if js inline #end public static function map<TIn, TOut>(array:ReadonlyArray<TIn>, callback:TIn->TOut):Array<TOut>
{ var r = []
; for (i in 0...array.length) r.push(callback(array[i]))
; return r
; } /** Same as `Array.map` but it adds a second argument to the `callback` function with the current index value. *
*/ #if js inline #end public static function mapi<TIn, TOut>(array:ReadonlyArray<TIn>, callback:TIn->Int->TOut):Array<TOut>
{ var r = []
; for (i in 0...array.length) r.push(callback(array[i], i))
; return r
...
*/ public static function pull<T, PT>(array:Array<T>, toRemove:ReadonlyArray<PT>, ?equality:T->PT->Bool) for (element in toRemove) removeAll(array, element, equality)
...
; return array
; } /** Given an array of values, it returns an array of indexes permutated applying the function `compare`. By default `rank` will return continuous values. If you know that your set does not contain duplicates you might want to turn off that feature by setting `incrementDuplicates` to `false`. ``` var arr = ["C","A","B"]
...
{ for (v in array) initial = f(initial, v)
; return initial
; } /** * Alias for reduce that puts the arguments in the proper order.
*/ public static inline function foldLeft<A, B>(array:ReadonlyArray<A>, init:B, f:B->A->B):B return reduce(array, f, init)
...
*/ public static function foldMap<A, B>(array:ReadonlyArray<A>, f:A->B, m:Monoid<B>):B return foldLeft(array.map(f), m.zero, m.append)
; /** * Reduce with a monoid
*/ public static function fold<A>(array:ReadonlyArray<A>, m:Monoid<A>):A return foldMap(array, Functions.identity, m)
; /** * Safely convert to a non-empty list.
*/ public static function nel<A>(array:ReadonlyArray<A>):Option<Nel<A>> return Nel.fromArray(array)
; /** * Reduce with a semigroup, returning None if the array is empty.
*/ public static function foldS<A>(array:ReadonlyArray<A>, s:Semigroup<A>):Option<A> return nel(array).map(function(x) return x.fold(s))
; /** Resizes an array of `T` to an arbitrary length by adding more elements to its end or by removing extra elements. Note that the function changes the passed array and doesn't create a copy. *
*/ public static function resize<T>(array:Array<T>, length:Int, fill:T)
{ while (array.length < length) array.push(fill)
; array.splice(length, array.length - length)
; return array
; } /** Copies and resizes an array of `T` to an arbitrary length by adding more elements to its end or by removing extra elements. Note that the function creates and returns a copy of the passed array. *
...
{ for (i in 0...array.length) initial = f(initial, array[i], i)
; return initial
...
{ if (null == equality) equality = cast Functions.equality
; var i = array.length
; while (--i >= 0) if (equality(array[i], element)) array.splice(i, 1)
; } /** Returns all but the first element of the array *
*/ inline public static function rest<T>(array:ReadonlyArray<T>):Array<T> return array.slice(1)
...
; for (i in 0...n) result.push(copy.splice(Std.random(copy.length), 1)[0])
; return result
; } /** Returns one element at random from the array or null if the array is empty. *
*/ inline public static function sampleOne<T>(array:ReadonlyArray<T>):Null<T> return array[Std.random(array.length)]
...
; while (Arrays.last(res).length < len) Arrays.last(res).push(pad)
; return res
; } /** It returns the elements of the array after the first. *
*/ inline public static function tail<T>(array:ReadonlyArray<T>):Array<T> return array.slice(1)
; /** Returns the first `n` elements from the array. *
*/ inline public static function take<T>(arr:ReadonlyArray<T>, n:Int):Array<T> return arr.slice(0, n)
; /** Returns the last `n` elements from the array. *
*/ inline public static function takeLast<T>(arr:ReadonlyArray<T>, n:Int):Array<T> return arr.slice(arr.length - n)
...
*/ public static function traverseOption<T, U>(arr:ReadonlyArray<T>, f:T->Option<U>):Option<Array<U>> return reduce(arr, function(acc:Option<Array<U>>, t:T)
{ return f(t).ap(acc.map(function(ux:Array<U>) return function(u:U)
{ ux.push(u)
; return ux
; } ))
; } , Some([]))
; /** Traverse the array with a function that may return values wrapped in Either. If any result is in Left, the first such value is returned
; if all results are in Right, then the array of those results is returned in Right. If you want to instead collect errors rather than fail on the first error, see traverseValidation. *
*/ public static function traverseEither<E, T, U>(arr:ReadonlyArray<T>, f:T->Either<E, U>):Either<E, Array<U>> return reduce(arr, function(acc:Either<E, Array<U>>, t:T)
{ return f(t).ap(acc.map(function(ux:Array<U>) return function(u:U)
{ ux.push(u)
; return ux
; } ))
...
*/ public static function traverseValidation<E, T, U>(arr:ReadonlyArray<T>, f:T->Validation<E, U>, s:Semigroup<E>):Validation<E, Array<U>> return reduce(arr, function(acc:Validation<E, Array<U>>, t:T)
...
; } , Validation.success([]))
; /** Traverse the array with a function that may return values wrapped in Validation. If any of the values are Failures, return a Failure that accumulates all errors from the failed values, otherwise return the array of mapped values in a Success. *
*/ public static function traverseValidationIndexed<E, T, U>(arr:ReadonlyArray<T>, f:T->Int->Validation<E, U>, s:Semigroup<E>):Validation<E, Array<U>> return reducei(arr, function(acc:Validation<E, Array<U>>, t:T, i:Int)
...
; for (i in 0...length) array.push(new Tuple2(array1[i], array2[i]))
; return array
...
{ var length = ArrayInts.min([array1.length, array2.length, array3.length]), array = []
; for (i in 0...length) array.push(new Tuple3(array1[i], array2[i], array3[i]))
; return array
...
{ var length = ArrayInts.min([array1.length, array2.length, array3.length, array4.length]), array = []
; for (i in 0...length) array.push(new Tuple4(array1[i], array2[i], array3[i], array4[i]))
; return array
...
{ var length = ArrayInts.min([array1.length, array2.length, array3.length, array4.length, array5.length]), array = []
; for (i in 0...length) array.push(new Tuple5(array1[i], array2[i], array3[i], array4[i], array5[i]))
; return array
...
*/ public static function zip2Ap<A, B, C>(f:A->B->C, ax:ReadonlyArray<A>, bx:ReadonlyArray<B>):Array<C> return zipAp(bx, ax.map(Functions2.curry(f)))
; /** * Zip three arrays by applying the provided function to the aligned members.
*/ public static function zip3Ap<A, B, C, D>(f:A->B->C->D, ax:ReadonlyArray<A>, bx:ReadonlyArray<B>, cx:ReadonlyArray<C>):Array<D> return zipAp(cx, zip2Ap(Functions3.curry(f), ax, bx))
; /** * Zip four arrays by applying the provided function to the aligned members.
*/ public static function zip4Ap<A, B, C, D, E>(f:A->B->C->D->E, ax:ReadonlyArray<A>, bx:ReadonlyArray<B>, cx:ReadonlyArray<C>, dx:ReadonlyArray<D>):Array<E> return zipAp(dx, zip3Ap(Functions4.curry(f), ax, bx, cx))
; /** * Zip five arrays by applying the provided function to the aligned members.
*/ public static function zip5Ap<A, B, C, D, E, F>(f:A->B->C->D->E->F, ax:ReadonlyArray<A>, bx:ReadonlyArray<B>, cx:ReadonlyArray<C>, dx:ReadonlyArray<D>, ex:ReadonlyArray<E>):Array<F> return zipAp(ex, zip4Ap(Functions5.curry(f), ax, bx, cx, dx))
; /** Returns a copy of the array with the new element added to the beginning. *
*/ inline public static function withPrepend<T>(arr:ReadonlyArray<T>, el:T):ReadonlyArray<T> return [el].concat(arr.unsafe())
; /** Returns a copy of the array with the new element added to the end. *
*/ inline public static function with<T>(arr:ReadonlyArray<T>, el:T):ReadonlyArray<T> return arr.concat([el])
...
*/ public static function withSlice<T>(arr:ReadonlyArray<T>, other:ReadonlyArray<T>, start:Int, ?length:Int = 0):ReadonlyArray<T> return arr.slice(0, start).concat(other.unsafe()).concat(arr.slice(start + length))
; /** Returns a copy of the array with the new element inserted at position `pos`. *
*/ public static function withInsert<T>(arr:ReadonlyArray<T>, el:T, pos:Int):ReadonlyArray<T> return arr.slice(0, pos).concat([el]).concat(arr.slice(pos))
; /** Finds the min element of the array given the specified ordering. *
*/ public static function maxBy<A>(arr:ReadonlyArray<A>, ord:Ord<A>):Option<A> return arr.length == 0 ? None : Some(reduce(arr, ord.max, arr[0]))
; /** Finds the min element of the array given the specified ordering. *
*/ public static function minBy<A>(arr:ReadonlyArray<A>, ord:Ord<A>):Option<A> return arr.length == 0 ? None : Some(reduce(arr, ord.min, arr[0]))
...
class for `Array<Float>`.
*
*/ class ArrayFloats
{ /** Finds the average of all the elements in the array. It returns `NaN` if the array is empty. *
...
// the cast is required to compile safely to C# return cast arr.filter(function(v) return null != v && Math.isFinite(v))
; /** Finds the max float element in the array. *
*/ public static function max(arr:ReadonlyArray<Float>):Null<Float> return Arrays.maxBy(arr, Floats.order).get()
; /** Finds the min float element in the array. *
*/ public static function min(arr:ReadonlyArray<Float>):Null<Float> return Arrays.minBy(arr, Floats.order).get()
; /** Resizes an array of `Float` to an arbitrary length by adding more elements (default is `0.0`) to its end or by removing extra elements. Note that the function changes the passed array and doesn't create a copy. *
*/ public static function resize(array:Array<Float>, length:Int, fill:Float = 0.0)
{ while (array.length < length) array.push(fill)
; array.splice(length, array.length - length)
; return array
; } /** Copies and resizes an array of `Float` to an arbitrary length by adding more elements (default is `0.0`) to its end or by removing extra elements. Note that the function creates and returns a copy of the passed array. *
...
; var mean = average(array), variance = Arrays.reduce(array, function(acc, val)
{ return acc + Math.pow(val - mean, 2)
; } , 0) / (array.length - 1)
; return Math.sqrt(variance)
; } /** Finds the sum of all the elements in the array. *
*/ public static function sum(arr:ReadonlyArray<Float>):Null<Float> return Arrays.reduce(arr, function(tot, v) return tot + v, 0.0)
; } /** Helper
class for `Array<Int>`.
*
*/ class ArrayInts
{ /** Finds the average of all the elements in the array. *
*/ public static function average(arr:ReadonlyArray<Int>):Null<Float> return sum(arr) / arr.length
; /** Finds the max int element in the array. *
*/ public static function max(arr:ReadonlyArray<Int>):Null<Int> return Arrays.maxBy(arr, Ints.order).get()
; /** Finds the min int element in the array. *
*/ public static function min(arr:ReadonlyArray<Int>):Null<Int> return Arrays.minBy(arr, Ints.order).get()
; /** Resizes an array of `Int` to an arbitrary length by adding more elements (default is `0`) to its end or by removing extra elements. Note that the function changes the passed array and doesn't create a copy. *
*/ public static function resize(array:Array<Int>, length:Int, fill:Int = 0)
{ while (array.length < length) array.push(fill)
; array.splice(length, array.length - length)
; return array
; } /** Copies and resizes an array of `Int` to an arbitrary length by adding more elements (default is `0`) to its end or by removing extra elements. Note that the function creates and returns a copy of the passed array. *
*/ public static function resized(array:Array<Int>, length:Int, fill:Int = 0)
{ array = array.copy()
; return resize(array, length, fill)
; } /** Finds the sum of all the elements in the array. *
*/ public static function sum(arr:ReadonlyArray<Int>):Null<Int> return Arrays.reduce(arr, function(tot, v) return tot + v, 0)
; } /** Helper
class for `Array<String>`.
*
*/ class ArrayStrings
{ /** Filters out all null or empty strings in the array *
*/ public static function compact(arr:ReadonlyArray<String>):Array<String> return arr.filter(function(v) return !Strings.isEmpty(v))
; /** Finds the max string element in the array. *
*/ public static function max(arr:ReadonlyArray<String>):Null<String> return Arrays.maxBy(arr, Strings.order).getOrElse(null)
; /** Finds the min string element in the array. *
*/ public static function min(arr:ReadonlyArray<String>):Null<String> return Arrays.minBy(arr, Strings.order).getOrElse(null)
; }
