...
*
*/ class Types
{ /** `isAnonymousObject` returns true if `v` is an object and it is not an instance of any custom
class . *
*/ public static inline function isAnonymousObject(v:Dynamic):Bool return Reflect.isObject(v) && null == Type.getClass(v)
; /** Returns `true` if the passed value is an anonymous object or
class instance but it is not any of the primitive types. *
*/ public static function isObject(v:Dynamic):Bool return Reflect.isObject(v) && !isPrimitive(v)
; /** Returns `true` if `v` is any of the following types: Int, Float, Bool, Date or String. *
*/ public static function isPrimitive(v:Dynamic) return switch Type.typeof(v)
...
*/ public static function isEnumValue(v:Dynamic) return switch Type.typeof(v)
{ case TEnum(_): true
; case _: false
; }
; /** Returns `true` if `cls` extends `sup` or one of its children. It also returns `true` if `cls` and `sup` are the same. *
...
*/ public static function sameType<A, B>(a:A, b:B):Bool return valueTypeToString(a) == valueTypeToString(b)
...
*/ inline public static function valueTypeInheritance<T>(value:T):Array<String> return typeInheritance(Type.typeof(value))
; /** Returns a string describing the type of any `value`. *
*/ inline public static function valueTypeToString<T>(value:T) return toString(Type.typeof(value))
; /** Returns a string describing the type of any `value`. *
*/ inline public static function anyValueToString(value:Dynamic)
{ if (Std.isOfType(value, Type.ValueType)) return toString(value)
; if (Std.isOfType(value, Class)) return Type.getClassName(value)
; if (Std.isOfType(value, Enum)) return Type.getEnumName(value)
