...
; // cancel immediately (doSomethingLater will never be executed) cancel()
...
; poll()
; }
; } // IMPLEMENTATIONS #if !(js || flash) static var timers = new Map<Int, haxe.Timer>()
; static var _id = 0
; #end
...
; #elseif !lime return throw "platform does not support delays (Timer.repeat)"
; #else var id = _id++, timer = new haxe.Timer(delayms)
; timer.run = callback
; timers.set(id, timer)
...
; // return handler.cancel.bind(true)
; #elseif !lime return throw "platform does not support delays (Timer.delay)"
; #else var id = _id++, timer = haxe.Timer.delay(function()
{ callback()
; clear(id)
; } , delayms)
; timers.set(id, timer)
...
; #elseif openfl var current = time(), next, listener = function(_)
{ next = time()
; callback(next - current)
; current = next
; }
...
; #elseif flash9 var current = time(), next, listener = function(_)
{ next = time()
; callback(next - current)
; current = next
; }
...
; #else var current = time(), next, listener = function()
{ next = time()
; callback(next - current)
; current = next
; }
...
; openfl.Lib.current.addEventListener(openfl.events.Event.ENTER_FRAME, listener)
; return cancel
; #elseif flash9 var listener = null, cancel = function() flash.Lib.current.removeEventListener(flash.events.Event.ENTER_FRAME, listener)
; listener = function(_)
{ cancel()
; callback()
; }
; flash.Lib.current.addEventListener(flash.events.Event.ENTER_FRAME, listener)
; return cancel
; #else return delay(callback, FRAME_RATE)
; #end
...
*/ public static function immediate(callback:Void->Void):Void->Void #if js return clear.bind(js.Syntax.code('setImmediate')(callback))
; #elseif java
// not sure why this is needed return delay(callback, 1)
; #else return delay(callback, 0)
; #end static #if js inline #end function clear(id):Void
{ #if js untyped js.Syntax.code('clearTimeout')(id)
; #elseif flash9 untyped __global__["flash.utils.clearTimeout"](id)
; #elseif flash untyped _global["clearTimeout"](id)
; #elseif !lime throw "platform does not support delays (Timer.clear)"
; #else var timer = timers.get(id)
; if (null != timer)
{ timers.remove(id)
; timer.stop()
; } #end
} /** Returns a time value in milliseconds. Where supported, the decimal value represents microseconds. Note that the initial value might change from platform to platform so only delta measurements make sense. *
*/ inline public static function time():Float #if js return js.Syntax.code("performance").now()
; #elseif flash return flash.Lib.getTimer()
; #elseif (cpp || neko || eval) return haxe.Timer.stamp() * 1000.0
; #elseif cs return (cs.system.Environment.TickCount : Float)
; #elseif java return cast(java.lang.System.nanoTime(), Float) / 1000000.0
; #elseif php return untyped __php__('microtime(true) * 1000.0')
; #elseif python return python.lib.Time.clock() * 1000
; #else return throw 'Timer.time() is not implemented in this target'
; #end static var _resolution:Null<Float>
; public static function resolution():Float
{ if (null != _resolution) return _resolution
...
; } #if js static function __init__() untyped
{ // Polyfills
// SCOPE var scope:Dynamic = js.Syntax.code('("undefined" !== typeof window && window) || ("undefined" !== typeof global && global) || Function("return this")()')
; // setImmediate if (!scope.setImmediate) scope.setImmediate = function(callback) scope.setTimeout(callback, 0)
; // rAF
// based on Paul Irish code: http:
// www.paulirish.com/2011/requestanimationframe-for-smart-animating/ var lastTime = 0, vendors = ['webkit', 'moz'], x = 0
; while (x < vendors.length && !scope.requestAnimationFrame)
{ scope.requestAnimationFrame = scope[vendors[x] + 'RequestAnimationFrame']
; scope.cancelAnimationFrame = scope[vendors[x] + 'CancelAnimationFrame'] || scope[vendors[x] + 'CancelRequestAnimationFrame']
; x++
; } if (!scope.requestAnimationFrame) scope.requestAnimationFrame = function(callback)
{ var currTime = Date.now().getTime(), timeToCall = Math.max(0, 16 - (currTime - lastTime)), id = scope.setTimeout(function() callback(currTime + timeToCall), timeToCall)
; lastTime = currTime + timeToCall
; return id
; }
; if (!scope.cancelAnimationFrame) scope.cancelAnimationFrame = function(id) scope.clearTimeout(id)
; // performance.now / High Resolution Timer if (js.Syntax.code("typeof")(scope.performance) == "undefined") scope.performance =
{
}
...
; scope.performance.now = function now() return Date.now() - nowOffset
; }
} #end
} #if java
@:nativeGen private
...
; } @:overload override public function run() callback()
; } #end
