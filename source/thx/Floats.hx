...
; /** `clamp` restricts a value within the specified range. ```haxe trace(1.3.clamp(0, 1))
; // prints 1 trace(0.8.clamp(0, 1))
; // prints 0.8 trace(-0.5.clamp(0, 1))
; // prints 0.0 ``` *
...
*/ public static function interpolateAngle(f:Float, a:Float, b:Float, turn:Float = 360) return wrapCircular(interpolate(f, a, a + angleDifference(a, b, turn)), turn)
...
; } if (Math.isNaN(a)) return Math.isNaN(b)
; if (Math.isNaN(b)) return false
; if (!Math.isFinite(b)) return (a > 0) == (b > 0)
