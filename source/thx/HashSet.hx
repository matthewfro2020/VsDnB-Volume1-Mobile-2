...
*/ inline function new(map:HashMap<T, Bool>) this = map
; /** Creates a new HashSet with optional intial values. *
*/ public static function create<T:
{ function hashCode():Int
; } >(?arr:Iterable<T>):HashSet<T>
{ var map:HashMap<T, Bool> = new HashMap()
; var hashSet:HashSet<T> = new HashSet(map)
; if (null != arr) hashSet.pushMany(arr)
; return hashSet
; } /** `add` pushes a value into `HashSet` if the value was not already present. It returns a boolean value indicating if `HashSet` was changed by the operation or not. *
*/ public function add(v:T):Bool return if (this.exists(v)) false
; else
{ this.set(v, true)
; true
...
{ var inst = empty()
; for (k in this.keys()) inst.push(k)
; return inst
...
; return result
; } /** `symmetricDifference` creates a new `HashSet` with elements that appear in either of the sets, but not in both. Equivalent to: `s1.union(s2) - s1.intersection(s2)` *
...
*/ inline public function exists(v:T):Bool return this.exists(v)
; /** `remove` removes an item from the HashSet. *
*/ inline public function remove(v:T):Bool return this.remove(v)
...
{ var result = empty()
; for (item in iterator()) if (set.exists(item)) result.push(item)
; return result
; } /** Like `add` but doesn't notify if the addition was successful or not. *
*/ inline public function push(v:T):Void this.set(v, true)
...
{ var arr:Array<T> = []
; for (k in this.keys()) arr.push(k)
; return arr
; } /** Converts `HashSet` to `String`. To differentiate from normal `Array`s the output string uses curly braces `
{
