...
; public static function empty():QueryString return new Map()
...
{ new Map()
; } else
{ if (null == decodeURIComponent) decodeURIComponent = QueryString.decodeURIComponent
...
; } , new Map())
; }
} @:from inline public static function parse(s:String):QueryString return parseWithSymbols(s, separator, assignment, decodeURIComponent)
; @:from public static function fromObject(o:
{
} ):QueryString
...
; if (v.length == 0) Reflect.setField(o, key, null)
; else if (v.length == 1) Reflect.setField(o, key, v[0])
; else Reflect.setField(o, key, v)
; return o
; } ,
{
} )
; } inline public function isEmpty():Bool return !this.iterator().hasNext()
; inline public function isEmptyOrMono():Bool
{ var arr = this.keys().toArray()
; if (arr.length == 0) return true
; if (arr.length != 1) return false
; return (this.get(arr[0]) : Array<String>).length == 0
; } inline public function exist(name:String):Bool return this.exists(name)
; inline public function remove(name:String) return this.remove(name)
; public function removeValue(name:String, value:String)
{ if (!this.exists(name)) return false
; return (this.get(name) : Array<String>).remove(value)
; } inline public function get(name:String):QueryStringValue return this.get(name)
...
; var map = new Map()
; for (key in this.keys()) map.set(key, (this.get(key).copy() : QueryStringValue))
; return map
...
{ if (null == this) return null
; if (null == encodeURIComponent) encodeURIComponent = QueryString.encodeURIComponent
; return this.keys().map(function(k)
{ var vs:Array<String> = this.get(k), ek = encodeURIComponent(k)
; if (vs.length == 0) return [ek]
; else
{ return vs.map((a) -> '$ek$assignment$
...
; if (!Arrays.equals((this.get(key) : Array<String>), (t.right : Array<String>))) return false
; tuples.remove(t)
; } return tuples.length == 0
; } @:to inline public function toString() return toStringWithSymbols(separator, assignment, encodeURIComponent)
; } @:forward(copy)
abstract QueryStringValue(Array<String>) from Array<String> to Array<String>
{ @:to function toString():String return this == null || this.length == 0 ? null : this.join(",")
; }
