...
; #end
/** Helper
class to work with `Iterator`. For documentation of specific methods refer to the equivalent methods in `thx.Arrays`
; *
*/ class Iterators
...
{ for (element in it) if (!predicate(element)) return false
; return true
...
{ for (element in it) if (predicate(element)) return true
; return false
; } /** It compares the lengths and elements of two given iterators and returns `true` if they match. An optional equality function can be passed as the last argument. If not provided, strict equality is adopted. *
...
*/ public static function getOption<T>(it:Iterator<T>, index:Int):Option<T> return Options.ofValue(get(it, index))
; /** Refer to `thx.Arrays.eachPair`. *
*/ public static function eachPair<TIn, TOut>(it:Iterator<TIn>, handler:TIn->TIn->Bool) Arrays.eachPair(toArray(it), handler)
; /** Refer to `Array.filter`. *
*/ public static function filter<TElement>(it:Iterator<TElement>, predicate:TElement->Bool):Array<TElement> return reduce(it, function(acc:Array<TElement>, element)
...
{ for (element in it) if (f(element)) return element
; return null
; } /** Refer to `thx.Arrays.findOption`. *
*/ public static function findOption<T, TFind>(it:Iterator<T>, f:T->Bool):Option<T> return Options.ofValue(find(it, f))
; /** Refer to `thx.Arrays.first`. *
*/ public static function first<T, TFind>(it:Iterator<T>):Null<T> return it.hasNext() ? it.next() : null
; /** Returns `true` if the iterator contains at least one element. *
*/ inline public static function hasElements<T>(it:Iterator<T>):Bool return it.hasNext()
...
*/ inline public static function isEmpty<T>(it:Iterator<T>):Bool return !it.hasNext()
; /** `isIterator` checks that the passed argument has all the requirements to be an `Iterator`. Note that no type checking is performed at runtime, the method only checks that the value has two fields `next` and `hasNext` and that they are both functions. *
...
*/ public static function fmapi<T, S>(it:Iterator<T>, f:T->Int->S):Iterator<S> return new MapIIterator(it, f)
...
; return initial
; } public static function foldLeft<A, B>(it:Iterator<A>, zero:B, f:B->A->B):B return reduce(it, f, zero)
; /** * Fold by mapping the contained values into some monoidal type and reducing with that monoid.
*/ public static function foldMap<A, B>(it:Iterator<A>, f:A->B, m:Monoid<B>):B return foldLeft(fmap(it, f), m.zero, m.append)
...
{ var elements = []
; for (element in it) elements.push(element)
; return elements
...
{ var array = []
; while (it1.hasNext() && it2.hasNext()) array.push(new Tuple2(it1.next(), it2.next()))
; return array
...
{ var array = []
; while (it1.hasNext() && it2.hasNext() && it3.hasNext()) array.push(new Tuple3(it1.next(), it2.next(), it3.next()))
; return array
...
{ var array = []
; while (it1.hasNext() && it2.hasNext() && it3.hasNext() && it4.hasNext()) array.push(new Tuple4(it1.next(), it2.next(), it3.next(), it4.next()))
; return array
...
{ var array = []
; while (it1.hasNext() && it2.hasNext() && it3.hasNext() && it4.hasNext() && it5.hasNext()) array.push(new Tuple5(it1.next(), it2.next(), it3.next(), it4.next(), it5.next()))
; return array
; }
...
{ this.base = base
; this.f = f
; } public function next():B return f(base.next())
; public function hasNext():Bool return base.hasNext()
