...
; *
*/ macro public static function ensure<T>(value:ExprOf<Null<T>>, alt:ExprOf<T>) return try macro if (null == $e
{ value
} ) $e
{ value
} = $e
...
*/ macro public static function exists(value:Expr) return macro($e
{ Nulls.opt(value)
} != null)
...
*/ macro public static function isNull(value:Expr) return macro($e
{ Nulls.opt(value)
} == null)
; /** It traverses a chain of dot/array identifiers and it returns the last value in the chain or null if any of the identifiers is not set. ```haxe var o :
{ a :
{ b :
{ c : String
}
...
{ case TArray(a, e): traverse(a)
; var index = TypedExprTools.toString(e, true)
; ids.push('[$index]')
; case TConst(TThis): ids.push('this')
; case TConst(TInt(index)): ids.push('$index')
; case TLocal(o): ids.push(o.name)
; case TField(f, v): traverse(f)
; switch v
{ case FAnon(id): ids.push(id.toString())
; #if (haxe_ver >= "3.2") case FInstance(_, _, n): #else case FInstance(_, n): #end ids.push(n.toString())
; case _: throw 'invalid expression $e'
; } case TParenthesis(p): traverse(p)
; case TCall(e, el): traverse(e)
; var a = el.map(TypedExprTools.toString.bind(_, true)).join(", ")
; ids[ids.length - 1] += '($a)'
; case TBlock(_): if (Context.defined("python")) Context.error("Nulls.opt doesn't support some method calls on Python", value.pos)
; var s = TypedExprTools.toString(e, true)
; // trace(s)
; ids.push(s)
; case _: throw 'invalid expression $e'
; } traverse(Context.typeExpr(value))
; var first = ids.shift(), buf = '(function()
{ \n var _0 = $first
...
; } /** Like `opt` but allows an `alt` value that replaces a `null` occurrance. ```haxe var s : String = null
; trace(s.or('b'))
; // prints 'b' s = 'a'
; trace(s.or('b'))
; // prints 'a'
// or more complex var o :
{ a :
{ b :
{ c : String
}
...
; trace((o.a.b.c).or("B"))
; // prints 'A' ``` Notice that the subject `value` must be a constant identifier (eg: fields, local variables, ...). *
*/ macro public static function or<T>(value:ExprOf<Null<T>>, alt:ExprOf<T>) return macro
{ var t = $e
{ Nulls.opt(value)
}
; t != null ? t : $e
...
*/ macro public static function notNull(value:Expr) return macro($e
{ Nulls.opt(value)
} != null)
; }
