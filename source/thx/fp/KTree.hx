...
; private
typedef Generator<T> = Void->GeneratorResult<T>
...
{ case [Cons(x, xs), Cons(y, ys)]: var l:KTree<T> = x
; var r:KTree<T> = y
...
{ case [Branch(l, ls), Branch(r, rs)]: var o:Bool = fn(l, r)
; if (!o)
{ o
; } else
{ handler(ls, rs)
...
{ return switch t
{ case Cons(Branch(x, xs), rst): if (xs == null)
{ xs = List.empty()
; } new Tuple2(Some(x), next.bind(concat(xs, rst), concat))
; default: nothing()
; }
