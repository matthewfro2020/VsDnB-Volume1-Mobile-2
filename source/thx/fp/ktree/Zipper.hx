...
{ return switch this
{ case Cons(cursor, Cons(Branch(v, siblings), previous)): switch (findHead(siblings, cursor))
{ case Cons(_, Cons(right, _)): var tree:KTree<T> = Branch(v, siblings)
; var o:Zipper<T> = new Zipper(Cons(right, Cons(tree, previous)))
; return o
; default: var tree:KTree<T> = Branch(v, siblings)
...
{ return switch (this)
{ case Cons(cursor, Cons(Branch(v, siblings), previous)): switch (findLeft(siblings, cursor))
{ case Cons(left, _): var tree:KTree<T> = Branch(v, siblings)
...
{ return switch (this)
{ case Cons(Branch(node, children), rest): var new_node = Branch(node, children.prepend(v))
; update(new_node)
...
; children = children.foldLeft(List.empty(), function(memo, next)
...
{ /** goes from tip to root, updating the references, and producing a tuple of old_reterence,new_reference, the old being used for reference equality in the next step.
*/ var changes:List<Tuple2<KTree<T>, KTree<T>>> = zip.foldLeft(Nil, function(memo:List<Tuple2<KTree<T>, KTree<T>>>, next:KTree<T>):List<Tuple2<KTree<T>, KTree<T>>>
{ return switch ([memo, next])
{ case [Nil, _]: var ls:List<Tuple2<KTree<T>, KTree<T>>> = Cons(new Tuple2(next, replace), Nil)
; ls
; case [memo, Branch(v, rst)]: rst = (rst == null) ? List.empty() : rst
; var leaves:List<KTree<T>> = rst.map(function(x:KTree<T>):KTree<T>
...
{ return switch (next)
{ case Cons(x, xs): var ls = handler(xs)
