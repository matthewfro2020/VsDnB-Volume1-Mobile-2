...
{ for (key in src.keys()) dst.set(key, src.get(key))
; return dst
; } /** Converts a Map<TKey, TValue> into an Array<Tuple2<TKey, TValue>> *
*/ public static function tuples<TKey, TValue>(map:IMap<TKey, TValue>):Array<Tuple2<TKey, TValue>> return map.keys().map(function(key) return new Tuple2(key, map.get(key)))
; /** It maps values from one `Map` instance to another. *
*/ public static function mapValues<TKey, TValueA, TValueB>(map:IMap<TKey, TValueA>, f:TValueA->TValueB, acc:Map<TKey, TValueB>):Map<TKey, TValueB> return reduce(map, function(m:Map<TKey, TValueB>, t)
{ m.set(t._0, f(t._1))
; return m
; } , acc)
; /** Applies the reduce function on every key/value pair in the map. *
*/ public static function reduce<TKey, TValue, TOut>(map:IMap<TKey, TValue>, f:TOut->Tuple<TKey, TValue>->TOut, acc:TOut):TOut return tuples(map).reduce(f, acc)
...
*/ public static function foldLeftWithKeys<K, A, B>(map:Map<K, A>, f:B->K->A->B, acc:B):B return map.keys().reduce(function(acc, k) return f(acc, k, map.get(k)), acc)
; /** Null-safe get. *
*/ public static function getOption<TKey, TValue>(map:IMap<TKey, TValue>, key:TKey):Option<TValue> return Options.ofValue(map.get(key))
...
*/ inline public static function isEmpty<TKey, TValue>(map:Map<TKey, TValue>) return !map.iterator().hasNext()
; /** Returns true if a value is of any type of Map. Equivalent to `Std.isOfType(v, IMap)`. *
*/ inline public static function isMap(v:Dynamic) return Std.isOfType(v, IMap)
...
*/ @:generic public static function semigroup<K, V>():Semigroup<IMap<K, V>>
