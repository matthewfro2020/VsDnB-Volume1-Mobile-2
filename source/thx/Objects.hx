...
{
} ):Bool return size(o) == 0
; /** `exists` returns true if `o` contains a field named `name`. *
*/ inline public static function exists(o:
{
} , name:String):Bool return Reflect.hasField(o, name)
; /** `fields` returns an array of string containing the field names of the argument object. *
*/ inline public static function fields(o:
{
} ):Array<String> return Reflect.fields(o)
; public static function deflate(o:
{
} , ?flattenArrays:Bool = true):
{
...
{ switch f(value)
{ case Left(v): map.set('$i', v)
; case Right(m): for (k in m.keys())
{ map.set('$i.$k', m.get(k))
; }
} return map
; } , new Map()))
...
{ case Left(v): map.set('$key', v)
; case Right(m): if (Maps.isEmpty(m))
{ map.set('$key',
{
} )
; } else
...
*/ macro public static function merge(first:ExprOf<
{
} >, second:ExprOf<
{
} >)
{ haxe.macro.Context.warning('use thx.Objects.shallowMerge or thx.Objects.deepMerge instead', haxe.macro.Context.currentPos())
; // macro-time @:deprecation return thx.macro.Objects.shallowMergeImpl(first, second)
; } /** Shallow, typed merge of two anonymous objects. *
*/ macro public static function shallowMerge(first:ExprOf<
{
} >, second:ExprOf<
{
} >)
...
*/ @:deprecated('use thx.Objects.shallowCombine or thx.Objects.deepCombine instead') public static function combine(first:
{
} , second:
{
} ):
...
; } /** Shallow, untyped merge of two anonymous objects. *
*/ public static function shallowCombine(first:
{
} , second:
{
} ):
...
*/ /* TODO: placeholder for future macro-based deepMergeImpl macro public static function deepMerge(first: ExprOf<
{
} >, second: ExprOf<
{
} >)
...
*/ public static function deepCombine(first:
{
} , second:
{
} ):
...
; } /** Copies the values from the fields of `from` to `to`. If `to` already contains those fields, then it replaces those values with the return value of the function `replacef`. If not set, `replacef` always returns the value from the `from` object. *
*/ public static function assign(to:
{
} , from:
{
} , ?replacef:String->Dynamic->Dynamic->Dynamic):
{
} { if (null == replacef) replacef = function(field:String, oldv:Dynamic, newv:Dynamic) return newv
...
{
} ):Map<String, Dynamic> return tuples(o).reduce(function(map:Map<String, Dynamic>, t)
{ map.set(t._0, t._1)
; return map
; } , new Map())
; /** `size` returns how many fields are present in the object. *
*/ inline public static function size(o:
{
} ):Int return Reflect.fields(o).length
; /** Returns a string representation of the object containing each field and value. The function is recursive so it might generate infinite loops if used with circular references. *
*/ public static function string(o:
{
} ):String
{ return "
{ " + Reflect.fields(o).map(function(key)
...
*/ inline public static function values(o:
{
} ):Array<Dynamic> return Reflect.fields(o).map(function(key) return Reflect.field(o, key))
; /** Converts an object into an Array<Tuple2<String, Dynamic>> where the left value (_0) of the tuple is the field name and the right value (_1) is the field value. *
*/ public static function tuples(o:
{
} ):Array<Tuple2<String, Dynamic>> return Reflect.fields(o).map(function(key) return new Tuple2(key, Reflect.field(o, key)))
; /** Determines whether an object has fields represented by a string path. The path can contain object keys and array indices separated by ".". E.g.
{ key1:
{ key2: [1, 2, 3]
}
} .hasPath("key1.key2.2") -> returns true *
*/ public static function hasPath(o:
{
} , path:String):Bool
...
{ var index = Std.parseInt(currentPath), arr = Std.downcast(current, Array)
...
; } /** Like `hasPath`, but will return `false` for null values, even if the key exists. E.g.
{ key1 :
{ key2: null
}
} .hasPathValue("key1.key2") -> returns false *
*/ public static function hasPathValue(o:
{
} , path:String):Bool return getPath(o, path) != null
; /** Gets a value from an object by a string path. The path can contain object keys and array indices separated by ".". Returns null for a path that does not exist. E.g.
{ key1:
{ key2: [1, 2, 3]
}
} .getPath("key1.key2.2") -> returns 3 E.g.
{ key1:
{ key2: [1, 2, 3]
}
} .getPath("key1.key2[2]") -> returns 3 *
*/ public static function getPath(o:
{
} , path:String):Dynamic
{ path = normalizePath(path)
; var paths = path.split(".")
...
; } /** Null-safe getPath *
*/ public static function getPathOption(o:
{
...
} .getPath("key1.key2.5", 7) -> returns 7 ``` *
*/ public static function getPathOr(o:
{
} , path:String, alt:Dynamic):Dynamic
{ return Options.getOrElse(getPathOption(o, path), alt)
; } /** Sets a value in an object by a string path. The path can contain object keys and array indices separated by ".". Returns the original object, for optional chaining of other object methods. Inner objects and arrays will be created as needed when traversing the path. E.g.
{ key1:
{ key2: [1, 2, 3]
}
} .setPath("key1.key2.2", 4) -> returns
{ key1:
{ key2: [ 1, 2, 4 ]
}
} *
*/ public static function setPath<T>(o:
{
} , path:String, val:T):
{
} { path = normalizePath(path)
; var paths = path.split("."), current:Dynamic = o
; for (i in 0...(paths.length - 1))
{ var currentPath = paths[i], nextPath = paths[i + 1]
...
; } return o
; } /** Delete an object's property, given a string path to that property. E.g.
{ foo : 'bar'
} .removePath('foo') -> returns
{
} *
*/ public static function removePath(o:
{
} , path:String):
{
} { path = normalizePath(path)
; var paths = path.split(".")
; // the last item in the list of paths is the target field var target = paths.pop()
; // iterate over all remaining fields to find the sub-object containing
// the target field to be removed try
...
{ return ~/\[(\d+)\]/g.replace(path, ".$1")
; // Normalize [x] array access to .x (where x is an integer)
} /* Create a copy of the object with the field `field` replaced by `value`.
*/ macro public static function with<T:
{
} >(o:haxe.macro.Expr.ExprOf<T>, field:haxe.macro.Expr, value:haxe.macro.Expr)
{ var matchField = haxe.macro.ExprTools.toString(field)
; var obj =
{ expr: switch haxe.macro.Context.typeof(o)
{ case TAnonymous(a): var t = a.get()
; buildFromAnonymous(matchField, t.fields, value, field.pos)
; case TType(tr, _): var t = tr.get()
; switch t
{ case
{ type: TAnonymous(a)
} : var t = a.get()
...
; $obj
; }
; } #if macro static function buildFromAnonymous(matchField, fields, value, pos)
{ var found = false
...
{ found = true
; return
{ field: field.name, expr: value
}
; } else
{ return
{ field: field.name, expr: macro o.$fieldName
}
; }
} ))
; if (!found)
