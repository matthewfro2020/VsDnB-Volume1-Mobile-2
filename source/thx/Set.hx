...
*/ public static function createObject<T:
{
} >(?it:Iterable<T>)
...
{ var map = new Map<T, Bool>()
; var set = new Set<T>(map)
; if (null != arr) set.pushMany(arr)
; return set
; } public var length(get, never):Int
; inline function new(map:Map<T, Bool>) this = map
; /** `add` pushes a value into `Set` if the value was not already present. It returns a boolean value indicating if `Set` was changed by the operation or not. *
...
; return result
; } public function filter(predicate:T->Bool):Set<T> return reduce(function(acc:Set<T>, v:T)
{ if (predicate(v)) acc.add(v)
; return acc
; } , empty())
; public function map<TOut>(f:T->TOut):Array<TOut> return reduce(function(acc:Array<TOut>, v:T)
{ acc.push(f(v))
; return acc
; } , [])
...
; return arr
; } /** Converts `Set` into `String`. To differentiate from normal `Array`s the output string uses curly braces `
{
