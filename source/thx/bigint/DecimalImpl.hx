...
{ var lhs = a.matchScale(b), rhs = b.matchScale(a)
...
{ var lhs = this.matchScale(that), rhs = that.matchScale(this)
...
; } public function divide(that:DecimalImpl):DecimalImpl return divideWithScale(that, Decimals.divisionExtraScale)
; public function divideWithScale(that:DecimalImpl, scale:Int):DecimalImpl
{ if (that.isZero()) throw new Error('division by zero')
; var lhs = this.matchScale(that), rhs = that.matchScale(this), pow = Small.ten.pow(Bigs.fromInt(rhs.scale + scale)), qr = lhs.value.multiply(pow).divMod(rhs.value), nscale = rhs.scale + scale
...
{ if (that.isZero()) throw new Error('modulo by zero')
; var lhs = this.matchScale(that), rhs = that.matchScale(this), pow = Small.ten.pow(Bigs.fromInt(scale)), qr = lhs.value.multiply(pow).divMod(rhs.value.multiply(pow)), nscale = lhs.scale + scale
; return new DecimalImpl(qr.remainder, nscale).trim(nscale)
; } public function multiply(that:DecimalImpl):DecimalImpl
{ // TODO scale sum can overflow return new DecimalImpl(value.multiply(that.value), scale + that.scale)
; } public function modulo(that:DecimalImpl):DecimalImpl
{ return moduloWithScale(that, Decimals.divisionExtraScale)
; } public function abs():DecimalImpl return new DecimalImpl(value.abs(), scale)
; public function negate():DecimalImpl return new DecimalImpl(value.negate(), scale)
; public function next():DecimalImpl return add(one)
; public function prev():DecimalImpl return subtract(one)
...
; var scaled = scaleTo(newscale), f = (scaled.isZero() ? one : modulo(scaled)).multiply(ten.pow(newscale)).toFloat()
; if (f <= 0)
{ return scaled
; } else
{ return new DecimalImpl(scaled.value.add(Small.one), scaled.scale)
; }
} public function floorTo(newscale:Int):DecimalImpl return scaleTo(newscale)
...
; } public function isNegative():Bool return value.sign
; public function isEven():Bool return value.isEven()
; public function isOdd():Bool return value.isOdd()
; public function isZero():Bool return value.isZero()
...
; } // TODO needs better implementation public function trim(?min = 0):DecimalImpl
{ if (scale == 0) return this
; var s = toString(), parts = s.split("."), dec = parts[1].trimCharsRight("0").rpad("0", min)
; if (dec.length > 0) s = parts[0] + "." + dec
; else s = parts[0]
; return Decimals.parse(s)
; } // TODO needs better implementation public function toFloat():Float
{ return Std.parseFloat(toString())
; } public function toInt():Int
{ var i = value.divide(Small.ten.pow(Bigs.fromInt(scale)))
; return i.toInt()
; } public function toString()
{ var sign = value.sign, i = (sign ? value.negate() : value).toString(), l = i.length
; if (scale == 0)
{ return (sign ? "-" : "") + i
...
{ if (scale >= that.scale) return this
