...
; case Right(v): v
; }
; } public static function map<L, RIn, ROut>(either:Either<L, RIn>, f:RIn->ROut):Either<L, ROut> return switch either
{ case Left(v): Left(v)
...
; case Right(v): f(v)
; }
; } public static function leftMap<LIn, LOut, R>(either:Either<LIn, R>, f:LIn->LOut):Either<LOut, R> return switch either
{ case Left(v): Left(f(v))
; case Right(v): Right(v)
; }
; public static function orThrow<L, R>(either:Either<L, R>, message:String):R return switch either
{ case Left(v): throw new thx.Error('$message: $v')
; case Right(v): v
; }
; public static function toValidation<E, T>(either:Either<E, T>):Validation<E, T> return either
; public static function toVNel<E, T>(either:Either<E, T>):Validation.VNel<E, T> return leftMap(either, Nel.pure)
; public static function cata<L, R, A>(either:Either<L, R>, l:L->A, r:R->A):A return switch either
{ case Left(l0): l(l0)
; case Right(r0): r(r0)
; }
...
; }
} public static function foldLeft<L, R, A>(either:Either<L, R>, a:A, f:A->R->A):A return switch either
{ case Left(l0): a
...
{ return
{ zero: (function(r:R) return Right(r) : EitherK<R, L, R>), append: function(f0:EitherK<R, L, R>, f1:EitherK<R, L, R>):EitherK<R, L, R>
