...
*/ inline public static function abs(v:Int):Int return v < 0 ? -v : v
...
*/ public static function canParse(s:String) return pattern_parse.match(s)
...
*/ public static inline function clampSym(v:Int, max:Int):Int return clamp(v, -max, max)
...
*/ public static function compare(a:Int, b:Int):Int return a - b
...
; n = t % m
; } while (true)
; } /** Given a value `t` between 0 and 1, it interpolates that value in the range between `a` and `b`. The returned value is a rounded integer.
*/ public static function interpolate(f:Float, a:Float, b:Float):Int return Math.round(a + (b - a) * f)
; /** `isEven` returns `true` if `v` is even, `false` otherwise. *
*/ inline public static function isEven(v:Int) return v % 2 == 0
; /** `isOdd` returns `true` if `v` is odd, `false` otherwise. *
*/ inline public static function isOdd(v:Int) return v % 2 != 0
...
; } inline public static function rpad(v:Int, ?pad = "0", len:Int):String return StringTools.rpad('$v', pad, len)
; /** It returns the maximum value between `a` and `b`. *
*/ inline public static function max(a:Int, b:Int):Int return a > b ? a : b
...
; #elseif flash9 if (base == null) base = 0
; var v:Int = untyped __global__["parseInt"](s, base)
; return Math.isNaN(v) ? null : v
; #else if (base != null && (base < 2 || base > BASE.length)) return throw 'invalid base $base, it must be between 2 and $
{ BASE.length
} '
...
; return acc * sign
; #end
} /** Integer random function that includes both upper and lower limits. A roll on a die with 6 sides would be the equivalent to the following: ```haxe var d6 = Ints.random(1, 6)
; ``` *
*/ inline public static function random(min = 0, max:Int) return Std.random(max - min + 1) + min
...
; if (base == 10 || value == 0) return '$value'
