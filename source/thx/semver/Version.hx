...
; var major = Std.parseInt(VERSION.matched(1)), minor = Std.parseInt(VERSION.matched(2)), patch = Std.parseInt(VERSION.matched(3)), pre = parseIdentifiers(VERSION.matched(4)), build = parseIdentifiers(VERSION.matched(5))
; return new Version(major, minor, patch, pre, build)
; } @:from public static function arrayToVersion(a : Array<Int>)
{ a = (null == a ? [] : a).map(function(v) return v < 0 ? -v : v) .concat([0,0,0]) .slice(0, 3)
; return new Version(a[0], a[1], a[2], [], [])
; } inline function new(major : Int, minor : Int, patch : Int, pre : Array<Identifier>, build : Array<Identifier>) this =
...
; public function nextMajor() return new Version(major+1, 0, 0, [], [])
; public function nextMinor() return new Version(major, minor+1, 0, [], [])
; public function nextPatch() return new Version(major, minor, patch+1, [], [])
; public function nextPre() return new Version(major, minor, patch, nextIdentifiers((this : SemVer).pre), [])
; public function nextBuild() return new Version(major, minor, patch, (this : SemVer).pre, nextIdentifiers((this : SemVer).build))
; public function withPre(pre : String, ?build : String) return new Version(major, minor, patch, parseIdentifiers(pre), parseIdentifiers(build))
; public function withBuild(build : String) return new Version(major, minor, patch, this.pre, parseIdentifiers(build))
; public inline function satisfies(rule : VersionRule) : Bool return rule.isSatisfiedBy(this)
...
; } @:op(A!=B) public function different(other : Version) return !(other.equals(this))
...
{ return false
; }
} @:op(A>=B) public function greaterThanOrEqual(other : Version) return equals(other) || greaterThan(other)
; @:op(A<B) public function lessThan(other : Version) return !greaterThanOrEqual(other)
; @:op(A<=B) public function lessThanOrEqual(other : Version) return !greaterThan(other)
...
; static function identifiersToString(ids : Array<Identifier>) return ids.map(function(id) return switch id
{ case StringId(s): s
; case IntId(i): '$i'
; } ).join('.')
; static function parseIdentifiers(s : String) : Array<Identifier> return (null == s ? '' : s).split('.') .map(sanitize) .filter(function(s) return s != '') .map(parseIdentifier)
...
; for(i in 0...a.length) switch [a[i], b[i]]
...
{ var identifiers = identifiers.copy(), i = identifiers.length
; while(--i >= 0) switch (identifiers[i])
{ case IntId(id): identifiers[i] = IntId(id+1)
; break
; case _:
} if(i < 0) throw 'no numeric identifier found in $identifiers'
; return identifiers
; } static var SANITIZER = ~/[^0-9A-Za-z-]/g
; static function sanitize(s : String) : String return SANITIZER.replace(s, '')
; } enum Identifier
{ StringId(value : String)
; IntId(value : Int)
; } typedef SemVer =
{ version : Array<Int>, pre : Array<Identifier>, build : Array<Identifier>
}
