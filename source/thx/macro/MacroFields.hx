...
*/ public static function isMethod(field:Field) return switch field.kind
{ case FFun(_): true
; case _: false
; }
; /** Returns true if the `field` is a variable. *
*/ public static function isVar(field:Field) return switch field.kind
{ case FVar(_): true
; case _: false
; }
; /** Returns true if the `field` is a property. *
*/ public static function isProperty(field:Field) return switch field.kind
{ case FProp(_): true
; case _: false
; }
...
*/ public static function getType(field:Field):ComplexType return switch field.kind
{ case FVar(t, _): t
...
*/ public static function getFirstMetaParam(field:Field, name:String) return getMetaParams(field, name)[0]
; /** Returns the params as `Array<String>` of a meta with `name`. It always returns an array even if no matches are found.
*/ public static function getMetaParamsAsStrings(field:Field, name:String) return getMetaParams(field, name).map(ExprTools.toString)
