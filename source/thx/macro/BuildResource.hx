...
{ var fields = Context.getBuildFields(), o = getResourceObject(Context.getLocalClass().get())
; return generateFieldsFromObjectLiteral(o).concat(fields)
; } static var formats = ["json" #if yaml, "yaml" #end]
; static var prefixSymbol = "&"
; static function generateFieldsFromObjectLiteral(o:
{
} ):Array<Field>
{ return Objects.tuples(o).map(function(t)
{ var v = t.right, e = macro $v
{ v
}
; return
{ pos: Context.currentPos(),
// TODO improve name: t.left, meta: null, kind: FieldType.FVar(null, e), doc: null, access: [APublic, AStatic]
}
; } )
; } static function resolveReferences(o:
{
} , prefix:String, module:String, path:String)
{ if (null == prefix || !Reflect.isObject(o)) return
; var length = prefix.length
; o.tuples().map(function(t)
{ if (t.left.startsWith(prefix))
{ if (!Std.isOfType(t.right, String)) return
; var key = t.left.substring(length), value:String = path.isEmpty() ? t.right : '$path/$
{ t.right
...
{ if (!meta.has(":resolve")) return null
; var values = meta.extract(":resolve") .map(function(v) return v.params) .flatten() .map(function(p) return ExprTools.getValue(p))
; if (values.length == 0 || !Std.isOfType(values[0], String)) return prefixSymbol
; return values[0]
...
{
} { if (!meta.has(":content")) return
{
}
; var o =
{
}
; meta.extract(":content") .map(function(v) return v.params) .flatten() .map(function(p) return ExprTools.getValue(p)) .map(function(n) Objects.assign(o, n))
...
{
} { if (!meta.has(":dir")) return
{
}
; var out = meta.extract(":dir") .map(function(v) return v.params) .flatten() .map(function(p) return ExprTools.getValue(p)) .map(function(path) return getFromDir(Std.string(path), module, prefix)) .reduce(function(acc, o) return Objects.assign(acc, o),
{
} )
; return out
...
; // ignore hidden files var fullPath = '$dir/$path', name = normalizeName(path)
...
; // strip extension path = path.split(".").slice(0, -1).join(".")
; // change file name path = path.replace("\\", "/") .split("/") .slice(0, -1) .concat([type.split(".").pop()]) .join("/")
; var list = formats.map(function(format) return './$path.$format').filter(function(path)
{ // lower case file name var lc = path.split("/").slice(0, -1).concat([path.split("/").pop().toLowerCase()]).join("/")
...
{ file: path, format: null
} )
; return getFromFiles(list, module, prefix)
; } static function getContentFile(meta:MetaAccess, module:String, prefix:String)
{ return getFromFiles(formats.map(function(f) return
{ format: f, meta: ':$f'
} ) .concat([
{ format: null, meta: ":file"
} ]) .map(function(des) return !meta.has(des.meta) ? [] : meta.extract(des.meta) .map(function(v) return v.params) .flatten() .map(function(p) return ExprTools.getValue(p)) .map(function(file) return
{ file: file, format: des.format
} )) .flatten() .filter(function(item) return item != null), module, prefix)
; } static function setField(o:
{
} , field:String, value:Dynamic)
{ if (Reflect.hasField(o, field)) throw 'duplicate key $field'
; Reflect.setField(o, field, value)
; return o
; } static function getFromFiles(list:Array<
{ file:String, format:String
} >, module:String, prefix:String)
{ var o =
{
}
...
{ case "json": (haxe.Json.parse(content) : Dynamic)
; #if yaml case "yml", "yaml": var options = new yaml.Parser.ParserOptions()
; options.maps = false
; yaml.Yaml.parse(content, options)
; #end case _: if (allowText) content
; else Context.error('Invalid format $format', Context.currentPos())
; }
