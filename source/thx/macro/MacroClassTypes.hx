...
{ /** It returns an array of types in string format representing the inheritance chain of the passed `ClassType`. *
*/ public static function inheritance(cls:ClassType):Array<ClassType>
{ var types = [cls], parent = null == cls.superClass ? null : inheritance(cls.superClass.t.get())
; if (null != parent) types = types.concat(parent)
; return types
...
{ var match = toString(superCls), types = inheritanceAsStrings(cls)
; return types.contains(match)
; } public static function classImplements(cls:ClassType, interf:ClassType):Bool
{ var match = toString(interf), types = interfacesAsStrings(cls)
; return types.contains(match)
...
; case _: haxe.macro.Context.error('$fullname is not a
class or
interface type: $t', haxe.macro.Context.currentPos())
; }
...
*/ public static function isPublic(cls:ClassType) return !cls.isPrivate
...
; } /** Extracts the first meta entry parameter for `name`. It returns `null` if it does not exist.
*/ public static function getFirstMetaParam(cls:ClassType, name:String) return getMetaParams(cls, name)[0]
; /** Returns the params of a meta with `name`. It always returns an array even if no matches are found.
