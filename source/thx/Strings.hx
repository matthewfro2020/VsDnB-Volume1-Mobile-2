...
*
*/ class Strings
{ /** `after` searches for the first occurrance of `searchFor` and returns the text after that. If `searchFor` is not found, an empty string is returned. *
*/ public static function after(value:String, searchFor:String)
{ var pos = value.indexOf(searchFor)
; if (pos < 0) return ""
; else return value.substring(pos + searchFor.length)
; } /** `afterLast` searches for the last occurrance of `searchFor` and returns the text after that. If `searchFor` is not found, an empty string is returned. *
*/ public static function afterLast(value:String, searchFor:String)
{ var pos = value.lastIndexOf(searchFor)
; if (pos < 0) return ""
; else return value.substring(pos + searchFor.length)
; } /** `before` searches for the first occurrance of `searchFor` and returns the text before that. If `searchFor` is not found, an empty string is returned. *
*/ public static function before(value:String, searchFor:String)
{ var pos = value.indexOf(searchFor)
; if (pos < 0) return ""
; else return value.substring(0, pos)
; } /** `beforeLast` searches for the last occurrance of `searchFor` and returns the text before that. If `searchFor` is not found, an empty string is returned. *
...
{ if (whiteSpaceOnly)
{ #if (php && haxe_ver >= 4.0) return php.Global.ucwords(value)
; #elseif (php && haxe_ver < 4.0) return untyped __call__("ucwords", value)
; #else return UCWORDSWS.map(capitalize(value), upperMatch)
; #end
} else
...
; *
*/ public static function canonicalizeNewlines(value:String):String return CANONICALIZE_LINES.replace(value, "\n")
...
*/ inline public static function caseInsensitiveEndsWith(s:String, end:String):Bool return StringTools.endsWith(s.toLowerCase(), end.toLowerCase())
; /** Compares a string `s` with many `values` and see if one of them matches its end ignoring their case. *
*/ inline public static function caseInsensitiveEndsWithAny(s:String, values:Array<String>):Bool return endsWithAny(s.toLowerCase(), values.map(function(v) return v.toLowerCase()))
; /** Compares two strings ignoring their case. *
*/ inline public static function caseInsensitiveStartsWith(s:String, start:String):Bool return StringTools.startsWith(s.toLowerCase(), start.toLowerCase())
; /** Compares a string `s` with many `values` and see if one of them matches its beginning ignoring their case. *
*/ inline public static function caseInsensitiveStartsWithAny(s:String, values:Array<String>):Bool return startsWithAny(s.toLowerCase(), values.map(function(v) return v.toLowerCase()))
...
*/ public static function collapse(value:String) return WSG.replace(StringTools.trim(value), " ")
...
*/ public inline static function compare(a:String, b:String) return a < b ? -1 : (a > b ? 1 : 0)
...
*/ inline public static function caseInsensitiveContains(s:String, test:String) #if php return test == "" || s.toLowerCase().indexOf(test.toLowerCase()) >= 0
; #else return s.toLowerCase().indexOf(test.toLowerCase()) >= 0
; #end
/** `contains` returns `true` if `s` contains one or more occurrences of `test`. *
*/ inline public static function contains(s:String, test:String) #if php return test == "" || s.indexOf(test) >= 0
; #else return s.indexOf(test) >= 0
; #end
/** Return the number of occurances of `test` in `s`. *
*/ public static function count(s:String, test:String) return s.split(test).length - 1
; /** `contains` returns `true` if `s` contains any of the strings in `tests` regardless of the text case *
*/ inline public static function caseInsensitiveContainsAny(s:String, tests:Array<String>) return tests.any(caseInsensitiveContains.bind(s, _))
; /** `contains` returns `true` if `s` contains any of the strings in `tests` *
*/ inline public static function containsAny(s:String, tests:Array<String>) return tests.any(contains.bind(s, _))
; /** `contains` returns `true` if `s` contains all of the strings in `tests` regardless of the text case *
*/ inline public static function caseInsensitiveContainsAll(s:String, tests:Array<String>) return tests.all(caseInsensitiveContains.bind(s, _))
; /** `contains` returns `true` if `s` contains all of the strings in `tests` *
*/ inline public static function containsAll(s:String, tests:Array<String>) return tests.all(contains.bind(s, _))
; /** `dasherize` replaces all the occurrances of `_` with `-`
; *
*/ public static function dasherize(s:String) return s.replace('_', '-')
; /** Compares strings `a` and `b` and returns the position where they differ. ```haxe Strings.diffAt("abcdef", "abc123")
; // returns 3 ``` *
*/ public static function diffAt(a:String, b:String)
{ var min = Ints.min(a.length, b.length)
; for (i in 0...min) if (a.substring(i, i + 1) != b.substring(i, i + 1)) return i
; return min
; } /** `ellipsis` truncates `s` at len `maxlen` replaces the last characters with the content of `symbol`. ```haxe 'thx is a nice library'.ellipsis(8)
; // returns 'thx is …' ``` *
...
; } /** Same as `ellipsis` but puts the symbol in the middle of the string and not to the end. ```haxe 'thx is a nice library'.ellipsisMiddle(16)
; // returns 'thx is … library' ``` *
...
; } var hll = Math.ceil((maxlen - symboll) / 2), hlr = Math.floor((maxlen - symboll) / 2)
; return s.substr(0, hll) + symbol + s.substr(sl - hlr, hlr)
; } else return s
; } /** Returns `true` if `s` ends with any of the values in `values`. *
*/ public static function endsWithAny(s:String, values:Iterable<String>):Bool return Iterables.any(values, function(end) return s.endsWith(end))
; /** `filter` applies `predicate` character by character to `s` and it returns a filtered version of the string. *
*/ public static function filter(s:String, predicate:String->Bool) return toArray(s).filter(predicate).join('')
...
; return codes.map(function(i:Int) return String.fromCharCode(i)).join('')
...
*/ public static inline function hasContent(value:String):Bool return value != null && value.length > 0
; /** Works the same as `underscore` but also replaces underscores with whitespaces. *
*/ public static function humanize(s:String) return underscore(s).replace('_', ' ')
; /** Checks if `s` contains only (and at least one) alphabetical characters. *
*/ public static function isAlpha(s:String) return s.length > 0 && !IS_ALPHA.match(s)
; /** `isAlphaNum` returns `true` if the string only contains alpha-numeric characters. *
*/ public static inline function isAlphaNum(value:String):Bool #if (php && haxe_ver >= 4.0) return php.Syntax.code('ctype_alnum(
{ 0
} )', value)
; #elseif (php && haxe_ver < 4.0) return untyped __call__("ctype_alnum", value)
; #else return ALPHANUM.match(value)
; #end public static function isBreakingWhitespace(value:String):Bool return !IS_BREAKINGWHITESPACE.match(value)
; /** Returns `true` if the value string is composed of only lower cased characters or case neutral characters. *
*/ public static function isLowerCase(value:String):Bool return value.toLowerCase() == value
; /** Returns `true` if the value string is composed of only upper cased characters or case neutral characters. *
*/ public static function isUpperCase(value:String):Bool return value.toUpperCase() == value
; /** `ifEmpty` returns `value` if it is neither `null` or empty, otherwise it returns `alt` *
*/ public static inline function ifEmpty(value:String, alt:String):String return null != value && "" != value ? value : alt
; /** `isDigitsOnly` returns `true` if the string only contains digits. *
*/ public static inline function isDigitsOnly(value:String):Bool #if (php && haxe_ver >= 4.0) return untyped php.Syntax.code('ctype_digit(
{ 0
} )', value)
; #elseif (php && haxe_ver < 4.0) return untyped __call__("ctype_digit", value)
; #else return DIGITS.match(value)
; #end
/** `isEmpty` returns true if either `value` is null or is an empty string. *
*/ public static function isEmpty(value:String):Bool return value == null || value == ''
; /** Convert first letter in `value` to lower case. *
*/ public static function lowerCaseFirst(value:String):String return value.substring(0, 1).toLowerCase() + value.substring(1)
...
*/ public static function randomSequence(seed:String, length:Int):String return Ints.range(0, length).map(function(_) return random(seed)).join("")
; /** Like `Strings.randomSequence`, but automatically uses `haxe.crypto.Base64.CHARS` as the seed string. *
*/ public static function randomSequence64(length:Int):String return randomSequence(haxe.crypto.Base64.CHARS, length)
...
*/ public static function removeAfter(value:String, toremove:String):String return StringTools.endsWith(value, toremove) ? value.substring(0, value.length - toremove.length) : value
; /** Removes a slice from `index` to `index + length` from `value`. *
*/ public static function removeAt(value:String, index:Int, length:Int):String return value.substring(0, index) + value.substring(index + length)
; /** If present, it removes the `toremove` text from the beginning of `value`. *
*/ public static function removeBefore(value:String, toremove:String):String return StringTools.startsWith(value, toremove) ? value.substring(toremove.length) : value
...
; } /** `repeat` builds a new string by repeating the argument `s`, n `times`. ```haxe 'Xy'.repeat(3)
; // generates 'XyXyXy' ``` *
*/ public static function repeat(s:String, times:Int) return [for (i in 0...times) s].join('')
...
; else return '"' + StringTools.replace(s, '"', '\\"') + '"'
...
*/ public static function startsWithAny(s:String, values:Iterable<String>):Bool return Iterables.any(values, function(start) return s.startsWith(start))
; /** `stripTags` removes any HTML/XML markup from the string leaving only the concatenation of the existing text nodes. *
*/ public static function stripTags(s:String):String #if (php && haxe_ver >= 4.0) return untyped php.Syntax.code('strip_tags(
{ 0
} )', s)
; #elseif (php && haxe_ver < 4.0) return untyped __call__("strip_tags", s)
; #else return STRIPTAGS.replace(s, "")
; #end
/** Surrounds a string with the contents of `left` and `right`. If `right` is omitted, `left` will be used on both sides
; */ inline public static function surround(s:String, left:String, ?right:String) return '$left$s$
...
*/ inline public static function toCharcodes(s:String):Array<Int> return map(s,
...
*/ inline public static function toLines(s:String) return SPLIT_LINES.split(s)
; /** `trimChars` removes from the beginning and the end of the string any character that is present in `charlist`. *
*/ public static inline function trimChars(value:String, charlist:String):String #if (php && haxe_ver >= 4.0) return untyped php.Global.trim('strip_tags(
{ 0
} )', s)
; #elseif (php && haxe_ver < 4.0) return untyped __call__("trim", value, charlist)
; #else return trimCharsRight(trimCharsLeft(value, charlist), charlist)
; #end
...
{ #if (php && haxe_ver >= 4.0) return untyped php.Global.ltrim(value, charlist)
; #elseif (php && haxe_ver < 4.0) return untyped __call__("ltrim", value, charlist)
; #else var pos = 0
; for (i in 0...value.length) if (contains(charlist, value.charAt(i))) pos++
; else break
...
{ #if (php && haxe_ver >= 4.0) return untyped php.Global.rtrim(value, charlist)
; #elseif (php && haxe_ver < 4.0) return untyped __call__("rtrim", value, charlist)
; #else var len = value.length, pos = len, i
; for (j in 0...len)
{ i = len - j - 1
...
*/ public static function upperCaseFirst(value:String):String return value.substring(0, 1).toUpperCase() + value.substring(1)
; /** `upTo` searches for the first occurrance of `searchFor` and returns the text up to that point. If `searchFor` is not found, the entire string is returned. *
*/ public static function upTo(value:String, searchFor:String)
{ var pos = value.indexOf(searchFor)
; if (pos < 0) return value
; else return value.substring(0, pos)
; } /** `wrapColumns` splits a long string into lines that are at most `columns` long. Words whose length exceeds `columns` are not split. *
...
; static function upperMatch(re:EReg) return re.matched(0).toUpperCase()
