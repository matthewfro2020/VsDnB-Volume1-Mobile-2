...
; } /** * Adds a module into the cache. * @param module The module to add.
*/ public static function addModule(module:Module)
{ if (module == null) return
; moduleList.set(module.moduleId, module)
; } /** * Retrieves a module from the cache based on the given id. * @param id The id to get the module for. * @return A `Module`
...
; } /** * Clear every module from the cache. * This'll Calls the `DESTROY` script event before they're destroyed.
*/ public static function clearModules():Void
{ if (moduleList != null && moduleList.size() > 0)
{ var event = new ScriptEvent(DESTROY, false)
; // Call the destroy event on all modules before destroying them. callEvent(event)
; } moduleList.clear()
; moduleList = []
; } /** * Dispatches the given script event to call all modules in the current cache. * @param event The event to dispatch.
*/ public static function callEvent(event:ScriptEvent):Void
{ // Since the module cache is ordered. for (moduleId in moduleSortCache)
{ callOnModule(moduleId, event)
; }
} /** * Dispatches a script event to a specific module with a given id. * @param id The id of the module to call. * @param event The event to dispatch to the module.
...
; } /** * Sorts all modules based on their priority priority. * @param module The first module being compared. * @param module2 The second module being compared. * * @return `Int` telling whether they should be swapped, or not.
