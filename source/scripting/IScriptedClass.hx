...
class that dispatches script events. * Implement this to allow be able to dispatch events to other scripts.
*/ interface IEventDispatcher
{ public function dispatchEvent(event:ScriptEvent):Void
; } /** * An
interface defining a list of base functions for a scripted classes to contain.
...
; /** * Called whenever the user changes a preference. * Override this if you want your
class to do custom behavior if the user ends up changing a preference. * @param event The event dispatched.
*/ public function onPreferenceChanged(event:PreferenceScriptEvent):Void
; } /** * An
interface defining a list of functions for a scripted
class relating to state changing.
...
; } /** * An
interface defining a list of base functions for a scripted
class linked to a conductor.
...
; } /** * An
interface defining a list of functions relating to notes.
