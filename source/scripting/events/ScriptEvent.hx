...
; /** * Whether this event is able to be cancelled by the children it's dispatched to. * When cancelled, the behavior can be customized for what happens for when the event is cancelled.
...
; /** * Creates a new ScriptEvent to be used by scripts. * @param type The type of event this ScriptEvent is. * @param cancelable Whether this script event is cancellable. Automatically false.
...
; }
} /** * A script event that dispatches on update. * This is not cancellable.
*/ class UpdateScriptEvent extends ScriptEvent
{ /** * The time passed since the last frame. * This can not be edited.
...
; }
} /** * A script event dispatched whenever the user changes a preference. * Stores the preference that was changed, and the new value of the preference. * * This script event is NOT cancellable.
...
; /** * Initalizes a new PreferenceScriptEvent. * @param preference The preference that was changed. * @param value The new value.
...
; }
} /** * A script event dispatched that stores information from a Conductor. * While non of these can be changed. This can help to allow the user to do events based on a certain step, beat, etc.
...
// /** * A script event that's attached to a countdown. * This is cancellable. Cancelling this can allowing for functionaility of cancelling the Countdown, stopping it, etc.
...
{ /** * Whether the camera is pointed to the opponent, or not. * This can not be edited.
...
// /** * A script event attached to the information relating to a note. * This is used for events such as when a note is hit, and missed.
*/ class NoteScriptEvent extends ScriptEvent
{ /** * The note sprite associated with this event. * This can not be edited.
*/ public var note(default, null):Note
; /** * The character that is being sung from this note. * This can not be edited.
*/ public var character(default, null):Character
; /** * The amount the health will change for this note event. * If this is an note miss, this value will be negative, a note hit script event will have this be positive. * * This can be edited by scripts to change the amount of health gained/loss for a specific character, song, etc lol.
*/ public var healthChange:Float
; /** * The current combo as this event was dispatched. * On a note hit event, this'll be the current combo incremented, on a miss event this'll be the combo before being reset. * * This can not be edited.
*/ public var comboCount(default, null):Int
; /** * The sound that plays when missed (used for the note miss script event). * This can allow to customize the miss sound through a script.
...
; }
} /** * A script event that's dispatched whenever the player misses from pressing a key when there's no notes in place. * This event IS cancellable. Cancelling this event will allow for the player to not miss from a ghost note.
...
// /** * A script event that's dispatched whenever a stage adds a sprite to the stage. * Can be used to further initalize a prop after it's added.
*/ class AddPropScriptEvent extends ScriptEvent
{ /** * The sprite associated with this script event. * This is can not be edited.
