...
; super()
; } /** * Loads, and returns this sound object based on the given sound asset. Synonym for `loadEmbedded` from FlxSound. * @param embeddedSound The sound asset to load. * @param looped Whether this sound should loop. * @param autoDestroy Should the sound destroy when completed? * @param onComplete Called when the audio is finished. * @return -> Void):GameSound
...
; return this
; } // Override to take into account the user volume preferences. override function updateTransform():Void
...
; } _transform.volume = #if FLX_SOUND_SYSTEM (FlxG.sound.muted ? 0 : 1) * FlxG.sound.volume * #end (group != null ? group.volume : 1) * _volume * _volumeAdjust * volumeMultiplier
