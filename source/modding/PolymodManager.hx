...
; /** * The current API version of the game of the game. * Should be incremented in accordance to semantic versioning, in case any changes to the game's modding API happen.
*/ public static var API_VERSION:Version = '1.0.2'
; /** * The version rule of this mod's API. * Used scanning and checking for mods for this mod's API. A mod's version will need to be in accordance to the rule, else it won't be loaded.
...
; // Imports need to be built first before configuring Polymod, or else you'll get a crash. buildImports()
; // Configure polymod. loadAllMods()
...
; } /** * Loads a list of mods by their directory id name. * @param ids The list of mod directorys to load.
*/ public static function loadModsById(ids:Array<String>)
{ createModRoot()
; loadedMods = Polymod.init(
{ modRoot: MOD_FOLDER, dirs: ids, framework: OPENFL, frameworkParams: buildFrameworkParams(), errorCallback: PolymodErrorHandler.printError, apiVersionRule: API_VERSION_RULE, useScriptedClasses: true,
} )
...
; loadModsById([])
; } /** * Retrieves a list of all mods, and their metadata from the game's mod folder. * @return An `Array<ModMetadata>`
*/ public static function getAllMods():Array<ModMetadata>
{ createModRoot()
; var mods = Polymod.scan(
{ modRoot: MOD_FOLDER, apiVersionRule: API_VERSION_RULE, errorCallback: PolymodErrorHandler.printError,
} )
; if (mods.length == 0)
...
{ trace('Polymod found $
{ mods.length
} mod(s) while scanning.')
; } return mods
; } /** * Retrieves the ids of all mods found within the game's mod folder.
*/ public static function getAllModIds():Array<String>
{ return [for (mod in getAllMods()) mod.id]
; } /** * Builds the file system used for Polymod. * @return A `SysFileSystem`
*/ public static function buildFileSystem():SysFileSystem
{ return new SysFileSystem(
{ modRoot: MOD_FOLDER,
} )
; } /** * Builds the framework parameters used for Polymod. * @return A `FrameworkParams` structure.
*/ public static function buildFrameworkParams():FrameworkParams
{ return
{ assetLibraryPaths: [ 'default' => 'preload', 'shared' => 'shared', 'songs' => 'songs' ], coreAssetRedirect: null
}
} static function buildParseRules():polymod.format.ParseRules
{ var output:polymod.format.ParseRules = polymod.format.ParseRules.getDefault()
; // Ensure TXT files have merge support. output.addType('txt', TextFileFormat.LINES)
; // Ensure script files have merge support. output.addType('hscript', TextFileFormat.PLAINTEXT)
; output.addType('hxs', TextFileFormat.PLAINTEXT)
; output.addType('hxc', TextFileFormat.PLAINTEXT)
; output.addType('hx', TextFileFormat.PLAINTEXT)
; return output
; } /** * Clears, and re-loads all scripts from the disk. * Useful for hot reloading so you don't have to constantly re-open the program to test out changes.
*/ public static function reloadAssets():Void
{ // Clear every script from the cache. ModuleHandler.clearModules()
; SongModuleHandler.clearModules()
; SongModuleHandler.clearModuleCache()
; // Forcibly re-load all mods, this re-registers scripted classes and updates any assets. loadAllMods()
; // Re-load all entries to include new data/scripted classes. data.character.CharacterRegistry.instance.loadEntries()
...
; // Blacklist powerful/dangerous classes. Polymod.blacklistImport(Type.getClassName(Sys))
; // Blacklist misc. classes. Polymod.blacklistImport('haxe.Unserializer')
...
; // Blacklist HScript classes. for (cls in ClassMacro.listClassesInPackage('hscript'))
{ if (cls == null) continue
; var className:String = Type.getClassName(cls)
; Polymod.blacklistImport(className)
; } // Blacklist any Polymod related classes. for (cls in ClassMacro.listClassesInPackage('polymod'))
{ if (cls == null) continue
; var className:String = Type.getClassName(cls)
; Polymod.blacklistImport(className)
...
; /** * HAXE SPECIFIC
*/
// Add default imports to normal Haxe classes. Polymod.addDefaultImport(Date)
