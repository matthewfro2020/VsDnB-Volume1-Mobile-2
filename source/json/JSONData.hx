...
{ return []
; } /** * Returns a value by specified `key`. * Can be used like `jsonData["key"]` * @return The value, or the defualt if the key is not present. * Can return `null` if the value was present but null.
*/ @:arrayAccess public inline function get(key:String, ?defaultValue:Dynamic):Null<Dynamic>
{ if (!exists(key)) return defaultValue
; #if js var result = untyped this[key]
; // we know it's an object, so we don't need a check #else var result = isObject() ? get_obj(key) : get_arr(key)
; #end return result
; } inline function get_obj(key:String):Null<Dynamic>
{ return Reflect.field(this, key)
; } inline function get_arr(key:String):Null<Dynamic>
{ // Only numeric keys are allowed in arrays var index:Null<Int> = Std.parseInt(key)
...
// but we got a string! That's not allowed. throw 'get(): bad array index: $
{ part.toString()
} '
; } else
...
; if (element == null)
{ if (pathParts.length == 1) return element
; throw 'K:/$
...
; }
} /** * Sets a `value` for a specified `key`. * Can be used like `jsonData["key"] = value` * @return The value provided, or `null` if the key is not present.
*/ @:arrayAccess public inline function set(key:String, value:Dynamic):Dynamic
{ #if js return untyped this[key] = value
; #else return isObject() ? set_obj(key, value) : set_arr(key, value)
; #end
...
{ // Only numeric keys are allowed in arrays var index:Null<Int> = Std.parseInt(key)
; if (index == null) throw 'Could not parse array index $
{ key
} '
; this[index] = value
; return value
...
// but we got a string! That's not allowed. throw 'set(): bad array index: $
{ part.toString()
} '
; } else
...
; if (index == null)
{ if (key == '-')
{ // See RFC 6901 this.insert(this.length, value)
; return value
; } else
{ throw 'Could not parse array index $
{ key
} '
; }
} else
...
{ if (part.isString())
{ if (isArray())
{ // `-` inserts into the end of the array, see RFC 6901 if (part.toString() == '-')
{ return insert_arr('-', value, strict)
; } else
{ // This is an array, so we should insert by index,
// but we got a string! That's not allowed. throw 'insert(): bad array index: $
{ part.toString()
} '
; }
} else
...
; throw e
; }
} /** * Tells if the data contains a specified `key`. * @return `true` if the key is present, `false` otherwise.
...
// If we check if the value is non-null, we get a false negative if the array CONTAINS nulls. return this.length > index
; } function existsByPart(part:PathPart):Bool
{ if (part.isString())
{ if (isArray())
{ // This is an array, so we should query by index,
// but we got a string! That's not allowed. throw 'exists(): bad array index: $
{ part.toString()
} '
; } else
...
{ if (pathParts.length == 0) throw 'No path provided'
; if (pathParts.length == 1)
...
; } /** * Removes a specified `key` in the data. * @return `true` if `key` was present in structure, or `false` otherwise.
...
// but we got a string! That's not allowed. throw 'remove(): bad array index: $
{ part.toString()
} '
; } else
...
{ if (pathParts.length == 0) throw 'No path provided'
; if (pathParts.length == 1) return removeDataByPart(pathParts[0])
; var element = getDataByPart(pathParts[0])
; if (element == null)
