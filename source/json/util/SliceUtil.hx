...
; // ERIC: Since `end` is exclusive, had to offset the >=0 case by 1 compared to the spec. if (end == null) end = (step >= 0) ? (len) : (-len - 1)
; // Normalize if (start < 0) start = len + start
...
; if (step > 0)
{ if (step > len)
{ // Edge case: large positive step result.push(lower)
; } else
{ // Standard case: positive step var i = lower
; while (i < upper)
{ result.push(i)
; i += step
; }
...
{ if (step < -len)
{ // Edge case: large negative step result.push(upper)
; } else
{ // Standard case: negative step var i = upper
; while (lower < i)
{ result.push(i)
; i += step
; }
