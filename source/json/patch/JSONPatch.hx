...
// datatracker.ietf.org/doc/rfc6902/ * @return The resulting JSON data.
...
; } return result
; } /** * For the given JSON data, apply a single JSONPatch operation. * @param data A JSON data object. You can pass `Dynamic` or `Array<Dynamic>` here. * @param patch A JSONPatch operation to perform. * @see https:
// datatracker.ietf.org/doc/rfc6902/ * @return The resulting JSON data.
...
{ case "add": result = applyOperation_add(result, operation.get('path'), operation.get('value', NoValue))
; case "remove": result = applyOperation_remove(result, operation.get('path'))
; case "replace": result = applyOperation_replace(result, operation.get('path'), operation.get('value', NoValue))
; case "move": result = applyOperation_move(result, operation.get('from'), operation.get('path'))
; case "copy": result = applyOperation_copy(result, operation.get('from'), operation.get('path'))
; case "test": result = applyOperation_test(result, operation.get('path'), operation.get('value', NoValue))
; default: throw 'Unsupported operation "$
{ operation.get('op')
} ", expected one of "test", "add", "replace", "remove", "move", "copy"'
; } return result
...
// If target specifies an object member that does exist, that member's value is replaced var targetPaths = parsePaths(path, data)
...
// If target is an array index, the value is removed and other elements are shifted var targetPaths = parsePaths(path, data)
; for (targetPath in targetPaths)
...
// Then, add the value at the path location var targetFromPaths = parsePaths(from, data)
; var targetPaths = parsePaths(path, data)
; for (targetFromPath in targetFromPaths)
{ if (!data.existsByPath(targetFromPath))
{ throw 'no element at from path $
{ targetFromPath
} '
...
// Otherwise, an error is thrown var targetPaths = parsePaths(path, data)
; for (targetPath in targetPaths)
{ try
{ if (!data.existsByPath(targetPath))
...
{ // Parse a JSONPath string if (path.startsWith('$')) return JSONPath.queryPaths(path, data)
; // Parse a JSONPointer string. return [JSONPointer.toJSONPath(path)]
; }
} enum NoValue
{ NoValue
; }
