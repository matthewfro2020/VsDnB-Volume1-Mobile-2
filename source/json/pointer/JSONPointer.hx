...
{ case "0": result += "~"
; case "1": result += "/"
; default: throw 'Expected 0 or 1 at position $
{ readPos-1
} , got $
{ escaped
} '
...
{ // TODO: Are exponents really bad numbers? var result = Std.parseInt(input)
; if (result != null)
...
; var charStr = popStr()
; switch (char)
{ case SLASH: var token = popReferenceToken()
; if (token.length > 0)
{ if (toInt(token) != null)
{ result += "[" + token + "]"
; } else
...
{ result += "['']"
; } default: throw 'Expected "/" at position $
{ readPos-1
} , got $
{ charStr
} '
