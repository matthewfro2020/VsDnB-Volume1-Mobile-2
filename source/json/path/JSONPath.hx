...
*/ class JSONPath
{ /** * Performs the provided JSONPath query on the query argument, * and provides the query result as a list of JSON values that were located. * @param path * @param value * @return A list of JSON data values.
...
; } /** * Performs the provided JSONPath query on the query argument, * and provides the query result as a list of normalized paths into the query argument. * @param path The JSONPath query to perform * @param value The value to query * @param allowNewPaths Whether to create new paths for non-filter selectors * For example, `$.a.b` will return `$['a']['b']` even if `a` or `b` do not exist. * @return A list of JSON normalized paths.
*/ public static function queryPaths(path:String, value:JSONData, allowNewPaths:Bool = false):Array<String>
{ var element = new JSONPathParser().parse(path)
; switch (element)
{ case Element.JSONPathQuery(segments): var nodelist = queryPaths_Element(element, value, allowNewPaths)
; return mapNodelistPaths(nodelist)
; default: throw pathError_noRootIdentifier(element)
; }
...
{ switch (element)
{ case Element.JSONPathQuery(segments): var nodeList:Array<JSONNode> = [
{ path: '$', value: rootValue
} ]
; for (segment in segments)
{ switch (segment)
{ case ChildSegment(selectors): nodeList = queryPaths_ChildSegment(selectors, nodeList, rootValue, allowNewPaths)
; case DescendantSegment(selectors): nodeList = queryPaths_DescendantSegment(selectors, nodeList, rootValue, allowNewPaths)
; default: throw pathError_unexpectedElement(segment)
; }
} return nodeList
; case Element.RelativeQuery(segments): var nodeList:Array<JSONNode> = [
{ path: '@', value: rootValue
} ]
; for (segment in segments)
{ switch (segment)
...
{ switch (element)
{ case Element.JSONPathQuery(_): return queryPaths_Element(element, rootValue, allowNewPaths)
; case Element.RelativeQuery(_): return queryPaths_Element(element, targetValue, allowNewPaths)
; default: throw 'Expected relative or absolute query, got $
{ element
} '
; }
...
{ switch (selector)
{ case NameSelector(name): for (node in nodeList)
{ if (node.value.isArray()) continue
; if (!allowNewPaths && node.value == null) continue
; if (!allowNewPaths && !node.value.exists(name)) continue
; var newPath = node.path + "['" + name + "']"
; result.push(
{ path: newPath, value: node.value.get(name)
} )
; } case IndexSelector(index): for (node in nodeList)
{ if (!node.value.isArray()) continue
; if (index < 0) index = node.value.length() + index
; // Index out of bounds, provide no result. if (index < 0 || (index >= node.value.length() && !allowNewPaths)) continue
; if (!allowNewPaths && !node.value.exists('$index')) continue
; var newPath = node.path + "[" + index + "]"
; result.push(
{ path: newPath, value: node.value.get('$index')
} )
; } case WildcardSelector: for (node in nodeList)
{ if (node.value.isPrimitive()) continue
; var isArray = node.value.isArray()
; var keys = node.value.keys()
; for (key in keys)
{ var newPath = node.path + (isArray ? "[" + key + "]" : "['" + key + "']")
; result.push(
{ path: newPath, value: node.value.get(key)
} )
; }
} case ArraySliceSelector(start, end, step): for (node in nodeList)
{ if (!node.value.isArray()) continue
...
; }
} case FilterSelector(filter): for (node in nodeList)
{ var results = queryPaths_ChildFilterSelector(filter, node, rootValue)
; result = result.concat(results)
; } default: throw pathError_unexpectedElement(selector)
; }
} return result
; } static function queryPaths_ChildFilterSelector(filter:Element, targetNode:JSONNode, rootValue:JSONData):Array<JSONNode>
{ // Calculate a list of all the child elements of the target node that match the filter. if (filter == null || targetNode.value == null || targetNode.value.isPrimitive()) return []
; var result = []
; switch (filter)
{ case LogicalOrExpr(values): for (value in values)
{ var subResult = queryPaths_ChildFilterSelector(value, targetNode, rootValue)
; // TODO: Does this break queries with duplicate values in? if (result.length == 0)
{ result = subResult
; } else
{ result = result.concat(ArrayUtil.subtract(subResult, result))
; }
} case LogicalAndExpr(values): var hasFirstResult = false
; for (value in values)
{ var subResult = queryPaths_ChildFilterSelector(value, targetNode, rootValue)
; if (!hasFirstResult)
{ hasFirstResult = true
...
; }
} case LogicalNotExpr(value): var keys = targetNode.value.keys()
...
; var childNode =
{ path: childPath, value: childValue
}
; subResultAll.push(childNode)
; } var subResult = queryPaths_ChildFilterSelector(value, targetNode, rootValue)
; result = ArrayUtil.subtract(subResultAll, subResult)
; case LogicalTestQueryExpr(value): switch (value)
{ case FilterQuery(value):
// Check for existance of the query result. var queryResult = queryPaths_TestFilterQuery(value, targetNode, rootValue)
; result = result.concat(queryResult)
; case FunctionExpressionElement(name, args): var queryResult = queryPaths_TestFunctionExpression(name, args, targetNode, rootValue)
; result = result.concat(queryResult)
; default: throw pathError_unexpectedElement(filter)
; } case LogicalComparisionExpr(value1, op, value2): var subResult = queryPaths_LogicalComparision(value1, op, value2, targetNode, rootValue)
...
; // Evaluate the left and right sides of the comparison to primitives, then compare them. var leftValue:PrimitiveLiteral = queryPaths_Comparable(left, childNode, rootValue)
...
; } /** * Evaluate the expression and evaluate it to a literal. * If the expression evaluates to a node, return a BoolLiteral for whether it exists. * If the expression evaluates to a value, return a Literal for that value.
...
{ switch (expression)
{ case LogicalTestQueryExpr(element): return queryPaths_Comparable(element, targetNode, rootValue, asNodelist)
; case PrimitiveLiteralExpr(value): return value
; case FunctionExpressionElement(name, arguments): return queryPaths_FunctionExpression_Value(name, arguments, targetNode, rootValue)
; case FilterQuery(value): var subResult = queryPaths_ValueFilterQuery(value, targetNode, rootValue)
...
; } default: throw pathError_unexpectedElement(expression)
; }
...
; } /** * Return each node which matches the subquery. * For example, [@.b] returns each node which contains `b`
*/ static function queryPaths_TestFilterQuery(subquery:Element, targetNode:JSONNode, rootValue:JSONData):Array<JSONNode>
{ // Perform a filter query on the child elements of the target node. var keys = targetNode.value.keys()
; var isArray = targetNode.value.isArray()
; var results:Array<JSONNode> = []
; for (key in keys)
{ var childValue = targetNode.value.get(key)
...
; switch (subResult)
{ case BooleanLiteral(value): if (value)
{ results.push(childNode)
; } default:
// Do nothing
}
} return results
; } /** * Return each value for the subquery. * For example, [@.b] returns each value of `b`
...
; for (selector in selectors)
{ switch (selector)
{ case NameSelector(name): for (node in fullList)
{ var newPath = node.path + "['" + name + "']"
; if (node.value.isArray()) continue
; var pathValue = node.value.get(name)
; if (pathValue == null) continue
; result.push(
{ path: newPath, value: pathValue
} )
; } case IndexSelector(index): for (node in fullList)
{ if (!node.value.isArray()) continue
; if (index < 0) index = node.value.length() + index
; // Index out of bounds, provide no result. if (index < 0 || index >= node.value.length()) continue
; var newPath = node.path + "[" + index + "]"
...
; } case WildcardSelector: for (node in fullList)
{ var isArray = node.value.isArray()
; var keys = node.value.keys()
; for (key in keys)
{ var newPath = node.path + (isArray ? "[" + key + "]" : "['" + key + "']")
...
; }
} case ArraySliceSelector(start, end, step): for (node in fullList)
{ if (!node.value.isArray()) continue
; var indices = SliceUtil.getSliceIndices(node.value.length(), start, end, step)
; for (index in indices)
{ var newPath = node.path + "[" + index + "]"
...
; }
} case FilterSelector(filter): for (node in fullList)
{ var results = queryPaths_ChildFilterSelector(filter, node, rootValue)
; result = result.concat(results)
; } default: throw pathError_unexpectedElement(selector)
; }
...
{ var isArray = node.value.isArray()
; children.push(
{ path: node.path + (isArray ? "[" + key + "]" : "['" + key + "']"), value: node.value.get(key)
} )
...
{ result.push(
{ path: relativeNode.path.replace('@', rootNode.path), value: relativeNode.value
} )
; } return result
; } static function pathError_noRootIdentifier(element:Element):String
{ return 'JSONPath must start with root identifier "$", but got: $
{ element
} '
...
{ return 'Got multiple results for singular query: $
{ nodes
} '
...
; // ]
/** * Split a normalized path $['a']['b']['c'][1] into ['a', 'b', 'c', '1'] * @returns An array of path parts, either string identifiers or integer array indices
...
{ return 'Invalid normalized path: $
{ path
} '
; } static function npathError_unexpectedChar(token:String):String
{ return 'Unexpected character in normalized path: $
{ token
} '
; }
} typedef JSONNode =
...
*/ var value:JSONData
; } enum Element
{ /** * A JSONPath query consists of a root identifier followed by * a sequence of segments (possibly empty) with optional blank space between. * * Each segment takes in an input node list (the first segment takes in the query value) * and produces an output node list, which the next segment operates on in turn.
...
// Input can be one of: FilterQuery, FunctionExpression LogicalTestQueryExpr(element:Element)
...
{ var token = popToken()
; switch (token)
{ case Token.Dollar: return Element.JSONPathQuery(consumeTokens_Segment())
; default: throw parserError_unexpectedToken_rootSelector(token)
; }
} function consumeToken_RelativeQuery():Element
{ var token = popToken()
; switch (token)
{ case Token.At: return Element.RelativeQuery(consumeTokens_Segment())
; default: throw parserError_unexpectedToken(token)
; }
} function consumeTokens_Segment():Array<Element>
{ var result:Array<Element> = []
; while (!isEnd())
{ switch (peekToken())
{ case Whitespace: var token = popToken()
; continue
; // Child Segments case Brackets(values): var token = popToken()
; result.push(Element.ChildSegment(consumeTokens_BracketedSelection(values)))
; case Dot: var token = popToken()
; result.push(Element.ChildSegment([consumeTokens_DotSelection()]))
; // Descendant Segments case DoubleDot: var token = popToken()
; switch (peekToken())
{ case Brackets(values): var token = popToken()
; result.push(Element.DescendantSegment(consumeTokens_BracketedSelection(values)))
; case Asterisk: var token = popToken()
; result.push(Element.DescendantSegment([Element.WildcardSelector]))
; case MemberName(name): var token = popToken()
; result.push(Element.DescendantSegment([Element.NameSelector(name)]))
; case StringLiteral(name): var token = popToken()
; // TODO: Handle $.."key"?
// result.push(Element.DescendantSegment([Element.NameSelector(name)]))
; throw parserError_unexpectedToken_doubleDotSelector_stringLiteral(name)
; default: throw parserError_unexpectedToken_doubleDotSelector(peekToken())
; } default:
...
{ var token = popToken()
; switch (token)
{ case Asterisk: return Element.WildcardSelector
; case MemberName(name): return Element.NameSelector(name)
; case IntegerLiteral(value):
// Handle $.2 as $['2'] return Element.NameSelector('$value')
; case StringLiteral(value):
// TODO: Handle $."key"?
// return Element.NameSelector(value)
; throw parserError_unexpectedToken_dotSelector_stringLiteral(value)
; default: throw parserError_unexpectedToken_dotSelector(token)
; }
...
// NameSelector result.push(Element.NameSelector(name))
; case MemberName(name):
// NameSelector result.push(Element.NameSelector(name))
; case Asterisk:
// WildcardSelector result.push(Element.WildcardSelector)
; case IntegerLiteral(number):
// Index or ArraySlice
// Look at the token after. switch (peekNonWhitespaceToken())
{ case Colon:
// ArraySlice result.push(consumeTokens_ArraySliceSelector(token))
; default:
// Index result.push(Element.IndexSelector(number))
; } case Colon:
// ArraySlice result.push(consumeTokens_ArraySliceSelector(token))
; case Question: result.push(consumeTokens_FilterSelector())
; default: throw parserError_unexpectedToken(token)
; } // Skip whitespace. popWhitespace()
; // See if there's a next token. switch (peekToken())
{ case Comma: var token = popToken()
; continue
; case null: break
; default: throw parserError_unexpectedToken(peekToken())
; }
} return result
...
; if (firstToken != null)
{ switch (firstToken)
{ case IntegerLiteral(number): start = number
; case Colon: emptyStart = true
; case NumberLiteral(value): throw parserError_unexpectedToken(firstToken)
; default: throw parserError_unexpectedToken(firstToken)
; }
} popWhitespace()
; if (!emptyStart)
{ var token = popToken()
...
; var token = peekToken()
; switch (token)
{ case IntegerLiteral(number): popToken()
; end = number
; default:
// Do nothing.
} popWhitespace()
...
{ switch (element)
{ case Element.LogicalOrExpr(elements): if (elements.length == 1)
...
; } case Element.LogicalAndExpr(elements): if (elements.length == 1)
{ return consumeTokens_cleanupFilterSelector(elements[0])
; } else
{ var result:Array<Element> = []
; for (element in elements)
...
{ popWhitespace()
; switch (peekToken())
{ case Parens(values): var token = popToken()
; var subParser = new JSONPathParser()
; @:privateAccess
{ subParser.tokens = values
...
{ throw parserError_unexpectedToken(subParser.peekToken())
; } return result
; } case LogicalNot: var token = popToken()
; popWhitespace()
; switch (peekToken())
{ case Parens(values): var token = popToken()
; var subParser = new JSONPathParser()
...
; } case MemberName(_): return Element.LogicalNotExpr(consumeTokens_logicalBasicExpr())
; case Dollar: return Element.LogicalNotExpr(consumeTokens_logicalBasicExpr())
; case At: return Element.LogicalNotExpr(consumeTokens_logicalBasicExpr())
; default: throw parserError_unexpectedToken(peekToken())
; } case Dollar: return consumeTokens_comparisonOrTest()
; case At: return consumeTokens_comparisonOrTest()
; case StringLiteral(value): return consumeTokens_comparisonOrTest()
; case NumberLiteral(value): return consumeTokens_comparisonOrTest()
; case IntegerLiteral(value): return consumeTokens_comparisonOrTest()
; case MemberName(name): return consumeTokens_comparisonOrTest()
; default: throw parserError_unexpectedToken(peekToken())
; }
...
; switch (peekToken())
{ case StringLiteral(value): popToken()
; left = PrimitiveLiteralExpr(StringLiteral(value))
; case NumberLiteral(value): popToken()
; left = PrimitiveLiteralExpr(NumberLiteral(value))
; case IntegerLiteral(value): popToken()
; left = PrimitiveLiteralExpr(IntegerLiteral(value))
; case Dollar: left = Element.FilterQuery(consumeToken_JSONPathQuery())
; case At: left = Element.FilterQuery(consumeToken_RelativeQuery())
; case MemberName(name): switch (name)
{ case "true": left = Element.PrimitiveLiteralExpr(BooleanLiteral(true))
; case "false": left = Element.PrimitiveLiteralExpr(BooleanLiteral(false))
; case "null": left = Element.PrimitiveLiteralExpr(NullLiteral)
; default: if (FunctionExpression.isValidFunctionExpression(name))
{ var token = popToken()
...
; var token = peekToken()
; switch (token)
{ case Comparison(op): var token = popToken()
; return Element.LogicalComparisionExpr(left, op, consumeToken_comparable())
; case Comma: return Element.LogicalTestQueryExpr(left)
; case null: return Element.LogicalTestQueryExpr(left)
; default: return Element.LogicalTestQueryExpr(left)
; // throw parserError_unexpectedToken(token)
; }
} function consumeToken_comparable():Element
{ switch (peekToken())
{ case Dollar:
// Singular query return Element.FilterQuery(consumeToken_JSONPathQuery())
; case At:
// Singular query return Element.FilterQuery(consumeToken_RelativeQuery())
; case IntegerLiteral(number):
// Integer var token = popToken()
; return Element.PrimitiveLiteralExpr(IntegerLiteral(number))
; case NumberLiteral(number):
// Float var token = popToken()
; return Element.PrimitiveLiteralExpr(NumberLiteral(number))
; case StringLiteral(value):
// String var token = popToken()
...
; default: throw parserError_unexpectedToken_comparable(peekToken())
; }
...
{ if (readPos + index >= tokens.length) return null
...
*/ class JSONPathLexer
{ static final BACKSPACE:Int = 0x08
; // \b static final TAB:Int = 0x09
; // \t static final NEWLINE:Int = 0x0A
; // \n static final FORMFEED:Int = 0x0C
; // \f static final CARRIAGE:Int = 0x0D
; // \r static final SPACE:Int = 0x20
...
; // ^ static final UNDERSCORE:Int = 0x5F
; // _ static final LBRACE:Int = 0x7B
; //
{ static final BAR:Int = 0x7C
; // | static final RBRACE:Int = 0x7D
; //
} // lowercase static final B:Int = 0x62
; // b static final D:Int = 0x64
; // d static final E:Int = 0x65
; // e static final F:Int = 0x66
; // f static final N:Int = 0x6E
; // n static final R:Int = 0x72
; // r static final T:Int = 0x74
; // t static final U:Int = 0x75
; // u
// uppercase static final A_U:Int = 0x41
; // A static final B_U:Int = 0x42
; // B static final C_U:Int = 0x43
; // C static final D_U:Int = 0x44
; // D static final E_U:Int = 0x45
; // E static final F_U:Int = 0x46
; // F
// digits static final ZERO:Int = 0x30
; // 0 static final NINE:Int = 0x39
; // 9 static final DIGIT:Array<Int> = [0x30, 0x31, 0x32, 0x33, 0x34, 0x35, 0x36, 0x37, 0x38, 0x39]
; static final HEXDIG:Array<Int> = [ 0x30, 0x31, 0x32, 0x33, 0x34, 0x35, 0x36, 0x37, 0x38, 0x39,
// Uppercase 0x41, 0x42, 0x43, 0x44, 0x45, 0x46,
// Lowercase 0x61, 0x62, 0x63, 0x64, 0x65, 0x66 ]
; // The input string var input:String
; // The current read position var readPos:Int
; var readLine:Int
; var readCol:Int
; // The position of the current token. var tokenMin:Int
; var tokenMax:Int
; // The current token list var tokens:Array<Token>
; public function new()
...
{ switch (peekChar())
{ case DOLLAR: var char = popChar()
; return Token.Dollar
; case ASTERISK: var char = popChar()
; return Token.Asterisk
; case COLON: var char = popChar()
; return Token.Colon
; case COMMA: var char = popChar()
; return Token.Comma
; case QUESTION: var char = popChar()
; return Token.Question
; case AT: var char = popChar()
; return Token.At
; case PERIOD: return readToken_distinguish_period()
; case GREATER | LESS | EQUALS: return readToken_comparison()
; case TAB | NEWLINE | CARRIAGE | SPACE: return readToken_whitespace()
; case SINGLE_QUOTE: return readToken_stringLiteral(true)
; case DOUBLE_QUOTE: return readToken_stringLiteral(false)
; case MINUS: return readToken_numberLiteral()
; case BAR: return readToken_logicalOr()
; case AMPERSAND: return readToken_logicalAnd()
; case EXCLAMATION: return readToken_distinguish_exclamation()
; case isDigit(_) => true: return readToken_numberLiteral()
; case LPAREN: return readToken_parens()
; case LBRACE: return readToken_braces()
; case LBRACKET: return readToken_brackets()
; case RPAREN: throw formatError_UnexpectedChar(')')
; case RBRACE: throw formatError_UnexpectedChar('
} ')
; case RBRACKET: throw formatError_UnexpectedChar(']')
; case isNameFirst(_) => true: return readToken_memberName()
; default: throw formatError_UnexpectedChar(String.fromCharCode(peekChar()))
; }
...
{ // Consume the opening paren var char = popChar()
; var tokens = readToken_consumeContents(RPAREN)
; // Consume the closing paren var char = popChar()
...
; // Read tokens until we find the closing paren var tokens = []
; while (!eof() && peekChar() != expected)
{ var tk = readToken()
; tokens.push(tk)
; } if (eof()) throw formatError_Unclosed(String.fromCharCode(expected), startPos)
; return tokens
; } #if !debug inline #end function readToken_comparison():Token
{ var char = popChar()
; switch (char)
{ case GREATER: if (peekChar() == EQUALS)
{ var char = popChar()
; return Token.Comparison('>=')
; } else
{ return Token.Comparison('>')
; } case LESS: if (peekChar() == EQUALS)
{ var char = popChar()
; return Token.Comparison('<=')
; } else
{ return Token.Comparison('<')
; } case EQUALS: if (!(peekChar() == EQUALS)) throw formatError_UnexpectedChar(String.fromCharCode(peekChar()))
; var char = popChar()
; return Token.Comparison('==')
; default: throw formatError_UnexpectedChar(String.fromCharCode(char))
; }
} #if !debug inline #end function readToken_logicalOr():Token
{ var char = popChar()
; if (eof() || peekChar() != BAR) throw formatError_UnexpectedChar(String.fromCharCode(char))
; var char = popChar()
; return Token.LogicalOr
; } #if !debug inline #end function readToken_logicalAnd():Token
{ var char = popChar()
; if (eof() || peekChar() != AMPERSAND) throw formatError_UnexpectedChar(String.fromCharCode(char))
; var char = popChar()
; return Token.LogicalAnd
; } #if !debug inline #end function readToken_whitespace():Token
{ // Read until we find a non-whitespace character var char = peekChar()
...
{ if (char == PERIOD || char == E || char == E_U || char == PLUS) isFloat = true
; result += readToken_unescaped(false)
; char = peekChar()
; } if (isFloat)
{ var num = Std.parseFloat(result)
; if (Math.isNaN(num)) throw formatError_InvalidNumber(result)
; return Token.NumberLiteral(num)
; } else
{ var num = Std.parseInt(result)
; if (num == null) throw formatError_InvalidNumber(result)
; return Token.IntegerLiteral(num)
; }
} #if !debug inline #end function readToken_memberName():Token
{ var result = ''
; var char = popChar()
; if (!isNameFirst(char)) throw formatError_UnexpectedChar(String.fromCharCode(char))
; result += String.fromCharCode(char)
; var char = peekChar()
; // Allow - and digits in member names but not first character. while (!eof() && (isNameFirst(char) || isDigit(char) || char == MINUS))
{ result += readToken_unescaped(false)
; char = peekChar()
...
{ if (eof()) throw formatError_UnexpectedEnd()
; var char = popChar()
; // Exclude control characters if (char < 0x20) throw formatError_UnexpectedChar(String.fromCharCode(char))
; // Exclude surrogate code points if (char > 0xD7FF && char < 0xE000) throw formatError_UnexpectedChar(String.fromCharCode(char))
; // Exclude very high Unicode characters if (char > 0x10FFFF) throw formatError_UnexpectedChar(String.fromCharCode(char))
; // Exclude quotes and backslash if (char == ESCAPE) throw formatError_UnexpectedChar(String.fromCharCode(char))
; if (!allowQuotes && char == SINGLE_QUOTE) throw formatError_UnexpectedChar(String.fromCharCode(char))
; if (!allowQuotes && char == DOUBLE_QUOTE) throw formatError_UnexpectedChar(String.fromCharCode(char))
; return String.fromCharCode(char)
; } #if !debug inline #end function readToken_escapable():String
{ var char = popChar()
; // Handle escape sequences switch (char)
{ case SINGLE_QUOTE | DOUBLE_QUOTE | ESCAPE | SLASH: return String.fromCharCode(char)
; case B: return String.fromCharCode(BACKSPACE)
; case F: return String.fromCharCode(FORMFEED)
; case N: return String.fromCharCode(NEWLINE)
; case R: return String.fromCharCode(CARRIAGE)
; case T: return String.fromCharCode(TAB)
; case U: return readToken_hexchar()
; default: throw formatError_UnexpectedChar(String.fromCharCode(char))
; }
...
; while (true)
{ if (eof()) throw formatError_UnexpectedEnd()
; if (HEXDIG.indexOf(peekChar()) == -1) break
...
{ // High surrogate if (peekChar() == ESCAPE && peekChar(1) == U)
{ popChar()
; popChar()
...
{ // Low surrogate return '$
{ hexStr
} '
; } else if (hexCode > 0xFFFF)
{ // Unicode code point out of range throw formatError_UnsupportedUnicode(hexStr)
; } else
{ // Normal code point return String.fromCharCode(hexCode)
; }
} //
// INPUT HANDLERS
...
{ if (isReserved(char)) return false
...
{ return 'Unexpected end of input at pos $
{ readPos
} '
