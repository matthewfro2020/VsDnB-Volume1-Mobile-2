...
; }
} /** * Displays an error message to the Lime Window before the application closes. * @param errorMessage The error messasge to display.
...
; } /** * Creates a message that provides details of the error message, and current state of the game during the crash. * @param e The uncaught error that's being reported. * @return A String containing the crash message.
...
; #elseif 32 buildType = "32 Bit"
; #elseif release buildType = "Release"
; #end var buildNumber:String = Application.current.meta.get("build")
; var haxeLibs:Array<String> = []
; // TODO: Find a way to get the project's haxelibs & versions. var currentStateName:String = Type.getClassName(Type.getClass(FlxG.state))
; // ERROR INFORMATION
// var errorMessage:String = e.error
; var callStackInfo:Array<String> = generateCallstackInfo()
...
; message += 'Crash Timestamp: $
{ date
} \n\n'
; message += 'Mod Information: Version $
{ modVersion
} \n'
...
; } return message
; } /** * Generates a crash file to insert into the game's crash folder. * @param message The message to be in the file's content
*/ static function buildCrashFile(crashMessage:String)
{ var messageToSave = ""
; messageToSave += '$
{ LINE_SEPERATOR
} \n'
; messageToSave += 'Vs. Dave & Bambi Crash Reporter - $
{ Date.now().toString()
} \n'
; messageToSave += crashMessage
; // TODO: does this work for all platforms? var crashFolder:String = Sys.getCwd() + "/.crashes"
; var crashFileName:String = 'crash-$
{ Date.now().toString().replace(":", "-")
} .log'
; // Save the file into the crash folder. FileUtil.createDirectory(crashFolder)
; File.saveContent('$
{ crashFolder
} /$
{ crashFileName
} ', messageToSave)
; } /** * Generates a list of messages for the crash log message to use based on the error callstack. * @return A list of stack messages to be used by the crash message.
...
; switch (stack)
{ case CFunction: stackItemMessage += "(Function) "
; case Module(m): stackItemMessage += "Module " + m
; case FilePos(s, file, line, column): stackItemMessage += '$
{ file
} -> Line $
{ line
} '
...
; } case Method(className, method): stackItemMessage += 'Class: $
{ className
} , Function: $
{ method
} '
; case LocalFunction(v): stackItemMessage += '(Local Function): $
{ v
} '
