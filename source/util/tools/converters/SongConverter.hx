...
; metadata.version = '$
{ SongRegistry.METADATA_VERSION
} '
; metadata.coders = ["Unknown"]
; chartData.version = '$
{ SongRegistry.CHART_DATA_VERSION
} '
; FileUtil.createDirectory(Paths.data('generated/songs/$entry'))
; File.saveContent(Paths.data('generated/songs/$
{ entry
} /$
{ entry
} -metadata.json'), metadata.serialize())
; File.saveContent(Paths.data('generated/songs/$
{ entry
} /$
{ entry
} -chart.json'), chartData.serialize())
; }
...
; FileUtil.createDirectory(Paths.data('generated/$
{ songData.song
} '))
; File.saveContent(Paths.data('generated/$
{ songData.song
} /$
{ songData.song
} -metadata.json'), metadata.serialize())
...
; // Initalize time signature. timeChangeMap.push(new SongTimeChange(position, curBPM, songNumerator, songDenominator))
...
; if (currentNumerator != sectionNumerator || currentDenominator != sectionDenominator)
...
{ mustHitSection: section.mustHitSection, notes: []
}
; if (section != null)
