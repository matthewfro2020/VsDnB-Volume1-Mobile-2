...
; MEDIA
; } class SUtil
{ // -------- ANDROID PATH HELPERS (Safe replacements) -------- #if android private static var getExternalFilesDir = JNI.createStaticMethod( "org/haxe/lime/LimeActivity", "getExternalFilesDir", "(Ljava/lang/String
; )Ljava/lang/String
; " )
; private static var getFilesDir = JNI.createStaticMethod( "org/haxe/lime/LimeActivity", "getFilesDir", "()Ljava/lang/String
; " )
; private static var getExternalStorageDirectory = JNI.createStaticMethod( "android/os/Environment", "getExternalStorageDirectory", "()Ljava/io/File
; " )
; #end
// -------- STORAGE DIRECTORY HANDLER -------- public static function getStorageDirectory(type:StorageType = MEDIA):String
{ var daPath:String = ""
; #if android switch (type)
{ case DATA: daPath = getFilesDir() + "/"
; case EXTERNAL_DATA: daPath = getExternalFilesDir(null) + "/"
...
; #end return daPath
; } // -------- FILE CHECKER -------- public static function checkFiles():Void
{ #if mobile if (!FileSystem.exists(SUtil.getStorageDirectory()))
{ try
...
{ Lib.application.window.alert( "You didn’t extract assets/mods from the APK!\nCopy them into:\n" + SUtil.getStorageDirectory(), "Error!" )
; LimeSystem.exit(1)
; } #end
} // -------- ERROR HANDLER -------- public static function uncaughtErrorHandler():Void
{ Lib.current.loaderInfo.uncaughtErrorEvents.addEventListener(UncaughtErrorEvent.UNCAUGHT_ERROR, onError)
; Lib.application.onExit.add(function(exitCode:Int)
{ if (Lib.current.loaderInfo.uncaughtErrorEvents.hasEventListener(UncaughtErrorEvent.UNCAUGHT_ERROR)) Lib.current.loaderInfo.uncaughtErrorEvents.removeEventListener(UncaughtErrorEvent.UNCAUGHT_ERROR, onError)
; } )
...
; File.saveContent( SUtil.getStorageDirectory() + "logs/" + Lib.application.meta.get("file") + "-" + Date.now().toString().replace(" ", "-").replace(":", "'") + ".txt", msg + "\n" )
; } catch (ex)
{ LimeLogger.println("Couldn't save crash log:\n" + ex)
; } #end LimeLogger.println(msg)
; Lib.application.window.alert(msg, "Error!")
; LimeSystem.exit(1)
; } // -------- DIRECTORY CREATION (safe) -------- #if sys public static function mkDirs(directory:String):Void
{ var total = ""
; if (directory.startsWith("/")) total = "/"
