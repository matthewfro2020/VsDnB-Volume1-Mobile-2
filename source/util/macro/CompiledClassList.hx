...
; /** * Class lists are injected into this
class 's metadata during the typing phase. * This function extracts the metadata, at runtime, and stores it in `classLists`.
*/ static function init():Void
{ initialized = true
; // Meta.getType returns Dynamic<Array<Dynamic>>. var metaData = Meta.getType(CompiledClassList)
; if (metaData.classLists != null)
{ for (list in metaData.classLists)
{ var data:Array<Dynamic> = cast list
; // First element is the list ID. var id:String = cast data[0]
; // All other elements are
class types. var classes:List<Class<Dynamic>> = new List()
...
; // Make the error only appear once.
} // A faulty request sets the value to an empty list above so this will never be null @:nullSafety(Off) return classLists.get(request)
