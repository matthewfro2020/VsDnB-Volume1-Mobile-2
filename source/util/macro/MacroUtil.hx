...
*/ @:nullSafety
class MacroUtil
{ /** * Gets the value of a Haxe compiler define. * * @param key The name of the define to get the value of. * @param defaultValue The value to return if the define is not set. * @return An expression containing the value of the define.
...
; } /** * Gets the current date and time (at compile time). * @return A `Date` object containing the current date and time.
...
// /** * Convert an ExprOf<Class<T>> to a ClassType. * @see https:
// github.com/jasononeil/compiletime/blob/master/src/CompileTime.hx#L201 * @param e The expression to convert. * @return The `ClassType`
...
; // Follow typedefs to get the actual
class type. var classTypeParsed:Type = Context.follow(classType, false)
; switch (classTypeParsed)
{ case TInst(t, params): return t.get()
; default:
// We couldn't parse this
class type.
// This function may need to be updated to be more robust. throw 'Class type could not be parsed: $
{ fullClassName
} '
; }
} return null
; } /** * Determine whether a field is static. * @param field The field to check. * @return Whether the field is static.
...
; } /** * Determine whether two classes are equal. * @param class1 The first
class to compare. * @param class2 The second
class to compare. * @return Whether the two classes are equivalent.
...
; } /** * Retrieve a ClassType from a string name. * @param name The name of the
class to retrieve. * @return The `ClassType`
*/ public static function getClassType(name:String):ClassType
{ switch (Context.getType(name))
{ case TInst(t, _params): return t.get()
; default: throw 'Class type could not be parsed: $
{ name
} '
; }
} /** * If we are in a build macro, return whether a field already exists on the current
class . * @param name The name of the field to check for. * @return Whether the field already exists.
...
{ return true
; }
} // recursively check superclasses return fieldAlreadyExistsSuper(name, superClass.superClass?.t.get())
; } return fieldAlreadyExistsSuper(name, Context.getLocalClass().get().superClass?.t.get())
; } /** * Determine whether a given ClassType is a subclass of a given superclass. * @param classType The
class to check. * @param superClass The superclass to check for. * @return Whether the
...
; } return false
; } /** * Determine whether a given ClassType implements a given
interface . * @param classType The
class to check. * @param interfaceType The
interface to check for. * @return Whether the
