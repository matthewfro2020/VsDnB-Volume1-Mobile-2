...
; #end
/** This
class is a video player that uses LibVLC for seamless integration with OpenFL display objects.
...
*/ @:noCompletion private static final MAX_AUDIO_BUFFER_COUNT:Int = DefineMacro.getInt('HXVLC_MAX_AUDIO_BUFFER_COUNT', 255)
; #end
/** * Regular expression used to validate the structure of a URL. * * This regex checks that the URL: * * 1. Starts with a valid protocol (letters, digits, or special characters like '+', '.', or '-'). * 2. Contains ":
// " after the protocol. * 3. Does not contain spaces after the ":
// ". * * This validation does not restrict specific protocols, so it allows any protocol * that follows the general URL format (e.g., https:
// , ftp:
// , file:
// ). * * Example: * - "https:
// example.com" -> valid * - "ftp:
// files.server" -> valid * - "invalid_url:
// something" -> invalid * - "https:
// example.com" -> invalid (space after ':
// ')
*/ @:noCompletion private static final URL_VERIFICATION_REGEX:EReg = ~/^[a-zA-Z][a-zA-Z\d+\-.]*:\/\/[^\s]*$/
; /** Enables hardware rendering (GPU textures) if supported
; otherwise, falls back to software rendering (CPU).
...
; @:noCompletion private final mediaMutex:Mutex = new Mutex()
; @:noCompletion private final textureMutex:Mutex = new Mutex()
; #if lime_openal @:noCompletion private final alMutex:Mutex = new Mutex()
; #end @:noCompletion private var mediaInput:Null<BytesInput>
; @:noCompletion private var mediaPlayer:Null<Pointer<LibVLC_Media_Player_T>>
; @:noCompletion private var textureWidth:UInt32 = 0
; @:noCompletion private var textureHeight:UInt32 = 0
; @:noCompletion private var texturePlanes:Null<BytesData>
; #if lime_openal @:noCompletion private var alUseEXTFLOAT32:Null<Bool>
; @:noCompletion private var alUseEXTMCFORMATS:Null<Bool>
; @:noCompletion private var alSource:Null<ALSource>
; @:noCompletion private var alBufferPool:Null<Array<ALBuffer>>
; @:noCompletion private var alSamples:Null<BytesData>
; @:noCompletion private var alSampleRate:UInt32 = 0
; @:noCompletion private var alFormat:Int = 0
; @:noCompletion private var alFrameSize:UInt32 = 0
; #end
/** * Initializes a Video object. * * @param smoothing Whether or not the object is smoothed when scaled.
*/ public function new(smoothing:Bool = true):Void
{ super(null, AUTO, smoothing)
; { while (Handle.loading) Sys.sleep(0.05)
; Handle.init()
; } Gc.doNotKill(this)
; } /** * Loads media from the specified location. * * @param location The location of the media file or stream. * @param options Additional options to configure the media. * @return `true` if the media was loaded successfully, `false` otherwise.
...
; if (URL_VERIFICATION_REGEX.match(location)) mediaItem = Pointer.fromRaw(LibVLC.media_new_location(Handle.instance.raw, location))
; else mediaItem = Pointer.fromRaw(LibVLC.media_new_path(Handle.instance.raw, Util.normalizePath(location)))
...
; mediaItem = Pointer.fromRaw(LibVLC.media_new_callbacks(Handle.instance.raw, untyped media_open, untyped media_read, untyped media_seek, untyped NULL, untyped __cpp__('this')))
; mediaMutex.release()
; } else return false
; } else return false
; if (mediaPlayer == null)
...
; setupVideo()
; setupEvents()
; } else trace('Unable to initialize the LibVLC media player.')
; } if (mediaItem != null)
{ setMediaToPlayer(mediaItem, options)
; return true
; } else trace('Unable to initialize the LibVLC media item.')
; return false
; } /** * Loads a media subitem from the current media's subitems list at the specified index. * * @param index The index of the subitem to load. * @param options Additional options to configure the loaded subitem. * @return `true` if the subitem was loaded successfully, `false` otherwise.
...
; } /** * Parses the current media item with the specified options. * * @param parse_flag The parsing option. * @param timeout The timeout in milliseconds. * @return `true` if parsing succeeded, `false` otherwise.
...
; } else trace('Unable to initialize the LibVLC media event manager.')
...
} /** * Adds a slave to the current media player. * * @param type The slave type. * @param uri URI of the slave (should contain a valid scheme). * @param select `true` if this slave should be selected when it's loaded. * @return `true` on success, `false` otherwise.
...
; } /** * Gets the description of available audio tracks of the current media player. * * @return The list containing descriptions of available audio tracks.
...
; if (rawDescription != null) getDescription(rawDescription, description)
; } return description
; } /** * Gets the description of available audio tracks of the current media player. * * @return The list containing descriptions of available audio tracks.
...
; if (rawDescription != null) getDescription(rawDescription, description)
; } return description
; } /** * Gets the description of available available video subtitles of the current media player. * * @return The list containing descriptions of available available video subtitles.
...
; if (rawDescription != null) getDescription(rawDescription, description)
; } return description
; } /** * Starts playback. * * @return `true` if playback started successfully, `false` otherwise.
*/ public function play():Bool
{ return mediaPlayer != null && LibVLC.media_player_play(mediaPlayer.raw) == 0
; } /** Stops playback.
*/ public function stop():Void
{ if (mediaPlayer != null) LibVLC.media_player_stop(mediaPlayer.raw)
; } /** Pauses playback.
*/ public function pause():Void
{ if (mediaPlayer != null) LibVLC.media_player_set_pause(mediaPlayer.raw, 1)
; } /** Resumes playback.
*/ public function resume():Void
{ if (mediaPlayer != null) LibVLC.media_player_set_pause(mediaPlayer.raw, 0)
; } /** Toggles the pause state.
*/ public function togglePaused():Void
{ if (mediaPlayer != null) LibVLC.media_player_pause(mediaPlayer.raw)
; } /** Moves to the previous chapter, if supported.
*/ public function previousChapter():Void
{ if (mediaPlayer != null) LibVLC.media_player_previous_chapter(mediaPlayer.raw)
; } /** Moves to the next chapter, if supported.
*/ public function nextChapter():Void
{ if (mediaPlayer != null) LibVLC.media_player_next_chapter(mediaPlayer.raw)
; } /** * Retrieves metadata for the current media item. * * @param e_meta The metadata type. * @return The metadata value as a string, or `null` if not available.
...
; LibVLC.media_release(currentMediaItem.raw)
; return meta
; } else LibVLC.media_release(currentMediaItem.raw)
; }
} return null
; } /** * Sets metadata for the current media item. * * @param e_meta The metadata type. * @param value The metadata value.
...
} /** * Saves the metadata of the current media item. * * @return `true` if the metadata was saved successfully, `false` otherwise.
...
; #if lime_openal alMutex.acquire()
; { if (alSource != null)
{ if (AL.getSourcei(alSource, AL.SOURCE_STATE) != AL.STOPPED) AL.sourceStop(alSource)
; for (alBuffer in AL.sourceUnqueueBuffers(alSource, AL.getSourcei(alSource, AL.BUFFERS_QUEUED))) AL.deleteBuffer(alBuffer)
...
; #end
} @:noCompletion private function get_mrl():Null<String>
{ if (mediaPlayer != null)
{ final currentMediaItem:Pointer<LibVLC_Media_T> = Pointer.fromRaw(LibVLC.media_player_get_media(mediaPlayer.raw))
; if (currentMediaItem != null)
...
; } @:noCompletion private function get_stats():Null<Stats>
{ if (mediaPlayer != null)
{ final currentMediaItem:Pointer<LibVLC_Media_T> = Pointer.fromRaw(LibVLC.media_player_get_media(mediaPlayer.raw))
; if (currentMediaItem != null)
...
{ if (mediaPlayer != null) LibVLC.media_player_set_time(mediaPlayer.raw, value)
; return value
...
{ if (mediaPlayer != null) LibVLC.media_player_set_position(mediaPlayer.raw, value)
; return value
...
{ if (mediaPlayer != null) LibVLC.media_player_set_chapter(mediaPlayer.raw, value)
; return value
...
{ if (mediaPlayer != null) LibVLC.media_player_set_rate(mediaPlayer.raw, value)
; return value
...
; if (track[0].i_type != LibVLC_Track_Video || LibVLC.video_get_track(mediaPlayer.raw) != track[0].i_id) continue
...
{ #if lime_openal return alSource != null ? Math.floor(AL.getSourcef(alSource, AL.GAIN) * 100) : -1
; #else return -1
; #end
} @:noCompletion private function set_volume(value:Int):Int
{ #if lime_openal if (alSource != null) AL.sourcef(alSource, AL.GAIN, Math.abs(value / 100))
; #end return value
...
{ if (mediaPlayer != null) LibVLC.media_player_set_role(mediaPlayer.raw, value)
; return value
; } @:noCompletion private function get_videoTrackCount():Int
{ return mediaPlayer != null ? LibVLC.video_get_track_count(mediaPlayer.raw) : -1
; } @:noCompletion private function get_videoTrack():Int
{ return mediaPlayer != null ? LibVLC.video_get_track(mediaPlayer.raw) : -1
; } @:noCompletion private function set_videoTrack(value:Int):Int
{ if (mediaPlayer != null) LibVLC.video_set_track(mediaPlayer.raw, value)
; return value
; } @:noCompletion private function get_audioTrackCount():Int
{ return mediaPlayer != null ? LibVLC.audio_get_track_count(mediaPlayer.raw) : -1
; } @:noCompletion private function get_audioTrack():Int
{ return mediaPlayer != null ? LibVLC.audio_get_track(mediaPlayer.raw) : -1
; } @:noCompletion private function set_audioTrack(value:Int):Int
{ if (mediaPlayer != null) LibVLC.audio_set_track(mediaPlayer.raw, value)
; return value
; } @:noCompletion private function get_audioDelay():Int64
{ return mediaPlayer != null ? LibVLC.audio_get_delay(mediaPlayer.raw) : 0
; } @:noCompletion private function set_audioDelay(value:Int64):Int64
{ if (mediaPlayer != null) LibVLC.audio_set_delay(mediaPlayer.raw, value)
; return value
; } @:noCompletion private function get_spuTrackCount():Int
{ return mediaPlayer != null ? LibVLC.video_get_spu_count(mediaPlayer.raw) : -1
; } @:noCompletion private function get_spuTrack():Int
{ return mediaPlayer != null ? LibVLC.video_get_spu(mediaPlayer.raw) : -1
; } @:noCompletion private function set_spuTrack(value:Int):Int
{ if (mediaPlayer != null) LibVLC.video_set_spu(mediaPlayer.raw, value)
; return value
; } @:noCompletion private function get_spuDelay():Int64
{ return mediaPlayer != null ? LibVLC.video_get_spu_delay(mediaPlayer.raw) : 0
; } @:noCompletion private function set_spuDelay(value:Int64):Int64
{ if (mediaPlayer != null) LibVLC.video_set_spu_delay(mediaPlayer.raw, value)
; return value
...
; __imageVersion = -1
; return __bitmapData
; } @:keep @:noCompletion @:noDebug @:unreflective private function mediaOpen(sizep:RawPointer<UInt64>):Int
{ mediaMutex.acquire()
; if (mediaInput != null)
...
; } mediaMutex.release()
; return -1
; } @:keep @:noCompletion @:noDebug @:unreflective private function mediaRead(buf:RawPointer<UInt8>, len:SizeT):SSizeT
{ mediaMutex.acquire()
; final bytesRead:Int = mediaInput != null ? Util.readFromInput(mediaInput, Pointer.fromRaw(buf), cast len) : -1
; mediaMutex.release()
; return bytesRead
; } @:keep @:noCompletion @:noDebug @:unreflective private function mediaSeek(offset:UInt64):Int
{ mediaMutex.acquire()
; if (mediaInput != null)
...
; } mediaMutex.release()
; return -1
; } @:keep @:noCompletion @:noDebug @:unreflective private function videoLock(planes:RawPointer<RawPointer<cpp.Void>>):RawPointer<cpp.Void>
{ textureMutex.acquire()
; if (texturePlanes != null) planes[0] = untyped texturePlanes.getBase().getBase()
; return untyped nullptr
; } @:keep @:noCompletion @:noDebug @:unreflective private function videoUnlock(planes:VoidStarConstStar):Void
{ textureMutex.release()
; } @:keep @:noCompletion @:noDebug @:unreflective private function videoDisplay(picture:RawPointer<cpp.Void>):Void
{ if ((__renderable || forceRendering) && bitmapData != null)
{ if (bitmapData.image != null && bitmapData.readable) updateImage()
; else updateTexture()
; }
} @:keep @:noCompletion @:noDebug @:unreflective private function videoFormatSetup(chroma:CastCharStar, width:RawPointer<UInt32>, height:RawPointer<UInt32>, pitches:RawPointer<UInt32>, lines:RawPointer<UInt32>):Int
{ textureMutex.acquire()
...
// for now we need to calculate it ourselves. if (!calculateVideoSize(Pointer.fromRaw(width), Pointer.fromRaw(height)))
{ width[0] = originalWidth
; height[0] = originalHeight
; } textureWidth = width[0]
; textureHeight = height[0]
; if (texturePlanes == null) texturePlanes = new BytesData()
...
; { @:privateAccess if (useTexture)
{ @:nullSafety(Off) if (Lib.current.stage?.context3D != null)
...
; } else trace('Unable to utilize GPU texture, resorting to CPU-based image rendering.')
; }
} if (onFormatSetup != null) onFormatSetup.dispatch()
; } textureMutex.release()
; } )
; return 1
; } @:keep @:noCompletion @:noDebug @:unreflective private function audioPlay(samples:RawPointer<UInt8>, count:UInt32, pts:Int64):Void
{ #if lime_openal if (alSource != null && alBufferPool != null)
{ alMutex.acquire()
; for (alBuffer in AL.sourceUnqueueBuffers(alSource, AL.getSourcei(alSource, AL.BUFFERS_PROCESSED))) alBufferPool.push(alBuffer)
; final alBuffer:Null<ALBuffer> = alBufferPool.shift()
; if (alBuffer == null)
{ alMutex.release()
; return
; } if (alSamples == null) alSamples = new BytesData()
...
; if (AL.getSourcei(alSource, AL.SOURCE_STATE) != AL.PLAYING) AL.sourcePlay(alSource)
; } #end
} @:keep @:noCompletion @:noDebug @:unreflective private function audioResume(pts:Int64):Void
{ #if lime_openal if (alSource != null)
{ alMutex.acquire()
; if (AL.getSourcei(alSource, AL.SOURCE_STATE) == AL.PAUSED) AL.sourcePlay(alSource)
; alMutex.release()
; } #end
} @:keep @:noCompletion @:noDebug @:unreflective private function audioPause(pts:Int64):Void
{ #if lime_openal if (alSource != null)
{ alMutex.acquire()
; if (AL.getSourcei(alSource, AL.SOURCE_STATE) != AL.PAUSED) AL.sourcePause(alSource)
; alMutex.release()
; } #end
} @:keep @:noCompletion @:noDebug @:unreflective private function audioFlush(pts:Int64):Void
{ #if lime_openal if (alSource != null)
{ alMutex.acquire()
; if (AL.getSourcei(alSource, AL.SOURCE_STATE) != AL.STOPPED) AL.sourceStop(alSource)
; alMutex.release()
; } #end
} @:keep @:noCompletion @:noDebug @:unreflective private function audioSetup(format:CastCharStar, rate:RawPointer<UInt32>, channels:RawPointer<UInt32>):Int
{ #if lime_openal alMutex.acquire()
; alSampleRate = rate[0]
; if (alSamples == null) alSamples = new BytesData()
; if (alUseEXTFLOAT32 == null) alUseEXTFLOAT32 = AL.isExtensionPresent('AL_EXT_FLOAT32')
; if (alUseEXTMCFORMATS == null) alUseEXTMCFORMATS = AL.isExtensionPresent('AL_EXT_MCFORMATS')
; var alChannelsToUse:Int = channels[0]
; if (alUseEXTMCFORMATS == true && alChannelsToUse > 8) alChannelsToUse = 8
; else if (alChannelsToUse > 2) alChannelsToUse = 2
...
{ case 1: alFormat = AL.getEnumValue(useFloat32 ? 'AL_FORMAT_MONO_FLOAT32' : 'AL_FORMAT_MONO16')
; alChannelsToUse = 1
; case 2 | 3: alFormat = AL.getEnumValue(useFloat32 ? 'AL_FORMAT_STEREO_FLOAT32' : 'AL_FORMAT_STEREO16')
; alChannelsToUse = 2
; case 4: alFormat = AL.getEnumValue(useFloat32 ? 'AL_FORMAT_QUAD32' : 'AL_FORMAT_QUAD16')
; alChannelsToUse = 4
; case 5 | 6: alFormat = AL.getEnumValue(useFloat32 ? 'AL_FORMAT_51CHN32' : 'AL_FORMAT_51CHN16')
; alChannelsToUse = 6
; case 7 | 8: alFormat = AL.getEnumValue(useFloat32 ? 'AL_FORMAT_71CHN32' : 'AL_FORMAT_71CHN16')
; alChannelsToUse = 8
...
; #else return 1
; #end
} @:keep @:noCompletion @:noDebug @:unreflective private function audioSetVolume(volume:Single, mute:Bool):Void
{
} @:keep @:noCompletion @:noDebug @:unreflective private function eventManagerCallbacks(p_event:RawConstPointer<LibVLC_Event_T>):Void
{ switch (p_event[0].type)
{ case event if (event == LibVLC_MediaPlayerOpening): MainLoop.runInMainThread(function():Void
{ if (isValid() && onOpening != null) onOpening.dispatch()
; } )
; case event if (event == LibVLC_MediaPlayerPlaying): MainLoop.runInMainThread(function():Void
{ if (isValid() && onPlaying != null) onPlaying.dispatch()
; } )
; case event if (event == LibVLC_MediaPlayerStopped): MainLoop.runInMainThread(function():Void
{ if (isValid() && onStopped != null) onStopped.dispatch()
; } )
; case event if (event == LibVLC_MediaPlayerPaused): MainLoop.runInMainThread(function():Void
{ if (isValid() && onPaused != null) onPaused.dispatch()
; } )
; case event if (event == LibVLC_MediaPlayerEndReached): MainLoop.runInMainThread(function():Void
{ if (isValid() && onEndReached != null) onEndReached.dispatch()
; } )
; case event if (event == LibVLC_MediaPlayerEncounteredError): final errmsg:String = LibVLC.errmsg()
; MainLoop.runInMainThread(function():Void
{ if (isValid() && onEncounteredError != null)
{ if (errmsg != null && errmsg.length > 0) onEncounteredError.dispatch(errmsg)
; else onEncounteredError.dispatch('Unknown error')
; }
} )
; case event if (event == LibVLC_MediaPlayerESAdded): final iType:LibVLC_Track_Type = untyped __cpp__('
{ 0
} .u.media_player_es_changed.i_type', p_event[0])
; final iID:Int = untyped __cpp__('
{ 0
} .u.media_player_es_changed.i_id', p_event[0])
; MainLoop.runInMainThread(function():Void
{ if (isValid() && onESAdded != null) onESAdded.dispatch((iType : Int), iID)
; } )
; case event if (event == LibVLC_MediaPlayerESDeleted): final iType:LibVLC_Track_Type = untyped __cpp__('
{ 0
} .u.media_player_es_changed.i_type', p_event[0])
; final iID:Int = untyped __cpp__('
{ 0
...
; MainLoop.runInMainThread(function():Void
{ if (isValid() && onESDeleted != null) onESDeleted.dispatch((iType : Int), iID)
; } )
; case event if (event == LibVLC_MediaPlayerESSelected): final iType:LibVLC_Track_Type = untyped __cpp__('
{ 0
} .u.media_player_es_changed.i_type', p_event[0])
; final iID:Int = untyped __cpp__('
{ 0
...
; MainLoop.runInMainThread(function():Void
{ if (isValid() && onESSelected != null) onESSelected.dispatch((iType : Int), iID)
; } )
; case event if (event == LibVLC_MediaPlayerCorked): MainLoop.runInMainThread(function():Void
{ if (isValid() && onCorked != null) onCorked.dispatch()
; } )
; case event if (event == LibVLC_MediaPlayerUncorked): MainLoop.runInMainThread(function():Void
{ if (isValid() && onUncorked != null) onUncorked.dispatch()
; } )
; case event if (event == LibVLC_MediaPlayerTimeChanged): final newTime:Int64 = untyped __cpp__('
{ 0
} .u.media_player_time_changed.new_time', p_event[0])
; MainLoop.runInMainThread(function():Void
{ if (isValid() && onTimeChanged != null) onTimeChanged.dispatch(newTime)
; } )
; case event if (event == LibVLC_MediaPlayerPositionChanged): final newPosition:Single = untyped __cpp__('
{ 0
} .u.media_player_position_changed.new_position', p_event[0])
; MainLoop.runInMainThread(function():Void
{ if (isValid() && onPositionChanged != null) onPositionChanged.dispatch(newPosition)
; } )
; case event if (event == LibVLC_MediaPlayerLengthChanged): final newLength:Int64 = untyped __cpp__('
{ 0
} .u.media_player_length_changed.new_length', p_event[0])
; MainLoop.runInMainThread(function():Void
{ if (isValid() && onLengthChanged != null) onLengthChanged.dispatch(newLength)
; } )
; case event if (event == LibVLC_MediaPlayerChapterChanged): final newChapter:Int = untyped __cpp__('
{ 0
} .u.media_player_chapter_changed.new_chapter', p_event[0])
; MainLoop.runInMainThread(function():Void
{ if (isValid() && onChapterChanged != null) onChapterChanged.dispatch(newChapter)
; } )
; case event if (event == LibVLC_MediaPlayerMediaChanged): MainLoop.runInMainThread(function():Void
{ if (isValid() && onMediaChanged != null) onMediaChanged.dispatch()
; } )
; case event if (event == LibVLC_MediaParsedChanged): final newStatus:Int = untyped __cpp__('
{ 0
} .u.media_parsed_changed.new_status', p_event[0])
; MainLoop.runInMainThread(function():Void
{ if (isValid() && onMediaParsedChanged != null) onMediaParsedChanged.dispatch(newStatus)
; } )
; case event if (event == LibVLC_MediaMetaChanged): MainLoop.runInMainThread(function():Void
{ if (isValid() && onMediaMetaChanged != null) onMediaMetaChanged.dispatch()
; } )
; }
} @:noCompletion @:noDebug @:unreflective private inline function isValid():Bool
{ return mediaPlayer != null
; } @:noCompletion @:noDebug @:unreflective private function updateImage():Void
{ textureMutex.acquire()
...
; } if (onDisplay != null) onDisplay.dispatch()
; textureMutex.release()
; } @:noCompletion @:noDebug @:unreflective private function updateTexture():Void
{ MainLoop.runInMainThread(function():Void
{ if (!isValid() || bitmapData == null || texturePlanes == null) return
; textureMutex.acquire()
...
; if (__renderable) __setRenderDirty()
; } if (onDisplay != null) onDisplay.dispatch()
; textureMutex.release()
; } )
; } @:noCompletion @:noDebug @:unreflective private function getDescription(rawDescription:Pointer<LibVLC_Track_Description_T>, description:Array<TrackDescription>):Void
...
; } @:noCompletion @:noDebug @:unreflective private function setMediaToPlayer(mediaItem:Pointer<LibVLC_Media_T>, ?options:Array<String>):Void
{ if (mediaPlayer == null) return
; if (options != null)
{ for (option in options)
{ if (option != null && option.length > 0) LibVLC.media_add_option(mediaItem.raw, option)
; }
} LibVLC.media_player_set_media(mediaPlayer.raw, mediaItem.raw)
; LibVLC.media_release(mediaItem.raw)
; } @:noCompletion @:noDebug @:unreflective private function setupVideo():Void
{ if (mediaPlayer == null) return
...
; } @:noCompletion @:noDebug @:unreflective private function setupAudio():Void
{ if (mediaPlayer == null) return
; #if lime_openal if (alSource == null) alSource = AL.createSource()
; if (alBufferPool == null) alBufferPool = AL.genBuffers(MAX_AUDIO_BUFFER_COUNT)
; #end LibVLC.audio_set_callbacks(mediaPlayer.raw, untyped audio_play, untyped audio_pause, untyped audio_resume, untyped audio_flush, untyped NULL, untyped __cpp__('this'))
...
; } @:noCompletion @:noDebug @:unreflective private function setupEvents():Void
{ if (mediaPlayer != null)
...
; } else trace('Unable to initialize the LibVLC media player event manager.')
; }
} @:noCompletion @:noDebug @:unreflective private function addEvent(eventManager:Pointer<LibVLC_Event_Manager_T>, type:Int):Void
{ if (LibVLC.event_attach(eventManager.raw, type, untyped event_manager_callbacks, untyped __cpp__('this')) != 0) trace('Failed to attach event ($
{ LibVLC.event_type_name(type)
} )')
; } /** * @see https:
// github.com/obsproject/obs-studio/blob/5d1f0efc43c64c25f5edd4101bc1f0013bcacb60/plugins/vlc-video/vlc-video-source.c#L385
*/ @:noCompletion @:noDebug @:unreflective private function calculateVideoSize(width:Pointer<UInt32>, height:Pointer<UInt32>):Bool
{ if (mediaPlayer != null)
{ final currentMediaItem:Pointer<LibVLC_Media_T> = Pointer.fromRaw(LibVLC.media_player_get_media(mediaPlayer.raw))
; if (currentMediaItem != null)
{ final tracks:RawPointer<RawPointer<LibVLC_Media_Track_T>> = untyped nullptr
...
; if (trackWidth == 0 || trackHeight == 0) break
